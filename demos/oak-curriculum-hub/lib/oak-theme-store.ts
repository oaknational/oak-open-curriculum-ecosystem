 ...
 * choice from an OS-triggered default (PR #637 review; the hub instance is
 * recorded on MCP-372). The store reads the runtime's persisted choice —
 * plus its own record of writes made through it, which survives a failed
 * persistence write — and renders '' when no explicit choice exists. The
 * runtime's collapsing get() remains correct for pre-paint application
 * only. Motion needs no such split: the runtime's 'system' IS its
 * no-choice semantic (no attribute set). Snapshots stay primitive
 * (string | undefined) — `useSyncExternalStore` re-render stability rests
 * on `Object.is` over these values, so combining axes into one object
 * snapshot would re-render unboundedly. The storage key is the runtime's
 * contract, read here and written only by the runtime; a `choice()`
 * runtime accessor is the recorded kit-contract cure (MCP-388).
 *
 ...
 *
 * Second in-estate copy of the showcase's adapter CONTRACT
 * (demos/oak-design-showcase/lib/oak-theme-store.ts) — CONVERGED on the
 * choice-model cure as of MCP-372 slice 1 (the showcase landed it in
 * PR #637). Canonical owner: the owned React binding tier per ADR-213
 * (React code never enters the CSS package) — intake recorded on MCP-134;
 * the final shape waits on the kit's choice() accessor (MCP-388).
 */
 ...
 *  choice exists — the state where the page default (or the OS contrast
 *  route) governs. A select pinned to a real value in that state would
 *  misreport the page AND make the first click on that value a dead
...
export function createOakThemeStore(
  ...
  return {
    ...
    // Call contract: ThemeSwitcher reads options only after the snapshot
    // gate (theme/motion defined ⇒ runtime present) — it renders null
    // pre-hydration, so no shell path reads them either. The fallbacks are
