...
const config = defineConfigArray(
  {
    ignores: [
      ...
      '*.log',
      '.turbo/**',
      // Generated data artefact: the canonical EEF corpus snapshot is a large
      // data module (~2.2k lines), not authored code. Code-quality rules such
      // as max-lines are a category error for it; the loader validates it
      // strictly through `EefToolkitSchema` at the boundary, and type-check +
      // prettier still apply. Regenerated by the gate-1b refresh script.
      'src/eef-strands/eef-toolkit.ts',
    ],
  },
