...
describe('generateSearchIndexModule', () => {
  ...
  it('re-exports all es-mapping constants and types including threads and zero-hit', () => {
    const files = generateSearchIndexModule(MINIMAL_SCHEMA);
    const content = files['../search/index.ts'];

    expect(content).toContain('OAK_THREADS_MAPPING');
    expect(content).toContain('OakThreadsMapping');
    expect(content).toContain('OAK_ZERO_HIT_MAPPING');
    expect(content).toContain('OakZeroHitMapping');
  });
