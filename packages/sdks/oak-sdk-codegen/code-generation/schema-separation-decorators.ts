 ...
 * Threads are data concepts without canonical URLs on the website,
 * so their canonicalUrl field should be typed as null, not string.
 */
...
function decorateSchema(
  ...
  if (hasPropertiesField(working)) {
    return decorateObjectWithCanonicalUrl(working, useNullType);
  }
...
function decorateObjectWithCanonicalUrl(
  ...
  const canonical: SchemaObject = useNullType
    ? {
        type: 'null',
        description:
