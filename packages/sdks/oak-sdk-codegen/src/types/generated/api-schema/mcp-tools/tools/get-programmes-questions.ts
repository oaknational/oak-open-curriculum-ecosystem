...
export const toolMcpFlatInputSchema = z.strictObject({ programme: z.string().describe("The programme slug identifier").meta({ examples: ["computing-secondary-year-7"] }), offset: z.number().describe("If limiting results returned, this allows you to return the next set of results, starting at the given offset point").meta({ examples: [0] }).optional(), limit: z.number().lte(300).describe("Limit the number of lessons, e.g. return a maximum of 300 lessons").meta({ examples: [20] }).optional(), filter: z.enum(["images"] as const).describe("Optional filter for question results. Use `images` to return only questions with a question image or image answer.").optional() });
