...
const inputSchema = { type: 'object' as const, properties: {"sequence":{"type":"string","description":"The sequence slug identifier, including the key stage 4 option where relevant."},"year":{"type":"number","description":"The year group to filter by. For the physical-education-primary sequence, a value of all-years can also be used."},"offset":{"type":"number","default":0},"limit":{"type":"number","default":10}} as const, additionalProperties: false as const, required: ["sequence"] };
 ...
 */
export const getSequencesQuestions: ToolDescriptor = {
  executor,
 ...
 */
export const getSequencesQuestionsTool: OakMcpToolBase<unknown, unknown> = {
  ...
  zodInputSchema: z.object({ path: z.object({ sequence: z.string() }), query: z.object({ year: z.number().optional(), offset: z.number().optional(), limit: z.number().optional() }).optional() }),
  zodOutputSchema: z.any(), // Response schema will be resolved at runtime
  validateInput: (input: unknown) => {
    const result = z.object({ path: z.object({ sequence: z.string() }), query: z.object({ year: z.number().optional(), offset: z.number().optional(), limit: z.number().optional() }).optional() }).safeParse(input);
    if (result.success) {
  ...
  validateOutput: (data: unknown) => {
    const schema: ZodSchema = getResponseSchemaForEndpoint('get', '/sequences/{sequence}/questions');
