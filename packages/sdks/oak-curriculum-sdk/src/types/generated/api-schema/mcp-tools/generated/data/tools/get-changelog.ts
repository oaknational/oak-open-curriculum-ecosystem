...
if (!primaryResponseDescriptor) {
  throw new TypeError('Missing response descriptor for documented status 200 on changelog-changelog.');
}
...
export const getChangelog = {
  invoke: async (client: OakApiPathBasedClient, args: ToolArgs) => {
    ...
    const endpoint = client["/changelog"];
    ...
    if (!descriptorForStatus) {
      throw new TypeError(`Undocumented response status ${String(status)} for changelog-changelog. Documented statuses: 200`);
    }
