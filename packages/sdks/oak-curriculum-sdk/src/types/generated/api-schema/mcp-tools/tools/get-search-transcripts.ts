...
const inputSchema = { type: 'object' as const, properties: {"q":{"type":"string","description":"A snippet of text to search for in the lesson video transcripts"}} as const, additionalProperties: false as const, required: ["q"] };
 ...
 */
export const getSearchTranscripts: ToolDescriptor = {
  executor,
 ...
 */
export const getSearchTranscriptsTool: OakMcpToolBase<unknown, unknown> = {
  name: 'get-search-transcripts',
  description: 'Search for a term and find the 5 most similar lessons whose video transcripts contain similar text.',
  inputSchema: { type: "object", properties: {}, additionalProperties: false },
  outputSchema: { type: "object", properties: {}, additionalProperties: false },
  zodInputSchema: z.object({ query: z.object({ q: z.string() }) }),
  zodOutputSchema: z.any(), // Response schema will be resolved at runtime
  validateInput: (input: unknown) => {
    const result = z.object({ query: z.object({ q: z.string() }) }).safeParse(input);
    if (result.success) {
  ...
  validateOutput: (data: unknown) => {
    const schema: ZodSchema = getResponseSchemaForEndpoint('get', '/search/transcripts');
