...
if (!primaryResponseDescriptor) {
  throw new TypeError('Missing response descriptor for documented status 200 on getKeyStages-getKeyStages.');
}
...
export const getKeyStages = {
  invoke: async (client: OakApiPathBasedClient, args: ToolArgs) => {
    ...
    if (!descriptorForStatus) {
      throw new TypeError(`Undocumented response status ${String(status)} for getKeyStages-getKeyStages. Documented statuses: 200`);
    }
