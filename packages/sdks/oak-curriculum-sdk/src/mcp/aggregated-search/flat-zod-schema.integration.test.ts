 ...
 * need `flatZodSchema` with `.describe()` and `.meta({ examples })` so
...
import { z } from 'zod';
import { SEARCH_FLAT_ZOD_SCHEMA } from './flat-zod-schema.js';

describe('search flatZodSchema round-trip', () => {
  it('exports a defined flatZodSchema', () => {
    expect(SEARCH_FLAT_ZOD_SCHEMA).toBeDefined();
  });

  it('z.toJSONSchema() produces examples on fields with examples', () => {
    const jsonSchema = z.toJSONSchema(z.object(SEARCH_FLAT_ZOD_SCHEMA));
