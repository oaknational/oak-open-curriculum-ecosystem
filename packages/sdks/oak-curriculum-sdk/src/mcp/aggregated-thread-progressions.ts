...
Ordering semantics: each run is Oak's curriculum order for that subject — years ascending (earliest → latest), and within a year the authored unit order of the subject's sequence; "All years" units follow the year-placed ones. A thread spanning subjects (the modern-language grammar and skill threads run through French, German and Spanish) returns parallel per-subject runs; Oak authors no order across subjects, so runs are never interleaved. At key stage 4 a unit merges its exam-board and tier variants into one node, so a KS4 run is a BIAS, not a guarantee: where boards order their units differently, the earliest authored position wins, and sibling variants of one topic can sit side by side. Read same-year order at KS4 as Oak's typical order, not any one board's.
