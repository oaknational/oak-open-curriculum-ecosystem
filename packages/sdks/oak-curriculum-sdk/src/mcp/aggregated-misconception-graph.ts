...
- threadSlug (+ optional unitOffset/unitLimit): a unit-granular window over one thread in Oak's curriculum order (years ascending within a subject, with units Oak places at no single year following the year-placed ones, then within a year the authored unit order — a unit node merges every board and tier and each unit takes its earliest recorded position where they disagree, so at KS4 the within-year order is a bias towards earliest placement, not any one board's order), default ${String(DEFAULT_THREAD_UNIT_LIMIT)} units per page (maximum ${String(MAX_THREAD_UNIT_LIMIT)}), with totalUnits and hasMore reported so partial coverage is always visible. unitOffset/unitLimit are valid ONLY with threadSlug — combining them with lessonSlugs or unitSlugs is rejected.

Ordering is a deterministic projection of Oak's authored order, not a single global sequence. A thread's units are grouped by subject, each subject's run in curriculum order (years ascending — units Oak places at no single year, the PE swimming units, follow the year-placed ones — then within a year the authored unit order — where a unit's board and tier variants disagree each unit takes its earliest recorded position, so at KS4 the within-year order is a bias towards earliest placement and not any one board's order), and the runs are concatenated — Oak records no order ACROSS subjects, so the join between runs carries no pedagogical claim. A unit's lessons follow the authored lesson order of the programmes that place them; where variants disagree the earliest recorded position is used with a slug tie-break, and a lesson no variant orders follows the ordered ones. What this rules out is the alphabet: the order is never the id-sorted one it used to be.
