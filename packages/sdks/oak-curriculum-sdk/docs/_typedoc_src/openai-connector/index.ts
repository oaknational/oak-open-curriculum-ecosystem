...
function normalizeSearchArgs(args: unknown): {
  ...
  if (args && typeof args === 'object') {
    const q =
      (args as { q?: string; query?: string }).q ?? (args as { q?: string; query?: string }).query;
    const { keyStage, subject, unit } = args as {
      keyStage?: string;
      subject?: string;
      unit?: string;
    };
    if (typeof q === 'string' && q.trim().length > 0) {
      return { q: q.trim(), keyStage, subject, unit };
    }
  }
...
export async function executeOpenAiToolCall(
  ...
  if (name === 'search') {
    ...
    const lessonsData = 'data' in lessons ? (lessons as { data: unknown }).data : null;
    const transcriptsData = 'data' in transcripts ? (transcripts as { data: unknown }).data : null;
  ...
  if (name === 'fetch') {
    ...
    switch (type) {
      case 'lesson': {
        ...
        const data = 'data' in res ? (res as { data: unknown }).data : null;
      ...
      case 'unit': {
        ...
        const data = 'data' in res ? (res as { data: unknown }).data : null;
        // Without context we may not be able to build a programme URL; allow undefined
