...
Fictional. The second counter-brand — drawn on a blank canvas first, designed to be maximally far from Oak _and_ from Educate My Creature, so that the three identities triangulate the contract's range.
...
- A teacher finds the current statutory framework in under a minute and _trusts_ it's current (gazette references on everything).
