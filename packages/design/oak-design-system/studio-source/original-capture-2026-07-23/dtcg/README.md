...
- **Functional values**: 15 tokens carry `color-mix()`/`calc()` expressions (state overlays, hover derivation, density-derived layout) verbatim in `$value` — DTCG has no function syntax. A consuming build should pass them through untouched.
