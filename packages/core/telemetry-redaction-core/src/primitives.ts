 ...
 *
 * @remarks
 * Thin, runtime-agnostic wrappers over `@oaknational/observability`'s
 * recursive redaction policy. These primitives are the building blocks
 * that every Sentry adapter — Node today, browser under L-12 tomorrow —
 * composes to redact the fields of its own vendor-typed event shapes
 * (`Event`, `Breadcrumb`, `Exception`, `Log`, `Span`,
 * `TransactionEvent`). This file contains **no** event-shape logic; the
 * adapters own their own event-shape wiring because vendor Event types
 * diverge between runtimes and preserving those types through a shared
 * neutral helper would require type assertions forbidden by the
 * repository's `no-type-shortcuts` rule.
 *
...
/**
 * Normalises an unknown error-like value into a displayable message.
 *
 * @remarks `instanceof Error` is preferred so that stacks are not lost;
 * non-Error values fall through to `String(...)` which invokes any
 * user-defined `toString()`.
 *
 * @example
 * ```ts
 * describeUnknownError(new Error('boom')); // => 'boom'
 * describeUnknownError('plain');           // => 'plain'
 * describeUnknownError(42);                // => '42'
 * ```
 */
 ...
 * violated invariant inside `@oaknational/observability`.
 */
...
/**
 * Sanitises an unknown value, asserts it's an object, and redacts it.
 *
 * @returns The redacted object, or `undefined` when the input sanitises
 * to a non-object (null, scalar, etc.).
 *
 * @remarks The double-sanitise is intentional — the inner pass forces
 * the input through `sanitiseObject`'s safe-JSON shape; the outer pass
 * re-normalises the post-redaction structure after
 * `redactTelemetryObject` has potentially introduced `'[REDACTED]'`
 * string substitutions in previously-typed positions.
 */
