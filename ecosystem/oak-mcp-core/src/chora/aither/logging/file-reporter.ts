...
/**
 * Simple log object structure for file reporter
 */
export interface FileLogObject {
  level: LogLevel;
  msg: string;
  args: unknown[];
  tag: string;
...
/**
 * File reporter interface
 */
export interface FileReporter {
  log(logObj: FileLogObject): void;
}

/**
 * Creates a file reporter that writes logs to a file
 */
  ...
  return {
    log(logObj: FileLogObject): void {
      try {
        ...
        const logLine = `${timestamp} [${level}] ${logObj.msg}${args ? ' ' + args : ''}\n`;
 ...
 */
const PRIMITIVE_FORMATTERS: Partial<Record<string, (value: unknown) => string | null>> = {
  string: (v) => String(v),
  number: (v) => String(v),
  boolean: (v) => String(v),
  bigint: (v) => {
...
function formatPrimitive(value: unknown): string | null {
  ...
  return formatter !== undefined ? formatter(value) : null;
}
