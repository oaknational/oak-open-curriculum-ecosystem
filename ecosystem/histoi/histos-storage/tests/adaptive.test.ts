...
import type { GThis } from '../src/detect-storage-options';
...
describe('createAdaptiveStorage', () => {
  ...
  describe('file storage', () => {
    ...
    it('should handle has operation', async () => {
      ...
      expect(mockFs.access).toHaveBeenCalledWith(expect.stringContaining('test-key.json'));
    });
    ...
    it('should use custom namespace', async () => {
      ...
      expect(mockFs.mkdir).toHaveBeenCalledWith(expect.stringContaining('custom-ns'), {
        recursive: true,
      });
    });
    ...
    it('should use custom base path', async () => {
      ...
      expect(mockFs.mkdir).toHaveBeenCalledWith(expect.stringContaining('/custom/path'), {
        recursive: true,
      });
    });
  });

  describe.skip('Cloudflare KV storage', () => {
    it('should create KV storage when available', async () => {
      ...
      const { createAdaptiveStorage: createAdaptiveStorageMocked } = await import(
        '../src/adaptive'
      );
      const storage = await createAdaptiveStorageMocked(gThis as unknown as GThis);
  ...
  describe.skip('error handling', () => {
    it('should throw when no storage is available', async () => {
      ...
      await expect(createAdaptiveStorageMocked(gThis as unknown as GThis)).rejects.toThrow(
