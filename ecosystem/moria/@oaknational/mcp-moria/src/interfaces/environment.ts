...
export interface EnvironmentProvider {
  ...
  /**
   * Check if running in development mode (optional)
   * @returns True if in development mode
   */
  isDevelopment?(): boolean;

  /**
   * Check if running in production mode (optional)
   * @returns True if in production mode
   */
  isProduction?(): boolean;

  /**
   * Check if running in test mode (optional)
   * @returns True if in test mode
   */
  isTest?(): boolean;

  /**
   * Get the current runtime environment (optional)
   * @returns The runtime environment identifier (e.g., 'node', 'edge', 'browser')
   */
  getRuntime?(): string;
}
