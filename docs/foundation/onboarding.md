...
| MCP server path            | Build features for the MCP (Model Context Protocol) servers that connect AI tools to curriculum data       | `OAK_API_KEY`, `ELASTICSEARCH_URL`, `ELASTICSEARCH_API_KEY` (HTTP auth mode also needs Clerk keys unless auth is disabled)                                                                                                                                                                                                  |
...
| Strategic / leadership     | Understand vision, impact, and investment case                                                             | None — start with [foundation/VISION.md](../foundation/VISION.md), then [Curriculum Guide](../domain/curriculum-guide.md), then [roadmap milestones](../../.agent/plans/high-level-plan.md). For technical depth: [ADR index](../architecture/architectural-decisions/), [practice.md](../../.agent/directives/practice.md) |
...
See [Build System](../engineering/build-system.md) for the single source of truth on all command definitions, caching, and gate ordering.
...
See [Environment Variables Guide](../operations/environment-variables.md) for complete setup details when you're ready.
...
- When renaming commands in `package.json`, search all markdown files for the old name and update them. See the [Command Naming](../engineering/build-system.md#command-naming-source-of-truth) section in the Build System docs.
...
Keep CI green locally; no `--no-verify` or disabled lint rules. See [Build System](../engineering/build-system.md) for details on caching and gate ordering.
...
- **Start here**: [Curriculum Guide](../domain/curriculum-guide.md) — plain-language overview of how the curriculum is organised, what makes KS4 different, and what it means for search
- **Then read**: [Data Variances](../domain/DATA-VARIANCES.md) — transcript availability, structural patterns, KS4 complexity
...
See the [Development Workflow](../engineering/workflow.md) for the complete lifecycle: branching, TDD, quality gates, commits, PRs, CI, AI sub-agent review, human review, merge, and release.
...
**How AI review fits the PR lifecycle**: AI sub-agent reviews happen during development. They are advisory and inform the code as it is written. CI gates and human review happen at PR time. See the [Development Workflow](../engineering/workflow.md) for the full lifecycle.
...
- [Data Variances](../domain/DATA-VARIANCES.md) — Subject/key stage differences
- [docs/architecture/architectural-decisions/](../architecture/architectural-decisions/) — ADRs
- [docs/governance/development-practice.md](../governance/development-practice.md) — Code standards
...
- [Release and Publishing](../engineering/release-and-publishing.md) — SDK publishing, operator runbook, rollback procedures
- [Extension Points](../engineering/extending.md) — How to add new MCP tools, search indices, SDK helpers, and core packages

If you run into problems, check the [Troubleshooting Guide](../operations/troubleshooting.md) first.
