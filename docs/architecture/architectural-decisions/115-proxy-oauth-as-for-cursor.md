...
resolution 3. Amended 2026-09-22 (MCP-759): the served AS metadata additively
carries an `agent_auth` block per RFC 8414 Section 2, and this server serves
`GET /auth.md` — see §Metadata Rewriting.
...
- **`agent_auth` (added 2026-09-22, MCP-759)**: an additive RFC 8414 Section 2 field on the same served AS metadata document — `{ agent_auth: { skill: "<self-origin>/auth.md" } }` — following the bootstrap-discovery shape published at <https://github.com/workos/auth.md>. Deliberately partial: that shape's `identity_endpoint`, `claim_endpoint`, `events_endpoint`, `identity_types_supported`, and `identity_assertion` describe an agent-identity registration and claim ceremony this server does not run, so they are omitted rather than pointed at endpoints that would 404. `skill` names this server's own `GET /auth.md` (`src/auth-md.ts`), which documents the standard OAuth 2.1 flow above (RFC 8414 discovery, open RFC 7591 registration at `/oauth/register`, Authorization Code + PKCE at `/oauth/authorize` and `/oauth/token`) as the registration path this server actually offers an autonomous agent. Mounted before `clerkMiddleware` and exempted in `clerk-skip-surfaces.ts`, the same pattern `robots-txt.ts` (MCP-703) established: a public discovery-adjacent document reachable only through the auth vendor is an unfetchable one.
  **Audience scope, stated explicitly rather than left implicit (review finding, 2026-09-22):** since the 2026-09-01 MCP-655 amendment above, the PRM's `authorization_servers[0]` names the upstream's issuer (Clerk), so a client that follows `workos/auth.md`'s own Step 1b to the letter — fetch AS metadata from `authorization_servers[0]` — reaches Clerk's metadata directly and never sees this origin's `agent_auth` block. The block reaches only a client that separately discovers or is configured to read AS metadata from THIS origin — the same population ADR-113's 2026-02-20 amendment and MCP-345 resolution already name (Cursor's pre-2025-11-25-spec discovery; ChatGPT's plugin-submission portal, which derives scopes from this same document). This is a real, established population and not a hypothetical one, but it is narrower than "every MCP client of this server", and `src/auth-md.ts`'s own Step 1 instructs the reader to fetch THIS origin's AS metadata directly for exactly this reason: an agent that already holds `agent_auth.skill` necessarily already fetched this document to find it, so the two are self-consistent for the audience that reaches either one.
  **RFC 9207, for that same audience (second review finding, 2026-09-22 — Copilot round 2, re-verified against Negative 8 below before responding):** reaching this origin's AS metadata at all does not make its `authorization_endpoint`/`token_endpoint` safe to use unconditionally. Negative 8 already documents the mechanism: this document's `issuer` names this origin, but the authorization response the proxy relays carries Clerk's own `iss`, so a client that validates `iss` per RFC 9207 §2.4 must not treat this origin as that issuer. This is a pre-existing, ratified trade-off — nothing here re-opens or fixes it. What was missing, and is now added, is disclosure: `src/auth-md.ts`'s Step 3 and `README.md`'s agent-registration section both now state the mismatch and point a validating client at the upstream's own AS metadata and endpoints (reached via the Step 1 PRM's `authorization_servers[0]`) instead of silently routing it through a leg Negative 8 already knows fails for it. A non-validating client, or one that cannot reach the upstream directly, still has this origin's proxy endpoints as a working compatibility path — that half of Positive 1/2 above is unchanged.
- **OpenAI domain-verification challenge** (`/.well-known/openai-apps-challenge`, added 2026-09-09, MCP-700): not an OAuth surface, but it shares the public `.well-known` tier. Returns the plugin-submission portal's verification token as bare `text/plain`, with no Host check and in every auth mode, so the portal can confirm control of the MCP host. Contract adopted from [OpenAI's plugin submission guide, "Domain verification"](https://developers.openai.com/plugins/deploy/submission) (read 2026-09-09): "The challenge endpoint must return only that plugin's verification token. Do not return JSON, a list of tokens, or multiple tokens from the same URL."
...
- Filtering, dropping, normalising, defaulting or reordering any field of a
  ...
  not a forwarded message, and its `scopes_supported` states the PRM's
  advertised set rather than the upstream list (ADR-113 resolution 3).
...
4. **Resilient**: Object-spread metadata rewriting automatically picks up new Clerk capability fields — except `scopes_supported`, which is stated from `SCOPES_SUPPORTED` (MCP-345) and is not inherited. `SCOPES_SUPPORTED` is generated from `DEFAULT_AUTH_SCHEME.scopes`, which also sets every protected tool's required scopes, so it can carry only scopes the resource requires; advertising an AS-only scope such as `offline_access` would need a separately sourced advertised list, a decision MCP-345 leaves open (Negative 9).
...
| `src/auth-md.ts`                                                   | `GET /auth.md` (MCP-759): the document `agent_auth.skill` points at                                                                                                                                   |
...
| `e2e-tests/auth-enforcement.e2e.test.ts`                           | End-to-end assertions that the PRM names the upstream, the origin's AS metadata endpoints are rewritten to this origin with its other capability fields preserved, and the proxy routes are unchanged |
...
- [`workos/auth.md`: agent-registration bootstrap profile](https://github.com/workos/auth.md) — the `agent_auth`/`/auth.md` shape MCP-759 partially adopts
