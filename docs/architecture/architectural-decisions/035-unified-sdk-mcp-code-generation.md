...
The MCP server and SDK were treating code generation as separate concerns, leading to:
...
We will generate ALL MCP tool types, parameters, and validators at SDK code-generation time, making the entire SDK+MCP system a pure function of the OpenAPI schema.

### Three-Phase Generation Pipeline

1. **codegen.ts** - Generates OpenAPI artifacts and basic MCP tool types
...
OpenAPI Schema (Single Source of Truth)
           ↓
    SDK sdk-codegen (ONE command: pnpm sdk-codegen)
           ↓
...
- **Automatic Updates**: Schema changes require only `pnpm sdk-codegen`
...
├── scripts/
│   ├── codegen.ts              # Phase 1: OpenAPI types
...
- ADR-036: Data-driven code generation pattern (refinement of this ADR)
...
- Implementation plan: `.agent/plans/unified-sdk-mcp-code-generation.md`
- Data-driven refinement: `.agent/plans/data-driven-mcp-code-generation.md`
