# ADR-230: MCP Apps widget compatibility as a per-commit served-surface gate, judged by the vendor's pinned capability catalogue
...
boundary this gate complies with. This ADR carries the decision; the
mechanism, flags, and routed findings that execute it live in delivery
planning, which cites this ADR rather than the reverse (PDR-105).
...
follow-up on MCP-644, and passing the generic test client's negotiated
