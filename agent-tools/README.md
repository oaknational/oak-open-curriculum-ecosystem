...
- `mcp-conformance`: run MCPJam conformance suites (lockfile-installed `@mcpjam/cli`) against a deployed MCP surface — verdicts BY NAME against committed baselines (default), capture observation seeds for authoring baselines via `--seed`, drive every advertised tool once with its advertised example inputs and render the reviewer walkthrough pack via `--drive`, or capture per-host compatibility verdicts for the deployed surface via `--compat` (reports, never judges; the wrapper always emits its own aggregate JSON to stdout, including on failure — it is the MCPJam CHILD whose stdout is empty when it fails, which is why compat carries its own evidence gate; see the help text for the full operation list and each operation's exit semantics). Root alias `pnpm -s mcp:conformance`; the `-s` keeps stdout pure JSON wherever a run emits a report.
