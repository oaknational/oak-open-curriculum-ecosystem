...
`PRACTICE_AGENT_SESSION_ID_CURSOR`, or `PRACTICE_AGENT_SESSION_ID_CODEX`). This
is a session cache, not a wordlist compatibility layer: the wordlists remain the
single derivation source for new sessions, while an already-started session
keeps the name it was given even if the wordlists change during active work.
