/**
 * Reviewed post-baseline semantic deltas — the generated MCP tool files
 * (descriptions rewritten in place by MCP-300). Each hash pins the exact
 * reviewed state; item ids cite the audit rows the file carries.
 */
import { reviewed, type CurrentSourceDeltaReview } from './current-source-delta-review-helpers.js';

export const SDK_GENERATED_TOOLS_DELTA_REVIEWS: Readonly<Record<string, CurrentSourceDeltaReview>> =
  {
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-key-stages-subject-assets.ts':
      reviewed('5d933eb1f2665521ca407e42d212747d8a556ef4f40bb026323902f79482432a', [
        'C499',
        'C500',
        'C501',
        'C503',
        'C504',
        'C505',
        'C506',
        'C507',
        'C508',
        'C509',
        'C510',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-key-stages-subject-lessons.ts':
      reviewed('1c1d6ac7231278f3f0647f48bd60457bd3e3a9368a3f6e01d0c97b3ecc21ed18', [
        'C511',
        'C512',
        'C514',
        'C515',
        'C516',
        'C517',
        'C518',
        'C519',
        'C520',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-key-stages-subject-questions.ts':
      reviewed('900e4c88e856c1654ac448f0e3890bd1a6050eafd6845600d2e8b2d31bc21afc', [
        'C521',
        'C522',
        'C524',
        'C525',
        'C526',
        'C527',
        'C528',
        'C529',
        'C530',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-key-stages-subject-units.ts':
      reviewed('e156274a83485148ddc216235c6d53afaa2449ee1195e7eebd0e0fbe96052c93', [
        'C531',
        'C532',
        'C534',
        'C535',
        'C536',
        'C537',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-key-stages.ts':
      reviewed('5a0dc6cf978f4a544c6bae3883126d1fb5d388f13831d7841b0bac436e41cdc3', [
        'C538',
        'C539',
        'C541',
        'C542',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-keywords.ts':
      reviewed('6438ad06c80f66dd74f82ebb52172e2abb9cb6b8621c9838d2ad720b31d6c9eb', [
        'C543',
        'C544',
        'C546',
        'C547',
        'C548',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-lessons-assets.ts':
      reviewed('88ec8ec35fef5d78ed77ac4421e84709123d1bf172709fa409e98ff2f4aef77e', [
        'C549',
        'C550',
        'C551',
        'C553',
        'C554',
        'C555',
        'C556',
        'C557',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-lessons-quiz.ts':
      reviewed('be86910b7339035e6f4c43be975d200b3cd1f33f89aa4535a61cefc24d24f9c3', [
        'C558',
        'C559',
        'C561',
        'C562',
        'C563',
        'C564',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-lessons-summary.ts':
      reviewed('b3ed1f878f5870cc727fd9c4907cec0cf4a7de93cea7d241829556db572114df', [
        'C565',
        'C566',
        'C568',
        'C569',
        'C570',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-lessons-transcript.ts':
      reviewed('98257974279c1aec65ef69982b67b0fbc3e23f8945c0ea6327de51e465229547', [
        'C571',
        'C572',
        'C574',
        'C575',
        'C576',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-programmes-assets.ts':
      reviewed('7312b08407a932c9ca519a7f078ee88cf863dcda6a85ac5332b9ec56f09bab82', [
        'C577',
        'C578',
        'C579',
        'C581',
        'C582',
        'C583',
        'C584',
        'C585',
        'C586',
        'C587',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-programmes-questions.ts':
      reviewed('c0d8fc66ea62ba7e37e8ef826f1803519fd9e9226911fd6fbbd68c1abbb3011b', [
        'C588',
        'C589',
        'C591',
        'C592',
        'C593',
        'C594',
        'C595',
        'C596',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-programmes-units.ts':
      reviewed('704940147530f69f033d9fa08fa76b647e4022e80dc36236ba6e7130b3eecb6c', [
        'C597',
        'C598',
        'C600',
        'C601',
        'C602',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-programmes.ts':
      reviewed('0236c58241aae7751797886298232deb44e05a4407dd84c0ef650fdf6a206506', [
        'C603',
        'C604',
        'C605',
        'C606',
        'C607',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-sequences-assets.ts':
      reviewed('f0e8c1b94606074101a9d2ce8f934642c2dc1713eeb1ff7f9b1a797f21e16790', [
        'C612',
        'C613',
        'C614',
        'C615',
        'C616',
        'C617',
        'C618',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-sequences-questions.ts':
      reviewed('0bb231c30ae5cd4997386507f145ac74c7e7a4f40e3a927760233e5485000ae2', [
        'C619',
        'C620',
        'C621',
        'C622',
        'C623',
        'C624',
        'C625',
        'C626',
        'C627',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-sequences-units.ts':
      reviewed('d29a597d9501df1303f337962e7cc9d07a8511db85f2bef84775f88002c6de07', [
        'C628',
        'C629',
        'C630',
        'C631',
        'C632',
        'C633',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-sequences.ts':
      reviewed('8d889645b1b5dcede5684e993f216ce169b65aa16067e31bd1e32ffa9eebb4d7', [
        'C634',
        'C635',
        'C636',
        'C637',
        'C638',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-subject-detail.ts':
      reviewed('e8855327d05d8dce4c28ccbbc240085d1d6ad6fa72efd671c4a8b8be5351ea68', [
        'C639',
        'C640',
        'C641',
        'C642',
        'C643',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-subjects-key-stages.ts':
      reviewed('ce8b97e1378d1aac70ab08402aa52d2cb53459bab10e3870c4548d06fd409a7c', [
        'C644',
        'C645',
        'C646',
        'C647',
        'C648',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-subjects-programmes.ts':
      reviewed('0e78d46c76c5c2549d74e45a2b3496647b869b912738f9a2430640a0304fe205', [
        'C649',
        'C650',
        'C651',
        'C652',
        'C653',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-subjects-years.ts':
      reviewed('615109f3291f8b30675370767e9613ef8f9ed7e72fa491f0e23d3e57d96434f6', [
        'C654',
        'C655',
        'C656',
        'C657',
        'C658',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-subjects.ts':
      reviewed('3ce7a56b4d415d43e3a1c79a2d52510b2e76c094974bee36b5b1c91c4ce844f7', [
        'C659',
        'C660',
        'C661',
        'C662',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-threads-units.ts':
      reviewed('9068a377f265d7764ed033bf022c29854fc7358fca3ccaf4f0ebd5627b7c5a0d', [
        'C663',
        'C664',
        'C665',
        'C666',
        'C667',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-threads.ts':
      reviewed('d380c2d5a6ef8d8ec6f7a56d6dfeedfe16932645d218b227800f57fc39cf0170', [
        'C668',
        'C669',
        'C670',
        'C671',
      ]),
    'packages/sdks/oak-sdk-codegen/src/types/generated/api-schema/mcp-tools/tools/get-units-summary.ts':
      reviewed('9e61e514ec28f32f06d0316bbc9110c7a4f5dc491b34b7024ccae4aa85de779b', [
        'C672',
        'C673',
        'C674',
        'C675',
        'C676',
      ]),
  };
