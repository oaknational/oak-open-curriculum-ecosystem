#!/usr/bin/env node
/**
 * Cursor CLI status-line adapter for the deterministic agent-identity CLI.
 *
 * @remarks
 * Reads the JSON object Cursor CLI passes on stdin, extracts `session_id`, and
 * prints the deterministic display name produced by the built `agent-identity`
 * CLI at `agent-tools/dist/src/bin/agent-identity.js`.
 *
 * The status line is a soft surface: missing input, missing build artefact, or
...
import {
  ...
  type CursorStatuslinePlan,
} from './statusline-identity-input.js';
...
function emitStatuslineIdentity(rawJson: string): void {
  const plan: CursorStatuslinePlan = planCursorStatuslineExecution(rawJson);
  if (plan.kind === 'noop') {
    return;
  }
