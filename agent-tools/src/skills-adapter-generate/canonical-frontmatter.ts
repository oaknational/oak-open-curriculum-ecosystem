 ...
 * parse — see {@link validateAgentSkillsFrontmatter}.
 *
 ...
 * compatibility ≤500, the `name` regex, the `name`-matches-directory
 * rule) belong to an estate-wide contract check over every canonical
 * rather than to per-emission validation, and NO such check exists
 * anywhere yet — do not read this module as providing one.
 */
import { err, ok, type Result } from '@oaknational/result';
import { z } from 'zod';

/**
 * The Agent Skills frontmatter schema. `z.object` STRIPS unknown keys —
 * unlike the sub-agent wrapper schema, which rejects them — so the parsed
 * value carries exactly the specification's fields and the canonical-only
 * keys stay behind in the source file.
 *
 * The `.min(1)` constraints are repo-added strictness, not the
 * specification: it bounds these fields from above, never below. A
 * declared field holding an empty string is malformed here because
 * projecting it would publish a field that says nothing.
 */
...
/**
 * The outcome of reading a canonical's frontmatter: the validated fields,
 * or a REASON the operator can act on. This is the estate's own
 * `Result<T, E>` (ADR-088, `use-result-pattern`) — a bare `undefined`
 * would throw the reason away at the only point that knows it.
 */
export type FrontmatterRead = Result<CanonicalFrontmatter, string>;

/**
 * Validate a parsed YAML frontmatter value against the specification.
 *
 * Refuses — with the field and the reason named — when a required field is
 * missing or a declared field holds a value the specification does not
 * admit: a non-string `license`, `compatibility`, or `allowed-tools`, or a
 * `metadata` that is not a string→string map (an unquoted YAML
 * `version: 1.0` parses to a number and is refused; the conforming form is
 * the quoted `version: "1.0"`). A malformed field is a REFUSAL rather than
 * a silent drop: emitting the adapter without it would publish a
 * projection that disagrees with its canonical, and dropping it quietly is
 * how a typo'd field stays invisible.
 */
export function validateAgentSkillsFrontmatter(value: unknown): FrontmatterRead {
  const result = canonicalFrontmatterSchema.safeParse(value);
  return result.success ? ok(result.data) : err(describeIssues(result.error.issues));
}

/**
 * Render zod's issues as ONE line — `field: message`, joined by `; ` — so
 * the CLI names the field that refused rather than the file that read
 * fine. `core/schema-parse.ts` renders via `z.prettifyError`, which is
 * multi-line and cannot sit inside a single `<dir> — <reason>` entry;
 * that, not oversight, is why this boundary does not use it.
 */
