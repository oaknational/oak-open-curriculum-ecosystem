...
function messageMatchesRecipient(message: DirectedCommsMessage, agentName: string): boolean {
  return agentName === '*' || message.to.agent_name === agentName;
}

async function readSeenIds(seenFile: string): Promise<ReadonlySet<string>> {
