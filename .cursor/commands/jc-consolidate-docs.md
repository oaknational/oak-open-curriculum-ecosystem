...
5. **Keep `distilled.md` within size constraints.** Beyond deduplication and clarification, the primary mechanism is **extracting established, well-defined concepts to permanent documentation** (ADRs, `/docs/`, READMEs). Permanent docs are discoverable without specialist knowledge; `distilled.md` is part of a specialist knowledge refinement flow and should contain only what has NOT yet matured into permanent documentation. When an entry represents settled practice rather than a fresh gotcha, create the permanent doc first, then remove the entry from `distilled.md`.
