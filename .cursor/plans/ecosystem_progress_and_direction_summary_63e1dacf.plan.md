...
4. **Adjacent threads beyond the seed list** — paragraph each, deliberately broader than what was provided:
   - **Open education knowledge surfaces** — composing Oak with EEF Toolkit evidence, education-skills surfaces, and an international comparator using the Finnish (Opetushallitus) public curriculum API as the first external consumer of the OpenAPI → SDK → MCP pipeline; this validates the pipeline's portability claim with a real second source.
