...
- 2025-10-21 (Codex): Unfiltered `pnpm build` exposed widespread `@typescript-eslint/no-unsafe-*` violations in the semantic search indexing pipeline. Began extracting sanitisation helpers (`document-transform-helpers.ts`) and converting call sites to operate on `unknown` inputs, but additional normalisation and test fixture updates are required before the gate can pass.

- 2025-10-22 (Codex): Completed semantic-search sanitisation rollout across indexing helpers, fixtures, and API route.
  - Changes: Added `document-transform-helpers.unit.test.ts` coverage for the new `resolveUnitSummaryIdentifiers` utilities; rewired `index-bulk-helpers.ts`, `sequence-facets.ts`, `sequence-facet-index.ts`, and the rebuild-rollup route to consume the helper normalisers; normalised sandbox fixtures and introduced `index-bulk-support.ts` / `sequence-facet-utils.ts` to keep max-line budgets under control.
  - Commands: `pnpm --filter @oaknational/open-curriculum-semantic-search lint` ✅, `pnpm --filter @oaknational/open-curriculum-semantic-search test -- src/lib/indexing/document-transform-helpers.unit.test.ts src/lib/indexing/document-transforms.unit.test.ts src/lib/indexing/lesson-planning-snippets.unit.test.ts` ❌ (known `search-index-target.unit.test.ts` expectation mismatch), `pnpm build` ✅, `pnpm lint` ❌ (pre-existing failures in `apps/oak-curriculum-mcp-stdio` and `packages/sdks/oak-curriculum-sdk`).
  - Outcome: Semantic-search workspace now lint-clean with sanitised helpers and fixtures; remaining red gates are confined to legacy packages and the unresolved search-index target test harness.
- 2025-10-23 (Codex): Simplified semantic-search env handling for Next.js deployments.
  - Changes: Replaced the adaptive provider/caching logic with direct `process.env` parsing in `src/lib/env.ts`; rewrote `env.unit.test.ts` to exercise the Next.js/Vercel assumptions and verified `optionalEnv`/`llmEnabled` behaviour. Adjusted tests to treat environment mutations as static per execution.
  - Commands: `pnpm --filter @oaknational/open-curriculum-semantic-search test -- src/lib/env.unit.test.ts` ✅, `pnpm --filter @oaknational/open-curriculum-semantic-search test -- src/lib/search-index-target.unit.test.ts` ✅ (full workspace suite executed by Vitest both times).
  - Outcome: Search index target tests now pass with sandbox expectations; env helpers operate without caching while preserving defaulting and validation guarantees.
