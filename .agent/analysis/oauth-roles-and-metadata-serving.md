...
When we say `authorization_servers: ["https://native-hippo-15.clerk.accounts.dev"]` in our protected resource metadata, where should clients fetch the authorization server metadata from?

**Option A:** `https://native-hippo-15.clerk.accounts.dev/.well-known/oauth-authorization-server` (Clerk)  
...
```
https://native-hippo-15.clerk.accounts.dev/.well-known/oauth-authorization-server
```
