...
- **Sparse** — an anchored projection whose members are selected by the anchor's scope rather
  than mutual reachability: one thread's per-subject curriculum-ordered unit sequence, a unit's lessons in
  Oak's authored teaching order with their misconceptions, the keywords of every lesson matching subject + key stage. Members may be
...
thread's units, or a unit's lessons, in the curriculum's own authored order — is projection, not reasoning: the same
