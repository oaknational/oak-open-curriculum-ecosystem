...
**Positioning:**  
ES-only, graph-adjacent features (Graph Explore API, significant terms, transforms) are the near-term path for search improvements. This document is the **later-phase** Neo4j integration view. When Neo4j is adopted, plan to **export graph-derived views back into Elasticsearch** (paths, concept clusters, derived subjects) to strengthen retrieval and enable novel subjects. For immediate ES-only guidance, see `../methods/graph-elastic.md`.
...
Architecture diagram illustrating the pipeline and query flow:

```text
+----------------------------------------------------------------------------------+
|                                  DATA SOURCES                                     |
|                                                                                  |
|  Oak Bulk Download API                                                           |
|  (Lessons, Units, Keywords, Definitions, Metadata, Curriculum Structure, etc.)   |
+------------------------------------+---------------------------------------------+
                                     |
                                     |  ETL / Processing Pipeline
                                     |  (normalisation, enrichment, embeddings)
                                     v
+----------------------------------------------------------------------------------+
|                                INGESTION LAYER                                   |
|                                                                                  |
|  - Content normalisation                                                         |
|  - Concept extraction (keywords, definitions, misconceptions)                    |
...
+-------------------------------+        +------------------------------------------+
|        ELASTICSEARCH          |        |                NEO4J                     |
|      (Elastic Cloud)          |        |             (AuraDB)                     |
|                               |        |                                          |
|  Hybrid Search Index           |        |  Knowledge Graph                         |
|                               |        |                                          |
|  - BM25 text fields            |        |  Nodes:                                  |
|  - Dense vector embeddings     |        |   • Resource (lesson, unit, etc.)        |
|  - Facets / filters            |        |   • Concept / Topic                      |
|                               |        |   • Subject / Key Stage / Exam Board     |
|                               |        |                                          |
|  Optimised for:                |        |  Relationships:                          |
|   • Keyword search             |        |   • (Resource)-[:TEACHES]->(Concept)     |
|   • Semantic search            |        |   • (Concept)-[:PREREQ_OF]->(Concept)    |
|   • Ranking & recall           |        |   • (Resource)-[:PART_OF]->(Unit)        |
|                               |        |   • (Concept)-[:USED_IN]->(Subject)      |
+---------------+---------------+        +-------------------+----------------------+
                |                                            |
                |                                            |
                |                                            |
...
|                                                                                  |
|  Responsibilities:                                                               |
|  - Orchestrate search + graph queries                                             |
|  - Merge, rank, and enrich results                                                |
|  - Apply curriculum constraints (KS, tier, exam board)                            |
|                                                                                  |
|  Typical Flow:                                                                   |
|   1. User query -> Elasticsearch hybrid search                                   |
|   2. Top results -> Neo4j traversal (context, paths, prerequisites)              |
...
+---------------------------+----------------------+--------------------------------+
                            ...
                            v                      v
+-------------------------------+      +-------------------------------------------+
|         USER INTERFACES       |      |        PROGRAMMATIC CONSUMERS              |
|                               |      |                                           |
|  - Teacher-facing UIs          |      |  - SDKs (TypeScript / Python)              |
|  - Curriculum design tools     |      |  - Internal services                      |
|  - Student learning apps       |      |  - MCP servers for LLMs                   |
|                               |      |                                           |
|  Features enabled:             |      |  Enables:                                 |
|   • Search + discovery         |      |   • Graph-aware RAG                       |
|   • Concept maps               |      |   • Explainable AI answers                |
|   • Learning pathways          |      |   • Safe, bounded traversal               |
+-------------------------------+      +-------------------------------------------+

Legend:
-------
->  Data flow
Elastic = fast retrieval ("what matches?")
Neo4j   = reasoning & traversal ("how is it connected?")
```
...
_(The `index_options` with HNSW is for approximate nearest neighbor search for efficiency.)_ Indexing data involves also generating the vector for each document (e.g. using an ML model). You might do this in the processing step or via Elasticsearch's [`inference` pipeline processor](https://www.elastic.co/guide/en/elasticsearch/reference/current/inference-processor.html) if using Elastic's model deployment. Each document in the index would then have textual fields and the embedding field.
...
- _Elasticsearch Documentation:_ Official docs on the JavaScript/TypeScript client and vector search:
  ...
  - **Vector Search** – Elasticsearch documentation on the `knn` query and hybrid search options (e.g., [Elasticsearch hybrid search documentation](https://www.elastic.co/docs/solutions/search/hybrid-search)).
...
By combining the above approaches – a robust search index, a rich knowledge graph, and modern application code – our system caters to the needs of various users in an education context. Teachers, students, and AI assistants alike can tap into the **merged power of Elasticsearch and Neo4j** to find not just information, but structured knowledge.
    ...
    1.	https://hoop.dev/blog/the-simplest-way-to-make-elasticsearch-neo4j-work-like-it-should/
    2.	https://www.elastic.co/guide/en/elasticsearch/reference/current/inference-processor.html
    3.	https://discuss.elastic.co/t/merging-hybrid-search-results-bm25-hnsw-in-elastic-8-7/330225
    4.	https://www.elastic.co/guide/en/elasticsearch/client/javascript-api/current/introduction.html
    5.	https://www.elastic.co/docs/solutions/search/hybrid-search
    6.	https://neo4j.com/docs/javascript-manual/current/
    7.	https://neo4j.com/cloud/aura
