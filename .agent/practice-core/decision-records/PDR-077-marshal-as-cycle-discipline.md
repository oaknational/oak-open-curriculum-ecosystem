...
Once a marshal-request event is emitted (Marshal-request step), the
referenced claim's scope field is frozen for the duration of that
cycle. Any Director-initiated scope change to a claim that has an
outstanding marshal-request MUST be surfaced as a blocking event to
the marshal before the Claim-verify step proceeds. The Director does
not mutate claim scope unilaterally mid-cycle; the marshal-cycle
Claim-verify step reads against the scope as stated at marshal-request
time.
...
### Orthogonal-Scope Preamble

The Claim-State Immutability Invariant (above) and the Gate-Singleton
Invariant (below) protect orthogonal failure classes: the former
prevents a Director–Marshal authority race on a single claim's scope
mid-cycle; the latter prevents two marshal seats running the husky
gate-chain concurrently against the same working tree. Both can hold
simultaneously without overlap, and each is independently necessary
for cycle integrity.
...
- An additional possible failure mode: marshal-orphaned-claims when
  ...
  combined with the auto-rebalance protocol cures this; see
  §Open Questions for the unresolved protocol shape.
...
1. **No SKILL amendment yet encodes the Commit Marshal as a
   gate-singleton-bearing seat distinct from coordinator with
   two-moments role-transition** in `start-right-team` SKILL §Closeout
   Contract or §Roster Discipline. The amendment is required to make
   the role-class discoverable to future sessions; SKILL surfaces
   already reference "marshal" in three places but without the
   gate-singleton + two-moments encoding that distinguishes the seat
   from coordinator-class roles.
...
4. **Gate-singleton invariant has no structural enforcement
   ...
   is named as the structural cure but not yet landed. When the
   schema landing happens, it composes naturally with the claim-state
   immutability invariant (§Claim-State Immutability Invariant above)
   — both rely on identity-bearing claim-area state, so a single
   schema cycle can land both fields together rather than duplicating
   the schema-edit work across separate landings.
...
(f) The claim-state immutability invariant has been exercised at
    least once in a real Director-initiated mid-cycle scope-change
    attempt, with the blocking-event handshake firing as specified,
    OR the invariant is explicitly marked unexercised in the
    first-independent-session report. The disjunctive form preserves
    falsifiability (the test session may not see a real mid-cycle
    scope-change) while preventing passive ratification through
    non-exercise.

AND throughput materially higher than the single-role baseline for
that session, with the empirical-claim metric normalised per the
floor metric named in §Open Question 5 (cycles per marshal-active-
minute, normalised to size-class) — §OQ5 is the precondition for
this measurement clause, not a downstream observation.
...
4. **`.agent/practice-index.md`** registers PDR-077 in the
   PDR-to-phenotype-ADR table (the same table that enumerates
   PDR-063 through PDR-066 with their phenotype-ADR cross-references
   or explicit phenotype-deferred annotations).
...
The following review findings were absorbed into this PDR's first
landing. Each entry names the class (SHOULD-ABSORB from the R3
synthesis cross-finding matrix, ABSORB for brief-construction items,
VERDICT for Director-decision items) and the section affected.

- [SHOULD-ABSORB] R3: replace ordinal step refs ("step 1" / "step 2")
  in the Claim-State Immutability Invariant with the canonical step
  names (Marshal-request, Claim-verify) — low-cost step-reordering
  immunity. Affected: §Claim-State Immutability Invariant.
  (source: `.agent/state/collaboration/handoffs/2026-05-23-charcoal-pdr-077/pdr-077-r3-final-iteration-fanout-synthesis.md` Finding 3, Betty)
- [SHOULD-ABSORB] R3: orthogonal-scope preamble between the
  Claim-State Immutability and Gate-Singleton invariants — resolves
  scope-boundary ambiguity by naming the orthogonal failure classes
  each invariant protects. Affected: new §Orthogonal-Scope Preamble.
  (source: same Finding 4, assumptions-expert)
- [SHOULD-ABSORB] R3: §Trigger to Graduate measurement clause names
  §OQ5 (cycles per marshal-active-minute, normalised to size-class)
  as the precondition floor metric, not a downstream observation —
  closes the graduation-boundary circular dependency. Affected:
  §Trigger to Graduate.
  (source: same Finding 5, Betty)
- [SHOULD-ABSORB] R3: §OQ4 half-sentence naming shared-schema
  landing with the claim-state immutability invariant — prevents
  duplicative future schema-edit cycles. Affected: §Open Question 4.
  (source: same Finding 6, docs-adr-expert)
- [SHOULD-ABSORB] R3: §OQ1 sharpened from "no SKILL amendment yet
  encodes the Commit Marshal as a third-class role" to the more
  precise "SKILL doesn't encode marshal as gate-singleton-bearing
  seat distinct from coordinator with two-moments role-transition"
  framing — corrects drift-stale wording given that SKILL surfaces
  already reference "marshal" in three places. Affected: §Open
  Question 1.
  (source: same Finding 7, docs-adr-expert)
- [SHOULD-ABSORB] R3: §Required Companion Edits item 4 text
  revised to name the actual locus in `.agent/practice-index.md`
  (the PDR-to-phenotype-ADR table) rather than the abstract
  "governance-PDR enumeration" phrase, with sibling examples
  (PDR-063 through PDR-066) named for unambiguity. Affected:
  §Required Companion Edits.
  (source: same Finding 8, docs-adr-expert)
- [VERDICT] R3: clause (f) added to §Trigger to Graduate covering
  claim-state immutability invariant first-exercise — disjunctive
  form per the Director-recommended composition of
  assumptions-expert's passive-ratification concern and
  docs-adr-expert's falsifiability concern. Affected: §Trigger to
  Graduate.
  (source: same Finding 2, assumptions-expert ABSORB vs
  docs-adr-expert NOT-NEEDED; Director-verdict disjunctive synthesis)
- [ABSORB] R3: marshal-request brief must instruct Mistbound (or
  any first-independent-session marshal) to emit
  `authoring_participation: none` on the Moment-2 acknowledgement
  event — enforceability anchor for §OQ13 self-referential
  ratification bar. This absorption affects marshal-request brief
  construction at landing time, not this PDR's body; the obligation
  is recorded here so the brief construction does not drift.
  (source: same Finding 1, assumptions-expert)
