...
todos:
  ...
  - id: w3-c1-vocabulary-bridge
    content: >-
      ...
      2026-06-11: unitSlugs narrowing, firstYear on keyword nodes; under the
      ADR-086 amendment (2026-09-13) each keyword carries the definitions its
      in-scope lessons authored. Discovery grounding: story 3 of
...
- The agent set-compares the ranked terms: **shared terms** are presented as
  ...
  **new terms** become the pre-teach list, each carrying the definitions its
  lessons in the anchor authored (the agent never invents definitions —
  ADR-194).

Data support verified at `origin/main` (2026-06-11): `unitSlugs` narrowing
within the anchor and `firstYear` on keyword nodes; under the ADR-086
amendment (2026-09-13) each keyword carries the definitions its in-scope
lessons authored, not one corpus-wide description. Discovery grounding and the wider story set (owner-owned
prioritisation):
