...
| 2 | P2 | Cross-subject keyword description leakage: `keyword:convert` (subjects: history, maths) serves "to convert to a new religion or belief…" into maths KS2 keyword results. | **In-repo (S2)** — corpus emission / keyword identity model. Cure: definitions are lesson-authored (ADR-086 amendment, 2026-09-13). |
...
| S2 | Investigation closed — root cause is the repo keyword-extractor first-occurrence-wins collapse (`keyword-extractor.ts:101-105,152-186`); upstream data is per-placement. Cure — a lesson-authored `keywordDefinitions` section beside the edges, served per scope by get-keyword-graph (ADR-086 amendment, 2026-09-13) |
