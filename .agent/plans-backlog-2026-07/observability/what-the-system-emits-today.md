...
| PostHog (Sink 3) | Interactive product analytics | **active** (live in production; 23,649 `$mcp_tool_call` events over 21 days, measured 2026-08-13. MCP-63 Done. The identity-policy gate this row cited is discharged by ADR-218) | [`future/second-backend-evaluation.plan.md`](future/second-backend-evaluation.plan.md) — vendor decision settled on existing Oak-org usage; promotion (timing) gated on a named data-scientist or product-owner question + warehouse-adapter-shipped (owner-confirmed hard blocker) + identity-policy ruling |
