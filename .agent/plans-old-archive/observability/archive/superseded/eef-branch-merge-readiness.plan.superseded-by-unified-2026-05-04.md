...
| 3 | Local dev server boot probe with `SENTRY_MODE=sentry` | "listening" log line; `<scratch>/dev-boot.log` |
| 4 | MCP tools exercise via protocol (`tools/list` + 3× `tools/call`) | Tool count, names, response shapes; `<scratch>/mcp-tool-exercise.log` |
...
| `security-reviewer` | Auth + trust-boundary | Step 4's auth path: which mode is exercised — DANGEROUSLY_DISABLE_AUTH or real OAuth? What's the exercise's coverage of the auth trust boundary? Is any sensitive material captured in `<scratch>/mcp-tool-exercise.log`? |
