...
- **Next safe step:** the next session opens a fresh orchestrator claim and continues Pass-1 per [`08`](../../../reports/plan-estate-survey-2026-06-21/08-next-session-execution-plan.md) §3 — the **10 remaining collections / ~58 plans** (start `sector-engagement` 12), ~one ~35-plan sub-batch per owner-reset window; then the **70-AEE idea-granular back-fill** (before Pass-3) → Pass-2 (cross-cutting + Saffron Spec-1 effectiveness widening) → Pass-3 (synthesis + completeness-critic, loop-until-dry) → the dated outputs + the independent no-loss audit. `coverage-ledger.md` is the living state. **Owner resets the budget window; owner controls push** (branch 112 ahead, unpushed).

## Where We Are (2026-06-21, Anvil lifts Solder — survey advanced: architecture-and-infrastructure 36/36; sole-successor deep handoff + linear plan written; standing down)

Picked up survey orchestration from **Aardvark turns Whisper** (live two-moments handoff, owner-directed; Aardvark moved to the monitor/tooling fix, then retired). Surveyed **`architecture-and-infrastructure` 36/36** this window in 3 conservable ~12-plan increments (`0b59c7934` / `71d3fbfe0` / `8dd00afa1`; 0 unreadable, 502 ideas inventoried, 14 locked-contradictions, all 36 substance-good / content-strong) → **Pass-1 now 106/286** (AEE 70 + arch 36). The deep handoff + self-contained step-by-step **linear plan for the SOLE successor** is [`07-sole-successor-survey-plan.md`](../../../reports/plan-estate-survey-2026-06-21/07-sole-successor-survey-plan.md) (committed `cf5319371`; supersedes 06/05): remaining **14 collections / ~180 plans** + the 70-AEE idea-granular back-fill before Pass-3 → Pass-2/3 → dated outputs + the independent no-loss audit, with the exact fire→conserve→commit mechanics, pacing, disciplines, and routing. Orchestrator claim `3185b1ff` closed. **Joint closeout with Nova wakes Genesis** (AX lane, `0444c4966`) — both standing down; n=2 owner-visible → sole. Owner controls push.

## Where We Are (2026-06-21, V1-fold lane: Ganymede → Saffron, design delivered, Saffron RETIRED → lane UNHELD; survey = Anvil lifts Solder, GO-gated; n=2 owner-visible, no Director)
...
**Session outcome (Saffron holds Sepal, closing 2026-06-21):** delivered the V1-fold lane's non-gated design work and an owner-directed cleanup sweep. Landed (committed, NOT pushed):

- **The 3 Pass-2 substance specs authored + hardened** — [`restructure-substance-specs.md`](../../../plans/product-development-governance/restructure-substance-specs.md) (`2a4df5423`), then the effectiveness arm hardened to a falsifiable capability-coverage rubric + the effectiveness-reviewer resolved (`e0456a56c`). assumptions-expert reviewed twice; all findings folded.
- **Owner-directed cleanup sweep:** 5 orphan-commits (`db9ed330d` Ganymede's distilled lesson, `2a4df5423` spec, `e7995e7d8` continuity, `73ed3e480` watcher-fix, the ARC channels); the watcher reference-shape doctrine bug fixed (`use-monitor-for-event-driven-wake.md`); stale collaboration-state **processed (substance verified-conserved) and archive-moved**, not deleted, to the new untracked `.agent/state/collaboration/archive/` (`c51c8182b`); the process-not-delete lesson captured (`7ea2674ce`).

**V1-fold lane — NEXT-SESSION PICKUP STATE (lane goes UNHELD on Saffron's retirement; survey-gated):**

- **(1) Pass-2 substance specs — DONE** (above). The three specs are input-to-verify for the orchestrator (Anvil) to fold into the Pass-2 workflow when it fires.
- **(2) V1-fold — OWNER-GATED + survey-gated:** when the survey's *synthesized* outputs land (taxonomy-grounding → V1; conformance + idea-granular substance inventory → restructure work-list, AFTER Passes 1–3), assess input-to-verify, ratify V1. `locked_contradiction` flags from the survey are owner re-ratification candidates and V1-fold signal — Anvil routes them.
- **(3) Stage-3 two-pass restructure — OWNER-GATED + survey-gated:** execute the idea-level decompose → re-compose to the committed Body-3 design + the substance gate (effectiveness arm = the dedicated-session-proposes/owner-ratifies shape now in the spec; no-loss arm = the dedicated parallel prover). Per Body-3 this "becomes its own executable plan when it begins" — author that plan from the survey work-list at pickup.
- **Pickup surfaces:** this section; the committed `restructure-substance-specs.md` + controlling-plan Body-3 + V0 §9; `.agent/reports/plan-estate-survey-2026-06-21/` (06 handoff + coverage-ledger as the survey advances).

**Effectiveness-arm reviewer — RESOLVED (owner, 2026-06-21):** a **dedicated independent session proposes** the per-choice effectiveness verdicts (with capability-map evidence), **the owner ratifies** — mirroring the no-loss arm, possibly the same session. The Pass-2 effectiveness spec now carries a falsifiable capability-coverage rubric (decompose each choice against a fixed dimension checklist → coverage × soundness → `effective | partial | ineffective`), assumptions-expert-reviewed twice. Home: [`restructure-substance-specs.md`](../../../plans/product-development-governance/restructure-substance-specs.md).

**Standing (at Saffron's retirement, 2026-06-21):** survey orchestration is held by **Anvil lifts Solder** (3c3b32→34f6b3; cast rotated Pinnace → Aardvark turns Whisper → Anvil, all clean no-gap handoffs), **GO-gated** by the owner (one ~35-plan sub-batch per owner-reset window; fresh session ≠ fresh window — account-level compute). **V1-fold is now UNHELD** — Saffron retired; a future session picks it up from the state above. **Aardvark retired** after landing the monitor-fix plan (`coordination-watcher-canonicalisation`, promoted to `current/`, canonical-comms + ARC co-equal; cures F-82/F-81). n=2 owner-visible, no Director — **heartbeat OFF** (PDR-078 §4 consumer-absent; verdict: condition-based not headcount-based — owner-visible with a live owner keeps it off even at n=3; re-arm only if the owner goes async or concurrent execution needs peer retirement-detection). Owner controls push (branch ~90 ahead, none pushed). The durable V1-fold ↔ survey-orchestration seam is the rapid-comms channel originally opened Saffron ↔ Aardvark, now carrying Saffron ↔ Anvil (roster-accreted).
...
- **Pinnace hunts Marsh** (868a9b) — survey orchestrator; running the multi-window Pass-1 (sub-batch 1b next; ~one ~35-plan sub-batch per fresh owner-reset window). Claim `f0055992`.
- **Ganymede herds Penumbra** (74cb92) — the V1-fold / alignment-checker; claim `37d251fa`. **Currently blocked-on-owner: has surfaced intent-alignment forks awaiting owner adjudication** (owner-bound by classification — in n=2 owner-visible mode these go to the owner directly, not through a Director). Otherwise survey-gated until the synthesized outputs land.

Birch tracks Arbor took the Director seat from Vesuvius (PDR-064 Moment 2), coordinated the window through two clean role rotations — **Drake → Ganymede** (implementer) and **Hobby → Pinnace** (orchestrator) — folded both into this record, set + confirmed the orchestrator pickup gate, and corrected the survey-output routing (synthesized-only → Ganymede). On owner direction, the seat is now dissolved and Birch has retired; coordination pressure cleared at n=2. The metacognition lesson (a coordinator seat held past its pressure is artefact-gravity on the role itself) is captured in the napkin + per-user memory. **Routing that was the Director's now lives with the two agents + owner directly:** Pinnace routes synthesized survey outputs to Ganymede for the V1 fold; Ganymede's intent-alignment forks go to the owner.

Standalone `pnpm check` deferred during the live multi-window survey (`no-unbounded-host-load` + `check-singleton`; the survey owns the host — Vesuvius's precedent). The closeout commit's pre-commit gate is green.

## Where We Are (2026-06-21, Drake hunts Beeswax stood down → Ganymede herds Penumbra picked up the implementer lane; Birch tracks Arbor directing; survey running)

The implementer lane rotated **Drake hunts Beeswax → Ganymede herds Penumbra** (74cb92, owner-named) at a clean, fully-committed boundary — Drake relinquished all claims, queue empty, nothing mid-flight, no handoff record needed. Ganymede grounded first-hand (Drake's closeout and this record), verified the registry uncontended, and **opened implementer claim `37d251fa`** on the planning-system design lane (`plan-node-schema.v0.md`, `future/repo-intent-graph.plan.md`, the Body-3 spec in the controlling plan), **continuing** Drake's V0 → survey → V1 fold and Stage-3 / Body-3 restructure lane. Ganymede is survey-gated: no source edit until Hobby's survey lands and V1 is ratified first-hand. The Director seat is **Birch tracks Arbor** (rotated from Vesuvius calls Quench mid-session — see the Vesuvius section below). This section folded by Birch as closeout-owner, per Drake's request (Ganymede does not write `.agent/memory/operational/**`).

**Drake's session outcome (the work Ganymede inherits — 4 commits, each full-gate-green, NOT pushed):**

- **V0 owner-gated settlement** (`disposition` and `gate.awaiting` enums, the current+active folder collapse, the 30-day gate-expiry default) — `13d01d55b`.
- **Orphan-commit sweep** — `d594b63d3` (markdown-links validator and wiring and remediation plan) and `4984dd5da` (napkin and Cutter's reflection). Commit B, the DORA / intent-graph docs, was Vesuvius's `62ec1f9c9`.
- **Restructure-design spec correction** — `4bf5d49fd`. A post-survey-steps audit found the consolidated-corpus output under-specified threads and under-weighted effectiveness, and that Body-3 Resolved-#1 had drifted to metadata-only. The owner reaffirmed the design and corrected two findings; Drake encoded them: **dual structure** (a stream → thread → plan directory for humans and frontmatter typed edges for agents, two co-equal audiences, always-both); **threads** as a co-equal intermediate layer (the `thread` node-type pulled forward from Stage 5+ to **Stage 3**); an **effectiveness / completeness** proof criterion (every strategic choice has adequate serving plans, with gaps closed by authored new plans). Intent-graph Stage-3 / 5+ updated to match. **The restructure itself stays survey-gated — this was spec-only.**

**Ganymede's open lane (survey-gated):** when Hobby's survey (multi-window, in flight) emits the **taxonomy-grounding** (→ V1 additive refinements) and the **conformance-and-traceability inventory** (→ the Stage-3 restructure work-list), assess them first-hand as input-to-verify, ratify **V1**, then execute the restructure to the encoded design above. Pickup surfaces: this section, the controlling plan Body-3 (`4bf5d49fd`), and `.agent/reports/plan-estate-survey-2026-06-21/`. The Director routes survey outputs from Hobby to Ganymede.

**Lesson graduated (Drake and Vesuvius, paired):** the inherited-`OWNER-RESERVED`-tag artefact-gravity miss — the firing gate is the *act of posing a question*; run the decision lenses before it reaches the owner. In pending-graduations (DUE) and the napkin.

**Survey progress + orchestrator rotation (later in the same window).** The survey orchestrator seat rotated **Hobby wakes Halo → Pinnace hunts Marsh** (868a9b, owner-pre-positioned), Director-coordinated. Hobby stood down at a clean boundary on the PDR-063 context-budget trigger after completing **Pass-1 sub-batch 1a**: 35 agentic-engineering-enhancements plans, full pipeline, **conserved** to `.agent/reports/plan-estate-survey-2026-06-21/pass1-agentic-engineering-batch1a.json` (21 keep / 11 archive-complete / 1 rewrite / 1 rehome / 1 extract-then-archive; 34 major-drift, 1 no-frontmatter; 45 high-stakes verdicts: 41 survive / 3 refuted / 1 uncertain; 5 locked-contradictions; emergent V1-input keys: `isProject` ×10, `shape`/`program`/`parent`/`owner_scope`/`follows`/per-todo `depends_on`/`Domain`). **Pacing finding:** one ~35-plan atomic sub-batch ≈ one session window (~6.5M tokens / ~150 agents) — the survey is a **multi-window rotating-cast** effort, one sub-batch per fresh owner-reset window. Hobby's claim `1d1acdd2` closed; self-contained handoff at `04-orchestration-state-and-successor-handoff.md` + the durable `survey-pass1.workflow.js` (args-arrives-as-string bug fixed). Pinnace opened orchestrator claim `f0055992` on the report dir; pickup gate (read 04 + script first-hand, arm paired monitors, broadcast) Director-confirmed before sub-batch 1b. **The synthesized taxonomy-grounding + conformance / traceability inventory are produced AFTER Passes 1-3** — only then do they route to Ganymede for the V1 fold; raw per-plan sub-batch findings are not V1 input.

**Team now:** Birch tracks Arbor (Director), Pinnace hunts Marsh (survey orchestrator, successor to Hobby), Ganymede herds Penumbra (implementer; V1-fold and restructure, survey-gated). Hobby wakes Halo, Drake hunts Beeswax, Vesuvius calls Quench, and Cutter / Ferret / Volcano retired cleanly this session.

**Next safe step:** Pinnace resumes Pass-1 from "Pending" in 04 — sub-batch 1b, then the remaining 15 collections in ~35-plan atomic sub-batches (one per fresh window), conserving each; then Pass 2 (cross-cutting) and Pass 3 (synthesis + completeness-critic loop-until-dry); then the 4 dated reports. The synthesized outputs → Ganymede ratifies V1 first-hand → executes the restructure to the encoded design. V1 ratification and Stage-1 build remain OWNER-GATED; the owner controls push (branch ahead 68 of upstream, none pushed).
