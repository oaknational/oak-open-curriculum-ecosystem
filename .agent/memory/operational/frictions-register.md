...
### F-167 — A subagent inherits the Director's PDR-027 identity, so concurrent implementers are indistinguishable in every record
...
### F-168 — The destructive-git hook policy matches on prose, blocking a command that contains no destructive operation
