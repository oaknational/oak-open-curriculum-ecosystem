...
| [`pending-graduations/*.md`](pending-graduations/README.md) | Active queue shards split out of `pending-graduations.md` when a coherent unprocessed cluster is too large for the main register | Until every entry has a durable home, unresolved route, or explicit withdrawal | `consolidate-docs` / curator passes | Live pending-graduations queue substance; not archive material |
