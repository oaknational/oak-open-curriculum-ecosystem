...
- **Disambiguate an overloaded term's distinct concepts before canonicalising or sweeping**
  ...
  MCP-app target-platforms canonicalisation | target: patterns/ (ecosystem-grounded instance)
...
- **Glanceable surfaces: show a token only when it diverges from its working-side counterpart**
  ...
  dedup | target: patterns/ (communication-design / DX) | trigger: a second instance of
  suppressing a redundant glanceable token for human-glance clarity | size: S | status: pending]`
