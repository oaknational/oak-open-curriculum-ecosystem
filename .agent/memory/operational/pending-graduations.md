...
### strict-types-without-loss is a system-wide architectural requirement (ADR)

- **captured-date**: 2026-06-07
- **source-surface**: owner stated emphatically across the `get-eef-evidence` session (Pelagic
  Charting Rudder); full analysis + type-flow diagram in
  `.agent/reports/eef-get-eef-evidence-tool-and-strict-type-flow.report.md`.
- **graduation-target**: an ADR (amend/extend ADR-038 Compilation-Time Revolution, or a new ADR)
  stating the absolute requirement: from external input to external output the type is known at
  every point WITHOUT loss; the ONLY narrowing is validation at the external input boundary
  (`unknown`→exact, schema-anchored), the ONLY erasure is serialisation at the external output
  boundary (e.g. JSON at the MCP wire). Between them: no `as`, no widening to `string`, no
  re-narrowing. A loose carrier (`Record<string,unknown>`, an erased `z.ZodRawShape` annotation) is
  correct ONLY for genuinely-unknown data. Corollary: where validated-input + `as const` data make
  the output fully known, that output must keep its precise type to the wire.
- **trigger-condition**: owner present at a consolidation (the principle is owner-stated and stable;
  a single strong instance + owner direction suffices — do not wait for a second instance), OR the
  next ADR-038 amendment pass.
- **status**: due

### aggregated-handler carrier erases `structuredContent` before the wire — the family's "persistent error" (ADR + execution)

- **captured-date**: 2026-06-07
- **source-surface**: the `get-eef-evidence` type-flow analysis (Pelagic Charting Rudder); report
  §4a. `oak-curriculum-sdk/src/mcp/universal-tools/executor.ts:164` `AGGREGATED_HANDLERS` returns
  `CallToolResult`, whose `structuredContent` is `Record<string,unknown>` — so every aggregated
  tool's structured output is type-erased inside our own code, before the MCP wire.
- **graduation-target**: (1) an ADR clause (under the strict-types ADR above) that aggregated/MCP
  handler result carriers preserve the precise per-tool result type to the wire, erasing only at
  `registerTool`; (2) execution work to make `AggregatedHandler`/executor carry per-tool types
  (owned by the EEF execution plan as the first consumer + a follow-on for the family); (3) an
  OWNER/config decision on scoping `@typescript-eslint/consistent-type-definitions` so a `type`
  alias is permitted where JSON index-signature assignability requires it (it currently rejects the
  required `EefEvidenceEnvelope` type alias). Do NOT disable the check; do NOT revert to interface.
- **trigger-condition**: the EEF carrier-fix execution (next EEF session) — the lint decision blocks
  the commit, so it is owner-due now.
- **status**: due
