...
| 2026-04-25 | Fresh Prince napkin + fitness validator interaction (WS1) | Tooling: *prose-line-width validator should treat `[label]: url` reference-link declarations as non-prose*. `validate-practice-fitness.mjs` classifies them as `prose` and trips the 100-char hard limit when URL alone exceeds budget. Three times bit during WS1 (workaround: shortest alias keys). Either rename ADRs to shorter slugs or extend classifier to recognise `^\[[\w-]+\]:` as `link-reference` kind. Config/tooling — not ADR/PDR-shaped. | Second instance, or explicit owner direction. | pending |
