...
- Cost-of-collaboration P0 is still open. Direct evidence:
  [`.husky/pre-commit`](../../../.husky/pre-commit) still runs whole-tree
  `format-check:root`, `knip`, `depcruise`, and Turbo
  `type-check lint test`; only Markdown has a staged route through
  `agent-tools:repo-check markdownlint-staged`.
