...
| ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `main-critical-sonar-remediation`                 | Product/tooling — rebuild current-main Sonar Quality Gate remediation on `fix/sonar-fixes-20260506`                                                                     | `[threads/main-critical-sonar-remediation.next-session.md](threads/main-critical-sonar-remediation.next-session.md)`                                 | Silvered Masking Owl / `codex` / `GPT-5` / `019dfd` / executor / 2026-05-06; Moonless Vanishing Lantern / `codex` / `GPT-5` / `019dfd` / evaluator / 2026-05-06.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
main-critical-sonar-remediation thread, Sonar rebuild implementation
handoff)**:
Implemented part of the current-main Sonar rebuild on
`fix/sonar-fixes-20260506`: Phase 0 evidence and plan staged; targeted
SonarJS detectors enabled; comparator/void/generated-schema/refactor
slices partially applied; specialist reviews had already found and
cleared several earlier gaps. Owner stopped the earlier attempt when
the generated MCP runtime executor refactor began chasing TypeScript
errors inside generated/core tool typing, then explicitly rejected any
route that excluded generated files from Sonar or lint. The recovery
rolled back the generic invoker-map direction, preserved the generated
flat MCP argument boundary, left `ToolDescriptor`,
`ToolClientForName`, `ToolArgsForName`, and adjacent core aliases
untouched, and fixed the generated executor with generator-owned
concrete per-tool helpers plus literal switch delegation. Full root
`pnpm check` passed after source and continuity-surface updates;
`pnpm markdownlint:root` and `pnpm format:root` passed after the final
handoff/comms edits. **Next safe step**: in a fresh session, inspect
the current remote Sonar state and choose the next highest-impact
remaining blocker from the refreshed remediation plan, keeping
generated files fully gated.
...
**Status (2026-05-06 Silvered Masking Owl, codex, GPT-5, `019dfd`,
owner-directed `/jc-session-handoff` during main-critical Sonar
remediation): `not due — session-scoped handoff, no plan/milestone
closed`. Entry-point sweep clean (`CLAUDE.md`, `AGENTS.md`, `GEMINI.md`
match contract); tracks contain only README + `.gitkeep`; practice-core
incoming contains only `.gitkeep`; no relevant open decision-thread or
escalation handoff was needed. Napkin now captures the generated-executor
type-system overreach, the generated-files-stay-gated owner correction, and
the full-gate recovery lesson. The inherited hard-fitness surfaces remain a
