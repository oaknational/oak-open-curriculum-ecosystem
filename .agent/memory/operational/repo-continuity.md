...
explicitly skipped. Outcome: 1 spine BLOCKER + 2 slice-plan BLOCKERs
    remediated same-session; 4 remaining Phase 4 FINDINGS + 2 topology BLOCKERs
...
on the connecting-oak-resources thread — (1) absorb 2 topology
    BLOCKERs into `graph-stack.plan.md` + ADR-173; (2) absorb 4 remaining
    Phase 4 FINDINGS into the three slice plans + slice 1; (3) owner-resolve the
...
+ ADR-173). NO slice execution; NO graph-stack ACTIVE promotion; NO
...
`.agent/research/graph-library.research.md` (renamed 2026-05-07 from `graph-iibrary.md` and reframed as a research direction). Foundation increment ingests the
...
| `eef`                                             | Sector-engagement subthread — EEF Teaching and Learning Toolkit as evidence corpus on graph foundation                                                                  | `[threads/eef.next-session.md](threads/eef.next-session.md)`                                                                                         | Most-recent: Windward Darting Horizon / `cursor` / `claude-opus-4.7` / `dd084d` / eef-tool-rename-eef-prefix-per-adr-157-and-mvp-arc-cross-ref (renamed 5 unprefixed tool/prompt names → `eef-*` across 19 occurrences in `eef-evidence-corpus.plan.md` via 5 replace-alls; updated t18 ADR-123 entry; added MVP-arc cross-reference + 2026-05-07 amendment note to plan body; coordinated as part of the cross-collection MVP-arc spine work on `connecting-oak-resources` thread) / 2026-05-07. Prior: Gnarled Fruiting Root / `claude-code` / `claude-opus-4-7-1m` / `e18e2c` / cross-ref-path-updates-from-thread-restructure (no substantive EEF work) / 2026-05-01. Prior: Vining Whispering Root / `claude-code` / `claude-opus-4-7-1m` / tracer-matrix-and-promotion-packet + holistic-alignment-audit + safety-cures-after-destructive-incident / 2026-05-01; Fragrant Sheltering Petal / `claude-code` / `claude-opus-4-7-1m` / type-reviewer-round / 2026-04-30; Iridescent Soaring Planet / `claude-code` / `claude-opus-4-7-1m` / architecture-restructure-and-handoff / 2026-04-30.                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
artefacts; `architecture-reviewer-betty` over ADR-173 + `graph-stack.plan.md`
...
ADR-173 (so both reach decision-complete; CURRENT → ACTIVE transition
    + ADR-173 ratification are NOT in scope this session); (2) absorb 4
    remaining Phase 4 FINDINGS into the three slice plans + slice 1
    (adapter Inc.2/Inc.3 timing; slice 3a topic ambiguity; slice 3a
    budget concretisation; slice 3b implementation-audit test shape);
    (3) owner-resolve the EEF plan
...
plan — spine + 3 slice plans + slice 1 + substrate + ADR-173. NO
slice execution; NO graph-stack ACTIVE promotion; NO ADR-173
ratification.

**2026-05-07 (Windward Darting Horizon, cursor, claude-opus-4.7,
`dd084d`, connecting-oak-resources thread + eef subthread, MVP-arc
spine plan author + cross-collection coordination amendments)**:
Authored top-level
[`graph-mvp-arc.plan.md`](../../plans/graph-mvp-arc.plan.md) as a
cross-collection coordination spine sequencing three vertical slices:
(1) EEF evidence corpus MCP surface; (2) Oak ontology Threads MCP
surface; (3a) misconception sub-graph queries + (3b) EEF×misconceptions
cross-corpus composition. Coordination amendments landed: ADR-157
namespace table extended with `oak-misconceptions-*` row + compound
prefix row + explicit-source-attribution discipline addendum;
[`eef-evidence-corpus.plan.md`](../../plans/sector-engagement/eef/current/eef-evidence-corpus.plan.md)
tool/prompt names re-prefixed `eef-*` (5 names, 19 occurrences via
replace-alls) + t18 ADR-123 entry updated + MVP-arc cross-reference;
[`graph-portfolio-index.md`](../../plans/graph-portfolio-index.md)
gained `## Vertical-slice arc` section pointing to the spine;
[`high-level-plan.md`](../../plans/high-level-plan.md) cross-links the
spine from Cross-cutting Threads. **Two course-corrections same
session**: (a) added unsequenced `mvp_arc_status: deferred` to NC SKOS
taxonomy plan — reverted by owner direction *"sequence properly or
admit not-doing"*; (b) re-introduced under `mvp_arc_sequencing`
framing + `## Out-of-MVP-Arc Items` spine section + per-slice
out-of-arc todos — reverted by owner direction *"the NC work is
explicitly NOT part of the MVP"*. Final state: NC plan owns its own
`promotion_trigger` (demand-tripwire on SKOS-specific consumer
demand) in its own frontmatter; spine plan tracks ONLY what's IN the
MVP. Two graduation candidates captured to pending-graduations
register: sequence-or-admit-not-doing doctrine; spine-drift-via-
comprehensive-cataloguing anti-pattern. **Next safe step**: author
the slice-2 executable plan (`oak-kg-threads-surface.plan.md`) per
spine's `author-slice-2-plan` todo — parallel-safe with
`author-slice-3a-plan`; both should land in next session(s) on the
connecting-oak-resources thread.
...
1. **Absorb the two topology BLOCKERs into `graph-stack.plan.md` +
   ADR-173** so both reach decision-complete (CURRENT → ACTIVE
   transition + ADR-173 ratification are NOT in scope this session;
   ...
   - **`practice-graph` workspace tier fix** — relocate from
     ...
     workspace-tier semantics; ADR-173 topology entry updates to
     match.
2. **Absorb the four remaining Phase 4 FINDINGS into the three slice plans +
   ...
   (a) slice 2 adapter timing inconsistency vs `graph-stack.plan.md`
   Inc.3; (b) slice 3a topic-context acceptance contradicts non-goal —
   tighten; (c) slice 3a budget concretisation (numeric token target +
   fixture selection rule); (d) slice 3b cycles 2 + Design Principle 2 contain
...
4. **Verify decision-complete state across the full MVP plan** —
   ...
   `graph-stack.plan.md`, ADR-173. Any artefact NOT decision-complete
...
CURRENT → ACTIVE transition; ADR-173 ratification; any production
code changes.
...
collection coordination amendments, capture-edge `/jc-session-handoff`
only): `not due — capture-edge work; pending-graduations drain ran
2026-05-07 earlier today (Pelagic Rolling Harbour); two new candidates
captured to register from this session as standard pending entries`**.
This session was substance authoring (spine plan) + coordination
amendments + two course-corrections; convergent doctrine emerged but
graduation paths are already named in the captured pending-graduations
entries. No fitness pressure observed on touched surfaces. Next
consolidation pass is the next thread-scoped invocation, not this
handoff.
