...
- **Docs consolidation repair PARTIAL (2026-05-31, Open Lofting Cliff / `019e7d`)**:
  the failed archive-only consolidation passes were repaired to live-buffer state
  by restoring active `napkin.md` and `distilled.md`, removing failed 2026-05-31
  archive sidecars, and correcting curator-ledger language so archive relocation
  is not completion evidence. A small real `distilled.md` batch was dispositioned
  through PDR-089, pending-graduations owner gates, or duplicate durable homes.
  The original consolidation remains pending: last repair-state fitness truth had
  `napkin.md` critical, `distilled.md` hard, and `repo-continuity.md` hard. Next
  safe step is a single-agent `/goal` session using
  `codex-docs-consolidation.brief.md`; do not run multiple repair agents and do
  not archive/split/shard/rename/pointer-replace live buffer content to improve
  fitness.
- **EEF plan repair + reviewer brief COMPLETE (2026-05-31, Tidal Charting Hull /
  `019e7d`)**: the live EEF plan and README were repaired after D0 closeout.
  Current truth: raw `EEF_TOOLKIT_DATA as const` is the source snapshot and type
  source, not automatically the graph contract; D5 owns an explicit pure
  graph-native construction/adaptation boundary; and MCP input/output schemas
  derive via one Zod call from the graph-native EEF view subset, with owner
  conversation if impossible. If no distinct graph-native shape adds value, the
  identical raw/graph-native shape requires owner ratification before
  implementation proceeds. Next safe step for EEF is the Codex reviewer session
  brief at `eef/current/eef-plan-reviewers.codex-brief.md`: run
  `docs-adr-expert`, a general `type-expert`, and a separate type-preservation
  `type-expert` pass tracing `EEF_TOOLKIT_DATA as const` through the full value
  chain. Final handoff did not run `pnpm check` by explicit owner direction;
  targeted markdownlint/Prettier and root Prettier were green.
- **EEF D0 COMPLETE + intent-vs-letter audited (2026-05-31, Opalescent Transiting
  ...
  D0's full DoD acceptance is satisfied. **Committed at `ce9745c7`** (2026-05-31,
  owner-directed single commit including the non-EEF `discovery/` plan collection
  and draft PDR-089; pre-commit hook green; 7 ahead of origin, not pushed). Full
  record: the `eef` thread record EXECUTION UPDATE banner and the decontamination
  ledger. This continuity SHA-stamp is the only delta now uncommitted on top.
...
| --- | --- | --- | --- |
| `eef` | EEF graph-tooling rebuild | [record][eef] | codex / GPT-5 / Tidal Charting Hull / `eef-plan-reviewer-brief` / **plan repair + reviewer brief COMPLETE (2026-05-31): raw `EEF_TOOLKIT_DATA as const` is source snapshot/type source, not automatically graph contract; D5 graph-native construction/adaptation boundary explicit; MCP schemas derive via one Zod call from graph-native EEF view subset; identical raw/graph-native shape requires owner ratification. Next: run `eef-plan-reviewers.codex-brief.md` (`docs-adr-expert`, general `type-expert`, special type-preservation `type-expert`), then D1 ∥ D2. `pnpm check` skipped by explicit owner direction; targeted markdownlint/Prettier + root Prettier green.** / 2026-05-31 |
| `agentic-engineering-enhancements` | Practice continuity (tactical lane; not the active delivery focus) | [record][agentic] | codex / GPT-5 / Open Lofting Cliff / consolidation-repair-handoff / partial repair: archive-only consolidation repaired to live-buffer state; single-agent `/goal` brief at `codex-docs-consolidation.brief.md`; original consolidation pending / 2026-05-31 |
...
> **Committed at `ce9745c7`** (2026-05-31, one owner-directed commit; not pushed).
> **Next safe steps: (1) a Codex EEF plan-consistency / residue sweep — brief at
> `sector-engagement/eef/current/eef-plan-consistency-sweep.codex-brief.md`; then
> (2) D1 (value, owner-ratified) ∥ D2 (typed foundation), then D3→D7.** A separate
> Codex dedicated docs-consolidation session is also queued — brief at
> `.agent/memory/operational/codex-docs-consolidation.brief.md` (note: comms-log
> rotation is PAUSED — see Repo-Wide Invariants). Authority = the `eef` thread
> record EXECUTION UPDATE banner + the plan's Remediation Context + Ratified
> Decisions sections + the decontamination ledger. Do not resume the S1–S5
> framing below.
...
- **comms-log rotation is PAUSED** until a dedicated comms research plan exists;
  the `consolidate-docs` calendar-age comms-event rotation threshold is
  **superseded** —
  do not rotate comms-events or act on the old calendar rule (owner direction
  2026-05-31; formal rule/ADR home still belongs with the comms research plan);
...
**handoff gate (2026-05-31, Open Lofting Cliff - consolidation repair and brief)**:
`due - explicit owner-requested follow-on consolidation repair.` The prior
archive-only consolidation was repaired to live-buffer state and the next
single-agent `/goal` brief is now
`codex-docs-consolidation.brief.md`. This handoff does not claim completion:
remaining live truth is critical/hard buffer pressure on active curation
surfaces, and the next session must continue item-by-item with valid
`graduated`, `duplicate`, `owner-gated`, or `stale-withdrawn` dispositions.
...
curation scale, not session-completion): napkin rotation (1005 lines) and the full
7c/7e register audits — falsifiable: those remain until a dedicated pass runs.
**Comms-event rotation is NOT deferred work — it is PAUSED** until a dedicated
comms research plan; the consolidate-docs calendar-age comms threshold is
superseded
(owner 2026-05-31), so the ~2787 comms-events are not to be rotated. Q-001 already
...
`2e8d5455`; D0's full DoD acceptance is satisfied. The D0 bundle was subsequently
committed at `ce9745c7` (one owner-directed commit; not pushed). No claim (solo,
empty registry).

**repair note (2026-05-31, Open Lofting Cliff)**: a previous pass moved the
2026-05-28 to 2026-05-30 handoff-gate prose into a fresh archive to improve the
fitness report. That was an invalid archive-only cure under
`consolidate-until-done`; the content is restored below until each item receives
real item-level disposition.
...
-> several escalating critique passes -> **rewrote the plan S1-S5**. No code
(owner held implementation). Three deepened directives baked in (corpus `as const`
is its own authority - delete the Zod/freshness ingest; NO stubs; extend the
generic `NodeFilter` to nested paths); thread record banner + this file refreshed;
the superseded plan-mode scratch file removed. No claim opened (nothing to close).
A strong behaviour-changing lesson captured (the verify-reflex regenerated in six
...
**handoff gate (2026-05-29, Shaded Prowling Threshold; updated at closeout as events landed)**:
`due - not this closeout`. Lane A landed owner-approved (uncommitted); **Lane E
now OWNER-APPROVED** (Furnace Melting Bellows; 3 files incl. scope-adjacent
PDR-029). The bounded deep-consolidation work is Group A **WS-Z convergence**
(index rows + the 6 register removals + single-gatekeeper commit), parked for the
**next session** and now **fully unblocked** (Lane E sign-off cleared). **Napkin is
NOT being rotated** - owner pulled fitness from Highland's scope to a separate
fitness session, so `napkin.md` stays **CRITICAL** (508 lines, hard 300 / critical
450 - past critical, a loop-failure signal per ADR-144); WS-Z records
`strict-hard` RED on it as **remediation-deferred-to-the-fitness-session**, an
owner-approved deferral, not a WS-Z defect. `pnpm check` deferred to WS-Z's single
gatekeeper commit per `check-singleton-per-window`. Shaded's + Furnace's claims
closed; the Group A + Lane E files are unclaimed for the next session. Separately
inbound: ~14 net-new graduation candidates from Highland for next-session
pending-graduations triage.
...
observable-guard added to consolidate-docs Section 4 after adversarial verification,
plus ADR-150 mirror + session-handoff Section 6c); Lane C (PDR-087
TDD-as-design, governance); Lane D (PDR-088 reviewers-carry-doctrine, governance).
PDR numbers 087/088 deconflicted from the role-emission PDR-086 reservation. ALSO
landed: ADR-168 Section 5 - graduates the `scripts/`-is-no-checks-zone doctrine
that Wooded captured in the napkin AND I misread (two-instance owner-directed
cure: doctrine in ADR-168, discoverable pointer in `vitest.config.base.ts`,
optional mechanical check noted; docs-adr-expert reviewed -> sound-with-suggestions,
suggestions folded); plus `.remember/` added to the vocab-validator exclusion
(mirrors `.agent/experience/`; vocab gate GREEN). Deep consolidation NOT
separately due - the Group A WS-Z convergence (next session) IS the bounded
convergence + commit + napkin rotation (napkin HARD 320/300). Owner-directed
session-end before lanes A/E/WS-Z. No retained claim (registry empty).
Minimal-ceremony session per owner direction (no comms broadcasts/monitors/
heartbeat cron).
...
Continuation: [`eef` thread record][eef] Section 2026-05-29 entry.
...
were removed and no rules/PDRs authored. Deep consolidation this handoff:
owner-directed `consolidate-docs` run (graduating this session's process insights).

**routing**: detailed consolidation, handoff, and session-close history now lives
in thread records, curator reports, completed plans, and prior continuity archives.
This file should stay an operational index and should not regain per-session
closeout prose.
...
`routing-legacy-fallback-sunset` Section Open-problems. A 62-item deferral/non-goal
audit of the collaboration-substrate cluster found one genuine orphan (substrate
...
check-singleton - Sunlit closing concurrently). napkin is co-mingled with Sunlit's
concurrent entries; left unstaged for the gatekeeper.
...
owner-ratified - plan SPECIFIED, foundation Section 10 resolved, new
...
(defer/menu/list-op/suppress-the-signal) was owner-caught 4 times -> generative
