...
**Last refreshed**: 2026-04-27 (Composer / cursor — session handoff after
MCP Apps widget allowlist and metadata alignment: `WIDGET_TOOL_NAMES` remains
`get-curriculum-model` + `user-search`; agent `search` and app-helper
`user-search-query` no longer advertise `_meta.ui.resourceUri`;
`user-search-query` keeps `_meta.ui.visibility: ['app']` only;
`ToolMeta.ui.resourceUri` is optional in `mcp-protocol-types`. Universal-tools
tests assert host-visible behaviour (`isAppToolEntry`, role-named tools) per
[testing-strategy](../../directives/testing-strategy.md), not the allowlist
as configuration. **TypeScript follow-ups remain; owner asked not to fix
types in this pass** — next session: `pnpm type-check` on affected
packages and resolve errors. No commit this handoff.)

**Prior refresh**: 2026-04-26 (Codex / codex / GPT-5 — session handoff
after owner-directed WS3 split. Multi-agent collaboration WS3 now has two
current plan files: WS3A decision-thread / claim-history / observability
is queued; WS3B sidebar / escalation is paused behind WS3A evidence and
owner direction. Branch-primary remains observability; Sharded
Stroustrup's separate active claim was observed and not touched.)

**Prior refresh**: 2026-04-26 (Cursor / GPT-5.5 / Codex — session handoff
...
- Branch: `feat/otel_sentry_enhancements` (in sync with origin at
  `63d48f4`; current working tree may include uncommitted SDK **MCP Apps
  metadata** and **universal-tools test** edits; **TypeScript errors may
  remain** until the next type-fix pass. Prior uncommitted WS3 split /
  handoff docs may still be present; the active-claims registry may contain
  Sharded Stroustrup's separate observability claim.)
...
| `agentic-engineering-enhancements` (**WS3A decision-thread next; WS3B evidence-gated**) | Practice — documentation roles, continuity surfaces, and fitness-pressure remediation. Multi-agent collaboration protocol WS0+WS1+WS2 landed; WS3 split into WS3A decision-thread / claim-history / observability and WS3B sidebar / escalation. | [`threads/agentic-engineering-enhancements.next-session.md`](threads/agentic-engineering-enhancements.next-session.md) | `Composer` / `cursor` / `Composer` / mcp-apps-widget-metadata; testing-strategy-tests; session-handoff / 2026-04-27; `codex` / `GPT-5` / Codex / practice-docs-consolidation; markdown-code-block-rule; collab-terminology-handoff; WS5-evidence-harvest-review; WS3-plan-split; session-handoff; WS3A-RED-fixtures; WS3A-claim-history-GREEN; WS3A-handoff / 2026-04-24→2026-04-26; `cursor` / `GPT-5.5` / Codex / grouped-commit-closeout; openapi-pipeline-api-boundary; ooc-issues-1; bug-report-stale; session-handoff / 2026-04-24→2026-04-26; `claude-code` / `claude-sonnet-4-6` / Jazzy / multi-agent-collaboration-protocol-plan-author / 2026-04-25; `claude-code` / `claude-opus-4-7-1m` / Jiggly Pebble / multi-agent-collaboration-protocol-WS0-landed-as-63c66c88 / 2026-04-25; `claude-code` / `claude-opus-4-7-1m` / Fresh Prince / multi-agent-collaboration-protocol-WS1-landed-as-a5d33519 / 2026-04-25 |
...
The 2026-04-26 (Codex / codex / GPT-5) WS3A session landed uncommitted
documentation/state edits: it split multi-agent collaboration WS3 into
[`multi-agent-collaboration-decision-thread-and-claim-history.plan.md`](../../plans/agentic-engineering-enhancements/current/multi-agent-collaboration-decision-thread-and-claim-history.plan.md)
(WS3A: evidence provision, protocol observability, durable
claim-closure history, lightweight decision threads) and
...
(WS3B: sidebar / timeout / owner escalation, evidence-gated).
It then completed WS3A RED and WS3A GREEN claim-history. Claim-history
now has `closed-claims.schema.json`, `closed-claims.archive.json` v1.1.0
closure metadata, updated claim-close guidance, and schema-valid
claim-history fixtures. WS3A remaining work is decision-thread GREEN,
protocol-observability refactor, and final validation/handoff. WS3B still
must not auto-resume.

### Prior — 2026-04-26 (Sharded Stroustrup)
...
- **MCP / SDK (uncommitted work)** — Run `pnpm type-check` in
  `@oaknational/oak-curriculum-sdk` and the repo root; **resolve
  TypeScript errors** introduced or exposed by the `ToolMeta` and
  `listUniversalTools` test work. Falsifiability: clean type-check with no
  new `// @ts-expect-error` on product paths unless paired with a tracked
  follow-up.
- If continuing the `agentic-engineering-enhancements` thread, start with
  [`multi-agent-collaboration-decision-thread-and-claim-history.plan.md`](../../plans/agentic-engineering-enhancements/current/multi-agent-collaboration-decision-thread-and-claim-history.plan.md).
  Continue at `ws3a-green-decision-thread`; after that, complete
  `ws3a-refactor-observability` and `ws3a-validation-and-handoff`.
  Do **not** promote
  [`multi-agent-collaboration-sidebar-and-escalation.plan.md`](../../plans/agentic-engineering-enhancements/current/multi-agent-collaboration-sidebar-and-escalation.plan.md)
  unless the owner explicitly directs it or WS3A decision-thread evidence
  shows async coordination is insufficient.

Expected next session (observability branch-primary), per owner direction:
...
**Status (2026-04-26 Codex WS3A handoff)**: **due, not run** —
`repo-continuity.md` is hard in practice fitness (lines and characters).
Not run in this handoff because the owner directed the next session to
finish WS3A's remaining executable slices. Falsifiability: run
`pnpm practice:fitness:informational`; if `repo-continuity.md` remains
hard and the owner chooses consolidation before WS3A, run
`jc-consolidate-docs`.

**Status (2026-04-26 Codex WS3 split handoff)**: **due, not run** —
`repo-continuity.md` itself is hard in `pnpm practice:fitness:informational`
(628 lines, 45,463 chars before this handoff). The session's requested
scope was lightweight handoff plus next-session opener, and the working
tree also contains a separate active observability claim. Falsifiability:
rerun `pnpm practice:fitness:informational`; if `repo-continuity.md`
still reports hard, the next consolidation should compact/archive this
file's historical closeout material before adding more repo-level prose.
No `jc-consolidate-docs` run.

**Prior status (2026-04-26 Cursor — second handoff, documentation-only)**: **not
...
| 2026-04-26 | `openapi-pipeline.md` + ooc-issues + `oak-openapi` read-only code review (Cursor) | PDR/ADR: elevate **API-only consumer data boundary** to a rule with CI/lint teeth if the docs-only preface proves insufficient. | Second near-violation (agent or code path attempting direct Hasura/MV), or owner direction. | pending |
