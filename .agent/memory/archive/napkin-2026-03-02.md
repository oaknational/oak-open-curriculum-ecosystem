...
## Session 2026-03-02f — Codegen plan restructure and simplification

### What was done
1. Created `.agent/plans/architecture-and-infrastructure/codegen/` collection
2. Moved `sdk-codegen-architecture-analysis.md` and `sdk-codegen-reviewer-findings.md`
   into the collection (updated internal cross-references)
3. Created `future/sdk-codegen-workspace-decomposition.md` — strategic plan for
   splitting `oak-sdk-codegen` into `oak-sdk-api-codegen` and `oak-sdk-bulkdata-codegen`
4. Mined release plan and session continuation for information to include
5. Reduced release plan §Curriculum Graphs Architecture Redesign (8 phases, ~200 lines)
   to §ESLint OOM Fix (2 tasks, ~50 lines)
6. Reduced session-continuation prompt to match simplified scope
7. Updated architecture-and-infrastructure README and distilled.md

### Key decisions
- The elaborate 8-phase architecture redesign is deferred to post-merge
- The M1 merge blocker is resolved with 2 pragmatic tasks:
  (a) remove NODE_OPTIONS from lint scripts
  (b) add ESLint ignores for the 10 large generated data files
- The architectural debt (generator duplication, naming, workspace decomposition)
  is captured in a strategic plan, not lost
- The ESLint ignores apply ONLY to the large serialised data files (5K-122K lines),
  not to all generated code — this is the narrow exception the owner granted

### Plan structure
- `future/` strategic plan is deliberately concise — it captures the direction
  (two workspaces, separation by data lineage) and the research (two companion
  reports) without trying to be an executable plan yet
- Promotion trigger is clear: after M1 merge, when CI is green on main
- The consumer subpath usage map in the plan is valuable — it shows exactly
  which exports go to which workspace after the split

### Learned
- When a plan is blocking a merge, simplify ruthlessly: do the minimum that
  unblocks CI, capture the rest as future work. The user made this call
  explicitly ("we will address the architectural debt after the merge")
- Plan documents should have a clear lifecycle: the 8-phase plan was never
  executed, so converting it to a strategic brief is cleaner than leaving it
  in the release plan as dead weight

---

## Session 2026-03-02e — Practice README/index restructure

### What was done
1. Restructured practice-core README.md and index.md to clarify their audiences:
   - **README.md** is now a human-friendly explainer: what the Practice is, point
     agents at index.md, simple 3-step hydration how-to for humans ("create dir,
     drop files, ask your agent"), attribution.
   - **index.md** is now the self-contained agent-operational document: day-to-day
     work, practice-core files table, practice box, cold-start hydration with
     "understand the repo first" emphasis, misplaced-files guidance.
2. Updated travelling unit from "plasmid trinity" (3 files) to "practice-core
   package" (5 files: trinity + README + index). Updated references across:
   practice.md, practice-lineage.md, practice-bootstrap.md, both start-right
   prompts, and the reference-docs agentic-engineering-practice.md.

### Key design decisions
- The old README had agent-oriented 4-step hydration instructions that belonged in
  the index. The old index's "Cold Start" section pointed back to the README for
  orientation — backwards. Both now serve their primary audience directly.
- The human how-to is deliberately casual ("drop these files into it, ask your
  agent...") to make it approachable for non-technical users.
- The index's cold-start section emphasises understanding the repo's intent, norms,
  and standards BEFORE adapting the Practice. This is the key first step for repos
  that do not yet have a Practice.
- Added misplaced-files guidance to the index: if a human drops files in the repo
  root or random directory, the agent knows to move them to `.agent/practice-core/`.
- The "plasmid trinity" name is preserved for the three core blueprint files that
  carry provenance frontmatter and evolve between repos. The README and index are
  "entry points" that travel with the trinity but don't need provenance tracking.
- References to "plasmid trinity files" in the consolidation command step 8 (fitness
  ceilings) are correct and unchanged — only the three trinity files carry ceilings.

### Self-containment rule and practice-index.md
- Recorded a new Learned Principle: practice-core files must be self-contained.
  No navigable markdown links outside practice-core, except to `../practice-index.md`.
- Created `.agent/practice-index.md` — the bridge file between the portable core
  and the local repo. It carries navigable links to directives, ADRs, commands,
  skills, and artefact directories. It is NOT part of the travelling package.
- practice.md had 18 external links; all converted to code-formatted paths. The
  one permitted link is to `../practice-index.md` in the "See also" line.
- index.md had 2 external links (AGENT.md, principles.md); converted.
- Added the practice-index format specification and template to practice-bootstrap.md.
- Added step 8 ("Create practice-index.md") to the Growing a Practice instructions
  in practice-lineage.md. Updated Bootstrap Checklist and cohesion audit steps.

### Hydration flow verification
Traced full end-to-end: human drops 5 files into new repo → README → index.md
Cold Start → practice-lineage.md Growing a Practice steps 1-10. Key gaps found
and fixed:
- Step 7 was "write from scratch" only — now conditional for received vs scratch
- Validation section was missing practice-index.md check — added as item 2
- Step 8 now explicitly says "populating every section with the local repo's
  actual directives, ADRs, commands, skills, and directories"

### Barney review findings addressed
- Low: added self-containment check script to practice-lineage.md that strips
  code-fenced blocks before grepping (avoids false positives from templates)
- Suggestion: added Boundary Contract section to index.md — portable vs local
  at a glance, with a table showing files/links/creation for each side

### ADR impact assessment
- Only ADR-119 references practice-core directly
- ADR-032, ADR-008 have no practice-core references
- ADR-119 needs additive updates: five-file package, self-containment principle,
  practice-index bridge. Core decision (name, boundary, three layers) unchanged.

### ADR-124 created
- Created ADR-124: Practice Propagation Model — a new ADR covering the five-file
  package, self-containment rule, and practice-index bridge. ADR-119 is about
  naming and framing; propagation is a distinct architectural decision about HOW
  the Practice travels.
- ADR-119 updated with cross-reference to ADR-124 and added ADR-124 as a
  canonical update surface in both the Documentation Propagation Contract and
  practice.md's documentation propagation step.
- Practice-index.md updated to include ADR-124 in its ADR table.
- ADR index README.md updated: flat list + agentic engineering practice section.

### The pattern
The practice-index.md is an adapter: it adapts the portable, self-contained core
to a specific repo's file structure. The core specifies the format, the hydration
process creates the instance, and each repo's instance carries its own links.
This is the same principle as dependency injection — the core depends on an
interface (the format), not on specific implementations (the repo's files).

---

## Session 2026-03-02d — Plan integration and handoff

### What was done
Integrated curriculum graphs architecture plan from `.cursor/plans/` into the
release plan (`release-plan-m1.plan.md`). Updated session-continuation prompt
as standalone next-session entry point. Deleted the cursor plan.

### Key decisions
- The curriculum graphs redesign is classified P1 (blocks CI, blocks merge)
  in the release plan
- The session-continuation prompt is concise — it names the 8 phases with
  one-line summaries and points to the release plan for detail
- The "Top Priorities" section was rewritten: graphs redesign is now #1,
  year normalisation and fetch bug moved to "recently completed"
...
6. Added `fitness_ceiling` and `split_strategy` frontmatter to 9 key documents:
   AGENT.md (200), principles.md (200), testing-strategy.md (400),
...
2. Encoded refactoring lint awareness for future agents:
   - Updated `principles.md` Refactoring section with explicit ESLint
...
- The previous session's agent didn't run lint after substantive
  ...
  is more effective than a directive buried in a long principles.md because
