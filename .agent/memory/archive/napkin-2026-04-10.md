...
### Session 2026-04-09i — Vercel plan promotion + handoff

#### Surprises

**S25: Runtime log showed missing file at single `dist/`, not `dist/dist/`.**
Expected: only the historical doubled-`dist` path bug. Actual: Vercel logs
also report `Widget HTML not found` at
`.../oak-curriculum-mcp-streamable-http/dist/oak-banner.html` — file absent from
the deployed artefact (bootstrap fails before request handling). Remediation
split into active plan `vercel-mcp-build-warnings-and-bootstrap.plan.md`
(moved from `current/` to `active/`, Phase 0 in progress).

#### Actions

- Promoted the Vercel build-warning + bootstrap plan to `active/`; linked from
  Phase 6 Required Inputs and `active/README.md`; synced
  `session-continuation.prompt.md` and `prompts/README.md`.

### Session 2026-04-09j — Vercel widget bootstrap + warning cleanup

#### Surprises

**S26: the clean fix was not `public/` or `includeFiles`, but anchoring the
widget path to the function working directory.** Expected: the Vercel Express
static-assets doc might force the widget into `public/**`. Actual: the widget
HTML is not served via `express.static()`; it is loaded by the MCP server and
returned as a resource payload. The real bug was that
`register-widget-resource.ts` anchored the file path to `import.meta.dirname`,
which breaks when the compiled module is relocated. Switching the runtime
contract to `process.cwd()/dist/oak-banner.html` made the relocated built
artefact proof pass without Vercel-specific overrides.

**S27: `vite-plugin-singlefile` deprecation came only from its
recommended-config helper.** The plugin itself still works; the warning
disappears if we keep the plugin for inlining but own the explicit Vite build
settings ourselves (`base`, `assetsInlineLimit`, `cssCodeSplit`, `assetsDir`,
chunk warning limit).

**S28: the `canonical-url-map.json` warning was a policy mismatch, not a build
failure.** ADR-132 and the reference README already define the sitemap scan as
soft/operator-driven. Missing local reference data should log as informational
skip guidance; malformed reference data should remain warning-level.

#### Resolutions

- Added a relocation-style built-artifact E2E proof for
  `dist/application.js`; it now passes after the `process.cwd()` path fix.
- Reworked `widget/vite.config.ts` to use `viteSingleFile` without its
  deprecated recommended config, preserving single-file output with no
  Rollup deprecation.
- Added `globalPassThroughEnv` entries in `turbo.json` for the exact Vercel
  project env keys seen in the build logs, keeping them available to strict
  Turbo tasks without putting their values into cache hashes.
- Reclassified missing sitemap reference data from WARN to INFO and updated
  `@oaknational/sdk-codegen` docs to match the explicit soft-gate policy.

### Session 2026-04-09k — HTTP dev widget orchestration in workspace operations

#### Surprises

**S29: the right "dev from source, prod from build" model is orchestration, not
runtime branching.** The HTTP MCP server must still read a final HTML widget
payload, so the clean seam is to keep `dist/oak-banner.html` as the single
runtime contract and make `pnpm dev*` own building and watching that artefact
from source inside the app workspace.

**S30: wrapping child commands in `pnpm exec` was enough to pass tests but not
enough to guarantee clean shutdown.** Sending SIGINT to the orchestration
process could leave the underlying `tsx src/index.ts` server alive with PPID 1.
Switching the execution plan to direct workspace binaries
(`node_modules/.bin/vite`, `node_modules/.bin/tsx`) made child ownership and
shutdown deterministic.

#### Resolutions

- Added `apps/oak-curriculum-mcp-streamable-http/operations/` with a documented
  development orchestration surface instead of adding more logic to `scripts/`.
- Repointed `dev`, `dev:observe`, and `dev:observe:noauth` to a thin operations
  CLI that performs initial widget build, starts the widget watcher, then boots
  the source HTTP server.
- Added pure contract tests plus orchestration shutdown tests so the mode/env
  plan and cleanup guarantees are covered without spawning real child processes.
- Verified the behavioural contract end-to-end: deleting `dist/` no longer
  blocks HTTP dev startup, `/healthz` comes up after auto-build, the watcher is
  terminated on shutdown, the relocated built-artifact E2E still passes, and
