...
## Session 2026-04-06 — SDK adoption Phase 5 + type-correctness audit

### Phase 5 E2E pipeline test

- Wrote `mcp-app-pipeline.e2e.test.ts` with 5 assertions proving the
  full tool registration → protocol response → widget resource chain.
- 4 specialist reviewers (code, test, mcp, type) — all findings addressed.
- `pnpm check` green. Committed as `27d66464`.

### Surprises

- **[S1] Type-reviewer recommended `z.unknown()` as a fix** — this is type
  destruction, not type safety. The reviewer applied the "no
  `Record<string, unknown>`" rule mechanically by replacing it with
  something worse. Zod-level type destruction was not in the reviewer's
  vocabulary.
  - **Impact**: Multiple iterations of shadow schemas and type widening
    before human review caught the root issue.
  - **Resolution**: Type-correctness audit — new rule, upgraded
    type-reviewer (12 Commandments), DRY consolidation.

- **[S2] Hand-crafted `JsonSchemaPropertySchema` was entropy** — re-invented
  a shape that `toolInputJsonSchema` already provided with literal types.
  The correct approach: import from `@oaknational/sdk-codegen/mcp-tools`
  and compare against the generated source of truth.

- **[S3] `.merge()` NOT deprecated in Zod v4** — `typescript-practice.md`
  claimed otherwise. The Zod 4.3.6 source has no `@deprecated` annotation
  on the `merge` method. Fixed.

- **[S4] `core.$ZodIssue` import path fragile** — not part of Zod's public
  API. Codebase already used `ZodError['issues'][number]` as the reliable
  derivation. Fixed in docs.

### Type-correctness audit

- Surveyed 19 type-defining sources across the practice.
- Created `.agent/rules/unknown-is-type-destruction.md` (new canonical rule).
- Created Claude + Cursor platform adapters.
- Upgraded type-reviewer to 12 Commandments (added z.unknown() + shadow
  schemas), new Zod Schema Integrity checklist, anti-pattern §5.
- DRY consolidation: `principles.md` references rules instead of restating
  them. `typescript-practice.md` restructured (Zod integrity section moved,
  `.merge()` claim removed, `core.$ZodIssue` fixed, Result<T, E> added,
  `as const satisfies` expanded).
