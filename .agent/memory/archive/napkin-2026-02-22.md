...
## Session: 2026-02-22 (c) — Widget Phase 5 Resilience Hardening

### What Was Done

- Executed Phase 5 of widget stabilisation: fixed 3 critical and 5
  important findings from architecture reviews
- C2/I4: Replaced single-quoted string interpolation with
  `JSON.stringify` in CTA config and tool renderer map generation
- I1: Fixed `scopeObj` to return null for unknown scopes instead of
  silently defaulting to 'sequence'
- I3: Removed heuristic scope inference from search renderer. Added
  fail-fast validation for required fields (`scope`, `results`, `total`)
- C1: Added try/catch around renderer calls in widget-script.ts with
  Playwright E2E test for error containment
- C3: Added four-way sync enforcement tests for the renderer chain
- I2: Added browse/explore fixture builders with SDK types and Zod
  schema validation
- I5: Documented intentional sequences omission in explore renderer
- Security fix: Replaced inline `onclick` handlers with `data-oak-url`
  attributes + delegated click handler to prevent JS injection via
  HTML-entity-decoded quotes in URLs

### Gotchas

- User directive "ZERO heuristics, only deterministic code" directly
  contradicted my initial I3 fix which inferred scope from data shape.
  Had to remove the heuristic and replace with fail-fast validation.
  Lesson: don't add "smart" fallbacks — if data is missing, say so
- `fixture-builder.ts` exceeded 250-line ESLint limit after prettier
  expanded compact type definitions. Had to split into two files
  (`fixture-builder.ts` for search, `fixture-builder-browse-explore.ts`
  for browse/explore). Prettier always wins on formatting.
- `expect.any(String)` returns `any` which triggers
  `@typescript-eslint/no-unsafe-assignment`. Use `toHaveProperty` for
  structural checks on `unknown` values from `new Function` evaluation
- Security reviewer confirmed `onclick="openOnOakWebsite(event, \\'' +
  esc(url) + '\\')"` is exploitable: `esc()` is HTML escaping, browser
  HTML-decodes the attribute value before evaluating as JS, so `&#39;`
  becomes `'` which breaks out of the JS string. Fix: `data-oak-url`
  attribute + delegated listener

### Reviewer Findings Addressed

- Code reviewer: simplified `generateToolRendererMapJs` to use
  `JSON.stringify(TOOL_RENDERER_MAP)` directly (eliminated
  `Record<string, string>` violation)
- Test reviewer: added `scopeObj` to `hasRequiredKeys` type guard in
  `helpers.unit.test.ts`
- Security reviewer: replaced all `onclick` inline handlers with
  `data-oak-url` + delegated click handler across search, browse, and
  explore renderers

## Session: 2026-02-22 (b) — Widget Phase 1-4 Implementation

### What Was Done

- Executed Phases 1-4 of widget stabilisation via genuine three-level
  TDD (RED/GREEN/REFACTOR at unit, integration, and E2E levels)
- Phase 1: Multi-scope search rendering — extracted field helpers
  (`extractTitle`, `extractSubject`, `extractKeyStage`, `extractUrl`,
  `scopeObj`) in `helpers.ts`, updated search renderer for 4 scopes +
  suggest, created `renderer-contracts.integration.test.ts` with Zod
  schema validation, created `helpers.unit.test.ts` with 27 tests
- Phase 2: Created `browse-renderer.ts` for `browse-curriculum` tool
  with 8 integration tests
- Phase 3: Created `explore-renderer.ts` for `explore-topic` tool
  with 9 integration tests, reusing Phase 1 helpers
- Phase 4: Full quality gates + specialist reviews (code-reviewer,
  test-reviewer, type-reviewer, architecture-reviewer-barney,
  architecture-reviewer-betty, architecture-reviewer-wilma)
- Reviewer fixes: XSS single-quote escaping in `esc()`, dead code
  removal (`RENDERER_FUNCTION_NAMES`), duplicated ternary extracted
  to `scopeObj()`, `rel="noopener noreferrer"` on all links, `Pick`
  type improvements in fixture-builder

### Gotchas

- Zod schema validation caught that `subject_parent` for units and
  `subject_slug` for sequences must use parent subject values (e.g.
  `'science'` not `'biology'`) matching the `SUBJECTS` enum
- Generic `buildScopeFixture<T extends FixtureBase>` in fixture-builder
  caused TS7053 weak type detection issues with `SuggestFixtureData`.
  Fix: revert to per-scope builder functions calling `buildWrapper()`
- Dynamic indexing `renderers[id]` in registry unit test caused TS7053
  when `renderers` was `unknown`. Fix: remove the type assertion
  check, rely on `id in renderers` presence check instead
- `isHelperFunctions` type guard exceeded ESLint complexity (12, max 8).
  Fix: two-phase narrowing (`hasRequiredKeys` then `allAreFunctions`)

### Architecture Review Findings (Track 2)

- **Wilma critical**: No `try/catch` around `renderer(fullData)` in
  `widget-script.ts` — any renderer error crashes all tools
- **Wilma critical**: CTA config string injection — `id`, `buttonText`
  etc. not escaped in generated JS (potential widget-breaking issue)
- **All three**: Triple/quadruple sync for adding renderers should
  become a single manifest in Track 2
- **Betty**: Browse uses camelCase (SequenceFacet), search/explore use
  snake_case (ES index docs) — normalise at MCP boundary in Track 2
...
- Closeout stubs were initially adopted but later removed — they are
  compatibility layers (principles.md forbids these). Clean break: fix
...
- Updated `docs/governance/ai-agent-guide.md`:
...
- `docs/governance/ai-agent-guide.md` — memory section, experience section
...
## 2026-02-22 Phase 0 Widget Stabilisation (Track 1a)

### What happened
- Executed Phase 0: deleted 18 non-search renderer files, reduced widget to
  search-only, removed 4-step fallback heuristic chain, added neutral shell
  (Oak logo + heading, hidden footer) for non-search tools.
- Three-level TDD: RED (6 unit + 3 Playwright tests failing), GREEN (all
  passing after deletion), REFACTOR (cleaned dead code, built infrastructure).
- Created renderer test harness (`new Function()` sandbox) and fixture builder.
- All quality gates pass (type-gen, build, type-check, lint:fix, format,
  markdownlint, test, test:ui, test:e2e, smoke:dev:stub).

### Gotchas
- `tsconfig.json` didn't include `tests/**/*.ts` — only `**/*.test.ts` and
  `**/*.spec.ts`. Test UTILITY files (harness, fixture builder) aren't matched
  by those patterns. Fix: add `tests/**/*.ts` to include array.
- ESLint `projectService: true` uses the nearest `tsconfig.json`, not
  `tsconfig.lint.json`. Files must be in BOTH tsconfigs.
- The `object` type (lowercase) is forbidden by `@typescript-eslint/no-restricted-types`.
  The `{}` empty object is forbidden by `@typescript-eslint/no-empty-object-type`.
  `Record<string, unknown>` forbidden. Use specific interface types instead.
  For generic functions that need to spread objects, use per-type overloads or
  specific builder functions rather than generic `T extends object`.
- `as AddressInfo` type assertion in Playwright test server setup: replace
  with `if (!address || typeof address === 'string') throw` pattern.
- TSDoc `>` character must be escaped: use "maps to" instead of `->`.
- `WIDGET_HELPERS` had 7 functions, search renderer only uses `esc()`.
  The other 6 were dead code after renderer deletion.
- `formatDate()` in WIDGET_STATE_JS was dead code — only used by deleted renderers.
- Duplicate type guards (`isToolWithRenderer` and `isRendererMapKey`) doing
  identical narrowing. Export one, import everywhere. DRY.
- Static `widget-preview.html` (1469 lines) was completely stale. The preview
  server generates dynamically from production code. Deleted the static file.

### Decisions
- `TOOL_RENDERER_MAP` uses `as const satisfies Readonly<Record<string, RendererId>>`.
  The `satisfies` validates without widening — narrow literal types preserved.
  The `Record<string, RendererId>` is in satisfies position only, not assignment.
- `getRendererIdForTool(toolName: string)` keeps broad `string` parameter
  because callers pass dynamic strings from widget JS. The type guard
  `isToolWithRenderer` narrows internally.
- Fixture builder uses specific per-tool builder functions
  (`buildSearchFixture`) instead of generic `buildWidgetFixture<T>` to avoid
  needing broad generic constraints (`object`, `Record`, `{}`). Phase 2-3
  will add `buildBrowseFixture` and `buildExploreFixture`.
- `status` field conditionally included in fixture builder (matching
  production `formatToolResponse` which only includes status when defined).

### ESLint type restrictions expanded
- Added `{}`, `Object`, `Function` to `@typescript-eslint/no-restricted-types`
  in `packages/core/oak-eslint/src/configs/strict.ts`.
- Also carried forward `Record<string, undefined>`,
  `Readonly<Record<string, undefined>>`, and `Record<PropertyKey, undefined>`
  from the recommended config — these were silently dropped because ESLint
  flat config uses last-writer-wins for rule declarations.
- Key insight from user: `{}` as a generic constraint (`T extends {}`) is
  just another escape hatch. MCP responses have full Zod validation, so the
  types ARE known. Don't reach for broad constraints — use specific types.
- `Object` (capital O) is semantically identical to `{}` — blocks the
  obvious workaround path.
- `Function` accepts any callable with zero parameter/return type safety —
  use specific function signatures instead.
