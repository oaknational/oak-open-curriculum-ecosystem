...
## 2026-06-25 — A test-only override on a load-bearing gate IS gate-weakening (Seal hunts Offing, 8210d6)

- **Surprise / mistake.** Implementing the F-95 `claims open` watcher-presence backstop, I added a `--comms-seen-dir` option so the integration test could point the gate at a temp heartbeat. A bot re-review (cursor, HIGH) flagged it: that override let a *planted* heartbeat at any path satisfy the load-bearing backstop — i.e. the test-convenience flag was itself a way to bypass the gate. Cure: removed the option entirely; the gate uses the canonical dir only; the live→present path is unit-covered via injected IO, and the integration test exercises the canonical default dir (solo opens; populated→refuse).
- **Lesson:** when a gate is load-bearing (security / coordination-safety), a knob added "just for the test" is a production surface that weakens the guarantee. Reach for injected IO / a fake adapter at the unit boundary, or exercise the canonical path, before adding a runtime override. Reinforces [[never-disable-checks]] and no-warning-toleration — "don't weaken a gate to solve a testing problem" extends to *additive* knobs, not just disabling existing ones.

## 2026-06-25 — I asserted branch ahead/behind from memory and had it inverted (Seal hunts Offing, 8210d6)

- **Mistake (6e.1 worked instance).** I read `git rev-list --left-right --count origin/main...HEAD` once, mis-parsed the left/right columns, and repeated "coordination is 19 commits behind main" across the session. At handoff the 6e.1 first-hand re-check (`--count origin/main..HEAD` / `HEAD..origin/main`) showed the truth was inverted: **22 ahead, 5 behind**. The load-bearing handoff fact was wrong because it was carried from memory, not re-grounded.
- **Lesson:** ahead/behind is a load-bearing handoff claim — verify it with the unambiguous two-dot `git rev-list --count A..B` form at write-time, never the easily-misread `--left-right ...` symmetric form from memory. Exactly the failure 6e.1 exists to catch; the discipline earned its keep this session.
- **Minor process notes:** (a) `git rebase` is permission-gated for the agent in this harness — a successor wanting to rebase a branch must run it themselves (`! git rebase ...`). (b) Each push to a PR re-triggers bot reviewers (cursor/copilot), which surface fresh findings per push; batching all fixes into one push converges the re-review loop instead of chasing it round by round.

## 2026-06-25 — Expensive-command discipline: capture once to a log, never tail-then-rerun (Seal hunts Offing, 8210d6)

- **Mistake (owner-flagged, cost real money).** I ran the full `pnpm check` (a multi-minute, whole-monorepo gate) piped straight through `tail -40` — which discards everything but the last 40 lines, so I could not see the overall verdict/exit code. Then I started to **re-run the entire expensive gate** just to recover the exit code. Two compounding failures: a capture format I knew would prevent analysis, then paying the full cost twice.
- **Standard (always-useful approach for any expensive/long command — full `pnpm check`, full builds, full test suites):**
  1. Run it **once**, redirecting the **complete** output to a log file and capturing the exit code in the same invocation: `cmd > <scratchpad>/check.log 2>&1; echo "EXIT:$?"`.
  2. Then analyse the log with `grep`/`Read` (e.g. `grep -nE 'error|✖|FAIL' check.log`, and read the tail for the summary). The full output is preserved, so no second run is ever needed.
...
## 2026-06-25 — no-machine-local-paths enforcement: dogfooding + the regex-too-narrow miss (Junk tracks Moorings, 3b1f1c)

- **The write-hook works — it blocked my OWN edits, repeatedly.** Once the `machine-local-path` `preToolUseContent` block landed, every attempt to write an example path (the validator's own docstring, the policy description, even a per-user memory note) was denied at write-time. Lesson: a content-pattern hook that polices a pattern must EXCLUDE the surfaces that legitimately *name* the pattern — the rule file, the policy file itself, and the validator's own source/test. Excluding by the validator's filename substring is precise (other test files stay policed — that was the whole PR-#203 gap).
- **A pattern is only as good as its forms — the regex was too narrow.** My first validator caught the user-home slash form but MISSED the flattened Claude/Cursor project-id form (`-Users-<user>-code-...`), which is exactly how the owner's username PII had spread across ~14 archived/report/analysis files. Owner-flagged. Lesson: when enforcing a PII/portability invariant, enumerate ALL the forms the leaked value actually takes (slash path, flattened id, lsof dump, test fixture), not just the canonical one.
- **PII's sneakiest carrier is a test fixture.** I authored the anti-PII validator's test using the owner's real username as a positive-control fixture — PII inside the file meant to prevent it. Tests prove behaviour with FAKE values + DI; a real username/path is leaked config. See per-user memory `feedback_no_pii_in_tests_delete_on_sight`; delete PII on sight, never narrate it as a finding (owner correction).
- **Critically assess subagents, including their recommendations.** The security-expert verified the S4036 finding + fail-loud fix correctly, but over-scoped the recommendation (an `OAK_TRUSTED_GIT` override; miscounted "third consumer"). Took the verified parts, declined the speculative override. (`feedback_validate_specialist_findings_before_acting`.)
