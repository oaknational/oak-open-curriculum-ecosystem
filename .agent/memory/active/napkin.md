...
session plan file at `~/.claude/plans/jc-plan-jc-metacognition-pr-87-phase-generic-clover.md`,
this napkin entry, and a session handoff to follow.
...
- Not yet a graduated pattern; recorded here as an observation. If a
  ...
  data point and a pattern entry under
  `.agent/memory/active/patterns/` becomes warranted in-session, not at
  some future consolidation pass.
