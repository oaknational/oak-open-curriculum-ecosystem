...
- **A Classification Instrument Is a Symptom of the System It Grades** -- Use this when: A proposal or plan executes an instrument's findings row by row (a census's dispositions, an audit's items, a matrix's per-row cures) — ask whether the instrument's NEED is the defect before executing its output. → [classification-instrument-is-a-symptom.md](classification-instrument-is-a-symptom.md)
...
- **An Unversioned Dependency Gets a Boot-time Invariant Check, Not a Runbook Line** -- Use this when: A service depends on state that is not in version control — environment variables, paired credentials, a vendor instance binding, a remote configuration — and a mismatch would fail only on a path the routine probes do not exercise. → [boot-time-invariant-check-for-unversioned-dependencies.md](boot-time-invariant-check-for-unversioned-dependencies.md)
...
- **A Scripted Transform Is All-or-Nothing, With Asserts at the End** -- Use this when: A change must be scripted rather than made with the Edit tool — a multi-site rewrite, a regex sweep, a batch of anchored replacements — and a partial application would leave the tree in a state nobody intended. → [all-or-nothing-scripted-transforms.md](all-or-nothing-scripted-transforms.md)
...
- **De-serialise a Dependent PR by Cherry-picking the Dependency's Exact Commit** -- Use this when: One open PR is paused behind another because it needs a fix the other carries, and the dependency is an ORDER (wait for a merge), not content the paused branch would author differently. → [de-serialise-dependent-prs-by-cherry-picking-the-dependency.md](de-serialise-dependent-prs-by-cherry-picking-the-dependency.md)
...
- **A Baseline Transmits Its Stance, Not Only Its Facts** *(anti-pattern)* -- Use this when: Composing a brief, a judge prompt, or a fleet dispatch that hands the legs a prior document — a matrix, a ratified node, a previous round's result — as their starting point, especially inside a space the owner has reopened. → [baseline-transmits-its-stance.md](baseline-transmits-its-stance.md)
...
- **A Surface That Misinforms Without Failing** *(anti-pattern)* -- Use this when: Authoring or reading any surface that can be empty, stale, or unreachable BY DESIGN — a monitor status, a dashboard column, a doc row, a derived field, a blocked-state record — and the reader's cheapest check is to look at the surface. → [surface-that-misinforms-without-failing.md](surface-that-misinforms-without-failing.md)
...
- **Claim Before Check** *(anti-pattern)* -- Use this when: About to state a status, mechanism, size, population or board fact to a peer, the owner, a PR record or a ticket — 'pushed', 'merged', 'the gate is released', 'the fleet is N seats', 'that plan is an unsanctioned sketch', 'the client should still complete' — and the state that would confirm it has not been read in the last few minutes. → [claim-before-check.md](claim-before-check.md)
