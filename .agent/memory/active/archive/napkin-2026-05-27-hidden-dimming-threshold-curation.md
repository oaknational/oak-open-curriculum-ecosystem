...
- Heartbeat-tagged comms events reject `--body` and `--body-file` now; they
  must be composed from typed state args (`--claim-id`, `--intent-id`,
  `--branch`, `--current-cycle-label`). Behaviour change: when starting the
  liveness loop, use typed heartbeat args immediately instead of copying the old
  narrative heartbeat shape.
- `comms watch --seed-from-now` and `comms inbox` can still flood stdout with
  `[routing-legacy-fallback]` diagnostics while classifying historical legacy
  events. Behaviour change: if the all-channel watcher is noisy, stop it,
  record the gap, and use focused comms reads until a quiet watcher shape is
  available; do not let diagnostic output become the work.
