...
- Parking WIP outside hook-discovered package paths preserved the source/test
  ...
  `/private<scratch>/fiery-ws1.4-parked/`.
...
## 2026-05-13 — Quiet Stalking Mirror / claude-code / opus-4-7-1m / fe8a4f

### Surprise: lockfile in flux without a comms-event declaration

**Expected:** at session open, `pnpm-lock.yaml` would either be clean or
the dirty agent would have a registered claim + a `commit window open`
comms-event so peer agents could sequence.

**Observed:** WS1.5 opened against a tree where Mossy Blossoming Canopy's
prior session had closed claims and committed three P8 commits, but the
working tree carried a 43-44-file owner-initiated pnpm/dependency refresh
slice (root packageManager bump pnpm@10.33.4→@11.1.1, every workspace
package.json, pnpm-workspace.yaml, lockfile, SDK codegen artefacts).
`active-claims.json` and `shared-comms-log.md` carried no declaration of
that slice's owner or intent. The clarification was buried inside
`repo-continuity.md § Next Safe Steps / Agentic` rather than appearing on
the live-coordination surfaces.

**Why it matters:** session-open grounding routinely reads active-claims
and shared-comms first because those are the documented live-coordination
surfaces. An owner-initiated bundle that lives only in `repo-continuity`
is invisible to that surface scan and looks like orphaned WIP from the
collision-risk perspective. WS1.5 was correctly held on lockfile-collision
grounds, but the held could have been avoided if the owner-initiated
dependency refresh had been visible at the coordination layer (a "owner
landing closeout bundle" comms-event would have settled it instantly).

**What I would do differently:** read `repo-continuity.md § Next Safe
Steps` BEFORE concluding that a dirty slice is uncoordinated. The
continuity file's Next Safe Steps notes can carry owner-direction context
that doesn't appear in comms-log or active-claims. Add this to grounding:
"if `active-claims.json` shows no claim on a dirty slice, cross-check
`repo-continuity.md § Next Safe Steps` for owner-direction landing notes
before treating the dirty slice as orphaned."

### Doctrine captured for graduation (candidate)

**Owner-stated 2026-05-13, on WS1.5 design**: *no aliases, no fallbacks,
fail fast and hard with helpful error message, replace old with new.*

Applied immediately to the WS1.5 canon wrapper design: hardcoded RDFC-1.0
+ `rejectURDNA2015: true`, no default-algorithm branch, no
inputFormat-string fallback, no default-hash fallback, errors carry kind +
adapter-specific message naming step + native cause, and the plan text
itself replaces URDNA2015 with RDFC-1.0 rather than retaining a
parenthetical alias.

The four-part shape is more general than any one of the existing rules
(`never-disable-checks`, `local-broken-code-never-leaves`,
`dont-break-build-without-fix-plan`). It articulates the *positive*
discipline behind those negative rules: design at boundaries should NEVER
accept deprecated names, accommodate optional inputs with defaults, swallow
errors silently, or carry forward the old shape parallel to the new one.

**Candidate destination**: a single rule (e.g. `boundary-design-strictness`
or `no-aliases-no-fallbacks-fail-fast-replace-old-with-new`) under
`.agent/rules/`, or amendment to `apply-architectural-principles.md` /
`local-broken-code-never-leaves.md`. **Trigger for graduation**: second
observation across a different boundary (next time owner reiterates or
applies this doctrine to a different surface). Capture-only at session
close per `jc-session-handoff` step 6b.
