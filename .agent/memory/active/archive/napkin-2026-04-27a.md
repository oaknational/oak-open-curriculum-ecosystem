# Napkin Archive — 2026-04-27a

Archived from active napkin during light consolidation pass at the
2026-04-27 18:47Z Opalescent Gliding Prism session-handoff. The
active napkin had reached HARD fitness pressure (386 lines / 18384
chars over the 300/18000 limits); these settled entries were rotated
out to keep the active drift-pattern triple (Vining → Pelagic →
Opalescent) in the active surface.

The lessons captured here have already been graduated to operational
rules and conventions:

- App-server title-mutation rollback informed
  `feedback_platform_independent_naming` (memory) and the Codex
  app-server rollback decision recorded in shared-comms-log at
  2026-04-27.
- Commit-window vs queue ordering graduated to PDR-029 Family A
  Class A.3 (queue-governance pass, Coastal Washing Rudder
  16:46:45Z).
- Quality-gated-scripts-belong-in-workspaces is now operational
  practice; the commit-queue migration to `agent-tools` TypeScript
