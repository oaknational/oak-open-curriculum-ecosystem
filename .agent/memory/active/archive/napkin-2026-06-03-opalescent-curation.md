...
- **Brief 1**: no citation surface at all (md-only export, citations flattened to 48
  ...
  Owner then supplied a verified official-source link list (approximate set, 21 URLs);
  added as a provenance-marked `### Verified official source links` References subsection
  (no inline positions claimed). All 21 probed live: 19 OK, 2 GIAS bot-blocked (expected).
...
## 2026-06-03 — Opalescent curation framing correction

- **Correction**: I described the next move as "make fitness pass", which is
  exactly the forbidden framing in the invoked consolidation skills. The right
  action frame is: conserve and home the knowledge item by item, then use the
  validator only as routing evidence and a rest-state health check.
