...
## Session: 2026-02-24 (e) — Consolidation and Push

### What Was Done

Documentation consolidation and commit/push of Phases 3-5. Quality gates
verified clean before starting. Code reviewer identified 5 documentation
extraction tasks for Phase 6 (synonyms README, subpath table in gen SDK
README, ESLint plugin README update, type-helpers README). These were added
to Phase 6 scope in the canonical plan.

Roadmap updated to reflect Phase 5 completion (Phases 0-5 done, 6-7 remain).
Prompt updated with Phase 6 documentation extraction tasks.

### Bug Found: Missing .js Extension

During push, E2E tests failed: `ERR_MODULE_NOT_FOUND` for
`dist/types/search-response-guards`. Root cause: `types/index.ts` imported
from `'./search-response-guards'` without the `.js` extension. ESM requires
explicit extensions. Fix: add `.js` to both `export` and `export type`
lines (lines 79, 85).

This is a Phase 2 regression — the F10 barrel simplification moved these
exports to `types/index.ts` but the import paths were likely copied from a
pre-ESM context. The unit tests and type-check passed because `tsx`/`tsc`
resolve without extensions, but the built `dist/` requires them.

### Key Learning

**ESM extension discipline applies to ALL relative imports, including
re-exports in barrel files.** `pnpm build` + `pnpm type-check` do NOT catch
missing extensions because tsup bundles (resolving at build time) and tsc
(with `moduleResolution: bundler`) both tolerate extensionless paths. The
error only surfaces when the built `dist/` is consumed as ESM at runtime
(E2E tests via STDIO server). The pre-push hook running E2E was the last
line of defence.
...
2. Reviewed both against directives (principles.md, testing-strategy.md, schema-first, AGENT.md)
...
- **B7**: Created `docs/domain/curriculum-guide.md` — plain-language curriculum structure for non-technical audiences. Cross-referenced from `docs/README.md`, `onboarding.md`, `foundation/VISION.md`.
...
guidance in foundation/quick-start.md, added as-const exception to principles.md.

**A3 (Workflow)**: Created docs/engineering/workflow.md — full dev lifecycle
...
foundation/VISION.md link, strategic/leadership path in onboarding, mission-framing
paragraph, Vision section in docs/README.md.

**A5 (Vision Translation)**: Enhanced foundation/VISION.md capability table with
...
**A8 (Extension Points)**: Created docs/engineering/extending.md — guidance for
...
- When adding `as const` exception to principles.md, also propagate to the
...
- foundation/VISION.md "7 different curriculum structures" was slightly misleading — 7
...
- Added canonical-source note to a reference-docs application note
