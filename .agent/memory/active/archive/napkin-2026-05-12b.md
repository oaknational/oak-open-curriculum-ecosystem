...
## 2026-05-12 — Vining Regrowing Grove / codex / GPT-5 / `019e1c`

### Cost-of-Collaboration P4

- Landed P4 identity disambiguation and active-agent visibility at
  `1bb369a5`. The new `claims active-agents` surface made the active/stale
  distinction visible immediately during closeout: my P4 claims were active,
  the peer graph-lane Lofty claims had become stale, and the same Codex
  session prefix no longer collapsed the two identities.

### Practice/tooling feedback

- **Surface**: rendered shared comms log
- **Signal**: owner-observed friction
- **Observation**: `tail -f .agent/state/collaboration/shared-comms-log.md`
  appeared to stop useful render observation. The likely shell-level cause is
  following an old file handle across renderer rewrites rather than a true file
  lock, but the operator need is clearer than the mechanism: tailing Markdown
  is not a proper collaboration monitor.
- **Behaviour change / candidate follow-up**: captured a pending P8
  collaboration TUI workstream in `cost-of-collaboration.plan.md`: read events
  and claim state directly, show main comms, direct-message threads, unread
  state, and active-agent visibility, and avoid depending on the rendered
  Markdown file as the live source of truth.

## 2026-05-12 — Brazen Stoking Ash / claude / claude-opus-4-7-1m / `913094`

### Comms-tooling friction log (peer pair with Lofty Vaulting Summit on graph-stack Inc.1a)

Tracking snags/frustrations with collaboration-state tooling and protocols
as they arise this session.

- **Discovery cost — directed-message peer identity**: could not send a
  directed `comms direct` without Lofty's `to-platform / to-model /
  to-session-prefix` triple. No canonical "active agents present" registry
  to look up from. I had to broadcast on the shared log first, wait for
  Lofty's ack (`d8c3203d`), then re-target. **Friction**: every fresh
  peer-pair requires one broadcast round-trip before directed comms can
  begin. **Candidate cure direction**: a `comms presence` action (or
  `identity list-active`) that lists agents whose claims or recent comms
  events fall inside a freshness window, so the directed-comms target
  triple is discoverable without a broadcast.

- **No "inbox" for shared-log targeting**: `comms inbox` only surfaces
  directed messages. A shared-log brief titled to a named recipient is
  invisible to inbox polling — recipient must scan the full log. For this
  session I used both surfaces (broadcast then directed) to avoid the gap,
  but that meant duplicating content. **Candidate cure direction**: either
  (a) `comms inbox` includes shared-log entries whose title or body
  references the agent name, or (b) doctrine says directed-only for peer
  targeting and the broadcast is reserved for genuinely shared coordination.

- **Directed brief duplicates plan/thread substrate**: Lofty will read the
  brief, the plan, the thread record, and active claims independently
  anyway. The brief partially duplicates the thread record's "Next Task"
  section. **Open question**: is the directed brief load-bearing for
  "what is this session's scope right now" or is it duplication of
  durable surfaces? If the former, current tooling has no way to anchor
  the brief as a first-class session artefact (it lives as one comms
  event in a directory of many).

- **Heredoc + Bash quoting friction**: posting a long brief through
  `comms send --body "$BODY"` works but apostrophes in the body require
  `'"''"'` escape sequences (e.g. `TripleTerm carries 'value: '''`).
  For multi-paragraph briefs a `--body-file <path>` flag would remove an
  entire class of quoting bugs. **Candidate cure direction**: add
  `--body-file` (and `--subject-file`) to `comms send` / `comms direct` /
  `comms append`.

- **No "where am I in protocol" indicator**: with so many steps in the
  commit-queue protocol (8 steps from the thread record), there is no
  command that prints "current intent, current phase, expected next
  action". Easy to lose place under interruption. **Candidate cure
  direction**: `commit-queue status --intent-id <id>` returning the
  phase plus the next required action with a copy-and-paste command.

- **`comms inbox` fails on missing seen-file** (reported by Lofty, recorded
  as agent-tooling F-21 their side): the `--seen-file` path is required
  even on first run; tool errors instead of treating absence as "no
  messages seen yet". **Candidate cure direction**: treat missing
  seen-file as empty-seen-set; create on first write. (Cross-platform
  observation — codex side hit it explicitly; my claude side worked only
  because I had already created `<scratch>/brazen-seen.json` implicitly.)

- **`comms direct --help` output diverges from required flags**: the help
  text shown by `comms direct --help` lists `--messages-dir / --to-agent-name
  / --to-platform / --to-model / --to-session-prefix / --kind / --subject /
  --body / --platform / --model [--event-id] [--now]` but the actual
  validator also requires `--active <path>` (pointing at active-claims.json)
  — invocation fails with "Error: missing required option --active" only
  when the flag is omitted. Worse: the pnpm wrapper swallows the diagnostic
  in the tail of `pnpm --filter` output; I had to drop down to raw
  `node agent-tools/dist/...` to see it. **Candidate cure direction**:
  (a) include `--active` in `comms direct --help` output; (b) make the
  pnpm-filter wrapper not bury the underlying tool's error message;
  (c) default `--active` to the canonical repo path when omitted.

- **My own brief contained a protocol-incompatible attribution split**: I
  told Lofty "pass me the staged-bundle fingerprint when ready; I run the
  commit-queue protocol end-to-end" — but the 8-step protocol's
  `verify-staged` and `git commit` steps assume single-agent ownership.
  Splitting stage-and-commit across two agents would either force me to
  claim authorship of code I didn't write (the Lush Sprouting Thicket
  scope-breach at 11:54 BST) or fail at `verify-staged`. Lofty caught it
  in their reply. **Lesson**: the commit-queue protocol implies authorship.
  Reviewers do not commit other agents' code; they review. **Cure
  direction (doctrine, not tooling)**: distilled rule candidate —
  "in peer-pair shape, the implementing agent stages and commits their
  own bundle under their own queue intent; the reviewing agent gates
  via review verdict only, not via taking over the commit window."

- **Repo-wide `pnpm knip` blocks one thread's commit because of another
  thread's unused exports**: Lofty's WS1.3 cannot pass `pnpm check`
  because the P4 work owned by Vining (different thread, different workspace, different
  scope) has unused exports that knip catches. This is the staged-only-
  gates P0 manifesting again, in a slightly different shape: even when
  no file overlap exists, the global gate scope creates a serialisation
  point between threads that should be independent. Captured as another
  instance of the existing P0 — not a new finding, but a concrete
  same-day occurrence on a non-markdown gate (knip, not markdownlint).

- **Premature unblock signal — I inferred clearance from a peer's commit
  without verifying the gate**: when Vining landed `1bb369a5`, I sent
  Lofty a GO message assuming the knip blocker was cleared. It wasn't —
  Vining's commit introduced new public API exports (the active-agents
  surface) that are themselves the unused-exports knip is flagging.
  Lofty opened their commit window; I had to send a STOP. **Lesson**:
  before signalling a gate is clear, RUN the gate. `pnpm knip` (or the
  exact gate that was failing) must show green before forwarding GO,
  not "the upstream-author committed; should be fine now." Mechanical
  verification, not inferred. **Cure direction (doctrine)**: distilled
  rule candidate — "the gatekeeper verifies the gate's actual state
  before issuing GO; a peer's commit landing is necessary but never
  sufficient evidence that a downstream gate has cleared."

- **`claims active-agents` is the right tool but I had to discover the
  required `--active <path>` the same way I did for `comms direct`**:
  help-text omission, generic "missing required option" error, pnpm
  wrapper swallows the underlying message. Same friction pattern as
  the earlier `comms direct` entry. Suggests systemic: every
  collaboration-state CLI verb that mutates or reads claim state
  requires `--active <path>` and none of them advertise it in `--help`.
  **Cure direction**: audit all collaboration-state CLI verbs for the
  same flag-omission pattern; either default to canonical path or
  include in help text universally.

- **STOP signal has an unrecoverable race window during the commit
  window**: I sent Lofty STOP (`7ddc9bd5`) after observing knip red,
  but Lofty's `git commit` was already inside the pre-commit hook
  execution. The exec wrapper had closed stdin; the hook completed
  cleanly; WS1.3 landed at `87e21125`. Lofty did not use `--no-verify`.
  Outcome was correct (commit was valid) but only by luck — the gate
  ordering was opaque to the dispatcher. **Cure direction**: there is
  no clean cure for the race itself (the hook is uninterruptible by
  design). The cure is upstream: the gatekeeper should not authorise
  GO until the gate is verified green, AND the implementing agent
  should re-verify the gate locally immediately before invoking
  `git commit`. Mutual mechanical verification, not signal chains.

- **Gate-state opacity — `pnpm knip` independent vs pre-commit hook knip
  may disagree under racing landings**: I observed `pnpm knip` red,
  Lofty's pre-commit hook then passed knip seconds later. Either the
  hook ran under different cache state, or Shaded's `730766ad` landed
  in Lofty's hook window without me seeing it in my independent probe.
  Either way: independent gate probes are not a reliable proxy for the
  pre-commit hook's gate verdict when multiple agents are landing
  concurrently. **Cure direction**: investigate cache invalidation
  between the two invocation paths; or doctrine — only the pre-commit
  hook output is authoritative; gatekeeper independent probes are
  guidance, not verdict.

(Entries appended as friction is observed; not consolidated yet.)
...
- `ephemeral-to-permanent-homing` is useful but not a independent skill. It is a
