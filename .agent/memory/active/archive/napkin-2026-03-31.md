## Session 2026-03-31 — WS3 Phase 0 execution and RED spec design

### What changed

- Committed plan corrections from last session's specialist reviewer pass.
- Executed Phase 0 of the WS3 MCP App rebuild: canonical and non-canonical
  contamination inventory, 3 E2E RED specs, 2 GREEN unit tests, evidence table,
  superseded stale renderer-reactivation plan.
- Invoked test-reviewer, code-reviewer, and MCP reviewer. All findings addressed
  and committed.
- Refined RED tests after user feedback: removed constant-value assertions
  (WIDGET_TOOL_NAMES.size, BASE_WIDGET_URI slug), kept only behaviour-through-
  product-code assertions.

### Patterns to remember

- RED tests must prove product behaviour, not assert constant values. The useful
  assertion is "at least one tool advertises `_meta.ui.resourceUri`" (tests the
  projection pipeline), not "WIDGET_TOOL_NAMES.size > 0" (tests a constant).
- RED specs that specify future behaviour belong in E2E (`*.e2e.test.ts`), not
  in-process tests (`*.unit.test.ts`), because the pre-commit hook runs `test`
  but not `test:e2e`. A RED unit test blocks all commits.
- When Zod schemas validate external boundary data in tests, use `.loose()` on
  objects whose fields may grow (MCP protocol responses), but keep strict
  validation on the specific sub-objects being asserted (`_meta.ui`).
- Type helper parameters from the declared interface (`StubbedHttpApp['app']`),
  not from implementation inference (`Awaited<ReturnType<...>>['app']`).
- Specialist reviewer convergence from independent lenses (protocol, test quality,
  code quality) produces stronger confidence than a single comprehensive review.

### Durable guidance

- Phase 0 is complete. Phase 1 (delete legacy widget framework) is next.
- 3 E2E RED specs target Phase 3: UI tool metadata, app-only visibility, fresh
  resource slug.
- `widget-renderer-reactivation.md` (semantic-search/future/) is superseded —
  all files it references are Phase 1 deletion targets.
- The MCP reviewer confirmed: `_meta.ui.visibility: ["app"]` is protocol-correct,
  `registerAppTool` does zero schema enrichment, resource URI rename is a clean
  identity operation.
