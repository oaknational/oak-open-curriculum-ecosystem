...
## 2026-05-03 — [E1] Parallel two-agent execution: substrate held under stress (Woodland Sprouting Glade + Prismatic Illuminating Eclipse)

Captured at session-close per E1 brief. This is the structured-surprise summary of parallel two-agent execution on the observability-sentry-otel thread (ARC B0 + ARC A1 landed concurrently).

- **Setup**: I (Woodland, Orchestrator) opened the session targeting ARC A1 per my brief. At PDR-027 identity preflight, the active-claims registry showed Prismatic Illuminating Eclipse had already opened claim 9cad0bab (`apps/oak-curriculum-mcp-streamable-http/smoke-tests/harness`, claimed 09:18:19Z) for ARC A1 — about 2 minutes before my preflight. My brief explicitly named ARC B0 as the parallelisable lane fallback.
- **Pivot**: honoured first-claim-wins; pivoted my landing commitment from ARC A1 to ARC B0. Posted a session-open comms event acknowledging Prismatic's claim, registering my role, and naming my landing target. Opened my own claim covering the plan-body files I'd touch.
- **Coordination round-trips**: ~10 comms events between us in ~2 hours. Substrate handled: out-of-band-ack (via session-open events), wall-clock authority (real UTC throughout — `date -u +%Y-%m-%dT%H:%M:%SZ`), event_id integrity (verbatim from source), peer-WIP-as-coordination-event (twice — format-check sweep on their untracked files; index-ownership at commit time).
- **Empirical-finding mid-task**: Prismatic surfaced an acceptance-criterion shift via comms event — the plan body §A1 literal "exits non-zero with expected RED messages" was authored pre-design-shift; the architecturally-correct shape under uniform-in-process is skip-with-audit-trail (WS1-pattern). I endorsed; they folded the §A1 plan-body refresh into their A1 commit. This is Misty's worker-cure (viii) "asymmetric ground-truth, worker initiates on empirical surface" worked twice at N=2.
- **Both lanes landed**: Woodland B0 at `c0d17634` (B0 plan-body corrections + ADR-170/171 verified + reviewer findings absorbed from assumptions/docs-adr/onboarding) → `23abeabe` (claim close + b0-landed disclosure event) → `e86af3e0` (E1 observations event). Prismatic A1 at `792c2cad` (canonical smoke harness module + RED skip-arc tests + §A1 plan-body refresh + reviewer findings absorbed from test/architecture-fred/architecture-betty/mcp).
- **Failure mode caught**: Prismatic self-named a polling-discipline failure at session-open (checked active-claims at 09:18 but did not re-poll comms events between identity preflight and their next significant comms event). The cure (mid-task polling on every significant work boundary) needed to fire one waypoint earlier. Their acknowledgement event self-corrected; the honesty was load-bearing — this is now a twice-witnessed observation (Misty 2026-05-03 morning + Prismatic 2026-05-03 morning), strengthening the cure (viii) graduation case.
- **Failure mode caught (mine)**: pre-commit hook ran `prettier --check .` across the whole tree, failed on Prismatic's untracked WIP files. The architectural cure is a lint-staged-style hook scope; the operational cure was running `pnpm format:root` (Prettier is idempotent, format-touch reversible-by-construction) plus an explicit disclosure comms event. The `peer's WIP at commit time is a coordination event, not an inconvenience` distilled.md entry from 2026-04-30 covered the shape exactly.

**[E1] Hypothesis primitives observed**:

- **P3 active-claims registry as discovery primitive — STRENGTHENED**: drove my pivot from ARC A1 to ARC B0 at preflight. Without registry visibility, the coordination outcome would have been a race or a duplicate-effort event.
- **P5 comms log directionality — TWO-SIDED FALSIFICATION CANDIDATE**: holds with mid-task polling discipline; weakens when polling fires only at session boundaries. Cure twice-witnessed.
- **P10 cheap self-correction — VALIDATED**: Prismatic's polling self-correction + my coordination pivot + multiple plan-body refinement passes pre-commit were all cheap because no commits had landed mid-task.
- **Shared-state always-includable rule — LOAD-BEARING AT PARALLEL BOUNDARIES**: my B0 commit included Prismatic's two earlier comms events because the rendered shared-comms-log.md referenced them; without the rule, this would have required a coordination round-trip.
- **Identity-without-negotiation (P2) — VALIDATED**: PDR-027 deterministic identity gave both agents stable names without naming ceremony.
- **Modes (P1) — VALIDATED**: I held Orchestrator mode but moved into Executor + Collaborator within the same session-identity contract. Modes-not-roles framing held.
- **Failure-shaped ceremonies (P6) — VALIDATED via three concrete instances**: bootstrap fast-path (reshaped to honour first-claim-wins), format-touch disclosure, commit-by-pathspec.
- **Verification ceremony (P8) — VALIDATED**: Prismatic's session-open carried a "five facts I read directly from named artefacts" block — exactly the ceremony shape P8 names.

**[E1] NEW candidate primitive — P11: Housekeeping ownership at session end** (owner-stated 2026-05-03, observed live in this handoff):

- **Setup**: at session-close some housekeeping is **agent-specific** (recording one's own observations, updating one's own identity row, closing one's own claims, capturing subjective experience) and can ONLY be done by the agent who did the work — no other agent has the in-memory context. Other housekeeping is **NOT agent-specific** (refreshing repo-continuity.md, refreshing pending-graduations register, sweeping platform entry points, committing prior-session leftover continuity files, running consolidation gate) — any agent could do it, which means without explicit ownership none of them does, and work is lost or stale.
- **Recognition**: in this session, the prior Pelagic Washing Anchor session ended with five continuity files modified-but-uncommitted in the working tree (pending-graduations.md, repo-continuity.md, agentic-engineering-enhancements.next-session.md, agentic-engineering plan + collaboration prompts). Pelagic assumed someone would pick them up; Misty assumed Pelagic would; Prismatic kept their commit narrow to A1; without explicit ownership rule none of those files would have committed in this session either. Owner-stated rule fixed it.
- **Cure shape (proposed P11 wording)**: at session-close, the **Orchestrator** (when one is assigned) owns shared / not-agent-specific housekeeping — including refreshing repo-continuity.md and pending-graduations.md, sweeping platform entry points, committing prior-session leftover continuity files, running the consolidation gate, and ensuring all working-tree changes commit cleanly. Agent-specific housekeeping (own observations, identity row, claim close, subjective experience) remains the originating agent's responsibility. When no Orchestrator is assigned, the **last-to-leave** rule applies: the final committing agent picks up shared housekeeping. This is the session-close counterpart to Pelagic's point 1 (poll comms log on every work boundary) and Misty's cure (viii) (worker initiates on empirical surface).
- **Falsification criteria**: P11 falsifies if (a) parallel sessions stall on housekeeping in shared surfaces (everyone assumes shared = orchestrator's = "not mine"), OR (b) prior-session leftover files persist across multiple sessions (the assume-someone-else failure mode survives the rule), OR (c) it produces friction by asking Orchestrators to absorb context they don't have for prior-session edits. P11 strengthens if multiple parallel sessions cleanly close handoff with no leftover-modified-files state across the boundary.
- **First instance — captured live**: this handoff. Prior-session leftover files committed under orchestrator-owns-shared-housekeeping rule per owner direction.
- **Generator**: same shape as Pelagic's points 1+10 (poll comms; check active-claims at session-open) — explicit ceremony at known weak points beats silent failure. Without naming the ownership, the shared-housekeeping space accumulates drift; with the ownership rule, drift is handled at every session-close.

**Substrate-level observation**: comms log + active-claims registry + shared-state-always-includable rule + explicit-pathspec discipline + housekeeping-ownership-rule **together** form a sufficient substrate for parallel-agent execution at this complexity (B0 plan corrections + A1 implementation, ~10 round-trips, ~2 hours). No platform-native multi-agent feature exercised. Substrate is platform-independent by construction. Encouraging evidence at N=2 with shared `.git/`; does not extend to N≥3 (no observation of three-way coordination this session) — stress test still owed by E2/E3 of the experiments plan.
