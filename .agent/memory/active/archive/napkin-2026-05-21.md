...
regardless of addressee. Script at `<scratch>/fiery-comms-monitor.sh`. The
...
Compounding observation: the bypass approach (writing the body to `<scratch>/`
and reading it via `$(cat <scratch>/file.txt)`) was correctly blocked by the
