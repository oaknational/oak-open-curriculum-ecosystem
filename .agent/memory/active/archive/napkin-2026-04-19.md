## 2026-04-18 → 2026-04-19 — Observability planning restructure: Phase 4 + status markers + 5-wave reorder + physical reorder (commits 2e0be715, f1f2c259, 7f5b18e7, 2e8a140d)

Four commits on `feat/otel_sentry_enhancements` capturing the
observability planning restructure from Phase 4 through to the
owner-directed full physical reorder. No product code written — pure
plan-text work.

**Substantive outputs**:

- **Phase 4 of the restructure** (commit `2e0be715`) — revised the
  maximisation plan per the hardened Phase 4 section. Three-column
  MVP Classification table citing ADR-162 §Principle for every
  MVP-deferred lane; Foundation Alignment extended with ADR-160 +
  ADR-162; eight cross-references to sibling MVP `current/` plans.
  Three-reviewer matrix (sentry, fred, docs-adr) returned nine
  TO-ACTIONs; all actioned; four REJECTED with written rationale.
- **Status markers** (commit `f1f2c259`) — flagged forward-pointing
  references as planned, not code. `packages/core/observability-
  events/` and `packages/core/telemetry-redaction-core/` do not yet
  exist on disk; cross-references link planning authority, not
  runtime state.
- **5-wave execution reorder** (commit `7f5b18e7`) — reshaped the
  execution order across three plans to land foundations before
  emitters. Wave 1: Gates & Foundation Extractions (ESLint rules +
  telemetry-redaction-core extraction + L-7 release linkage + ADR-162
  Accepted flip). Wave 2: Schema Foundation (events workspace +
  vendor-independence ESLint carve-out). Wave 3: Primary Emitters
  (L-1, L-2, L-3, L-4b, L-9). Wave 4: Cross-axis & Widget (L-12 +
  security-observability + accessibility-observability). Wave 5:
  Operations + Conformance + Close-out.
- **Physical reorder** (commit `2e8a140d`) — complete the reshape by
  physically relocating lane sections so document structure matches
  execution order. Removed the dual-frame labelling introduced in the
  previous commit. Plan is now single-frame.

**Surprises / corrections this session**:

1. **"We are building containers for things without actually
   building them"** — owner observation. The plan work had created
   cross-references to plans for workspaces that don't yet exist on
   disk. The containers (plans, classification tables, cross-refs)
   were real; the contents (code, schemas, workspaces) were planned
   work. Rule extracted: forward-pointing references to planned
   surfaces MUST carry "planned, not yet code" status markers so
   readers can't infer existence from the reference. Deeper rule:
...
2. **Schemas-before-emitters is a load-bearing ordering principle.**
   Initial maximisation plan ordering had emitters (L-1, L-3, L-4b,
   L-9) landing before the events-workspace schemas they would emit
   through. This guarantees retrofit: every fixture assertion emits
   through ad-hoc shapes, schemas are authored later, and every
   emitter's tests need rewriting. The reorder fixed it by putting
   the events workspace in Wave 2 before any Wave 3 emitter. Related
   principles: rules-before-code (ESLint rules gate the code they
   police); extracted-cores-early (telemetry-redaction-core extracted
   before consumers use it). Generalises: when a plan has N emitters
   depending on a shared schema, authoring the schema is the gate,
   not the coincidence.

3. **Dual-frame labels invite drift; collapse them before the reader
   encounters them.** My initial reshape kept historical `## Phase N`
   headers + per-lane "**Execution phase**" notes + an authoritative
   §Phase Structure table. Three frames for the same information.
   Owner flagged this as "inaccurate plans shouldn't persist" and
   directed a full physical reorder. Rule: a document with multiple
   authoritative frames for the same concept is a drift trap; resolve
   to one frame when the decision is settled, even if the immediate
   edit cost is higher. "Transitional dual-frame with sunset note"
   is not stable — the sunset never fires in practice.

4. **`git commit` after `git add <file>` includes pre-existing
   staged changes.** When `git status` shows `MM` (modified in both
   index and working tree) on unrelated files, a plain `git commit`
   after `git add <my-file>` will bundle those already-staged changes
   into the commit. The commit message described only the intended
   change; the commit itself carried more. Not destructive (the
   bundled work was legitimate parallel research), but the mismatch
   between commit message and commit content matters. Rule: before
   committing, inspect the index state (`git diff --cached` or
   `git status` with careful read of the two-letter codes); if
   unrelated changes are staged, reset them (`git restore --staged
   <path>`) before committing.

5. **Auto mode requires commit-behaviour discipline.** Under auto
   mode the gate hook fires after the commit message lands;
   commit-msg hook's 100-char header limit caught a 101-char header
   and required a retry. Rule: commit subjects should target ≤80
   chars for safety margin against the 100-char ceiling.

**Candidate patterns for future extraction**:

- **"Forward-pointing planning references must carry 'planned, not
  yet code' markers"** — validated once this session; candidate if a
  second instance surfaces in a different planning context.
- **"Schemas-before-emitters is the canonical wave-1 gate for
  multi-emitter plans"** — strong candidate; the architectural
  reasoning generalises. Rule-before-code + extracted-core-early are
  companion principles.
- **"Transitional dual-frame labelling is unstable"** — generalises
  from this session's plan-structure frame to any doc with two
  authoritative descriptions of the same thing. Watchlist.
- **"Stage what you commit, commit what you staged"** — the
  inspect-index-before-commit discipline. Single instance this
  session; watchlist.

**Deep consolidation status** — due at next session. Four commits +
major architectural reshape + multiple surprise-level patterns
warrant a deliberate consolidation pass rather than a rushed tail
on this session. Not well-bounded for this closeout.

## 2026-04-19 — Sentry and PostHog research normalised + skill upgrades

Recovered `.agent/research/sentry-and-posthog/Sentry and PostHog.md` ChatGPT
deep-research export into sibling `Sentry and PostHog-clean.md`: stripped
PUA-wrapped `cite`/`turn…` blocks, mapped 80 positions 1:1 to
pre-bibliography `pandoc -t gfm` citations from the paired DOCX,
drift-proofed stripped body text against the source scaffold.

**Citation-set audit (this report)** — the canonical diagnostic for this
normalisation pass. Recorded here in the napkin per the updated skill
policy; the clean file does not embed any `### Recovery notes` block.

- 318 `turn…` references in source markdown (115 unique). These are
  ChatGPT's internal deep-research search-result identifiers, not
  citations.
- 80 PUA-wrapped citation blocks at distinct prose positions (mean ~4 turn
  refs per block).
- 36 unique external URLs in the DOCX `_rels`.
- 36 unique citation URIs in the PDF annotations (extracted via `pypdf`,
  ignoring one branding footer link
  `https://chatgpt.com/?utm_src=deep-research-pdf`).
- 36 citations shown in the ChatGPT Sources panel screenshot for the
  conversation.
- 80 numbered citations emitted in the clean body, mapping to 36 unique
  URLs after deduplication — the full canonical citation set.

The DOCX `_rels`, PDF annotations, and Sources panel **agree exactly** on
36 cited URLs. The 115→36 funnel is editorial selection (sites consulted
vs sites cited), not recovery loss. **Disagreement between those three
surfaces** is the real signal worth chasing in a future audit; agreement
means the citation set is intact.

Three durable upgrades landed in
`.agent/skills/chatgpt-report-normalisation/SKILL.md` and
`.agent/memory/patterns/chatgpt-report-normalisation.md`:

1. **References-table rendering convention is now the default.** Inline form
   is `[[N]](#ref-N)` (anchor link, brackets visible). Bibliography is a
   `## References` section grouped thematically by source family with
   `- <a id="ref-N"></a>**[N]** <https://url>` entries. Same URL at multiple
   positions reuses one number — reuse becomes visible. URLs only, no
   invented titles.
2. **Citation-set audit is required and lives in the napkin only.** The
   clean file is a faithful copy of the report plus its references table;
   meta-information about the recovery pass (counts, surface-agreement
   notes) does not belong inside it. The audit is agent-side meta
   recorded in `.agent/memory/napkin.md` for the current session, where
   the next normalisation pass will read it.
3. **DOCX `_rels` is the canonical citation URL-set surface for ChatGPT
   deep-research exports.** Earlier guidance treated `_rels` as
   often-empty and pandoc as the "primary citation recovery surface" —
   the 2026-04-19 audit disproved that for this class of export. Updated
   framing: `_rels` gives the **URL set**, pandoc gives the **positions**
   in the prose, both are needed.

Final folder state requested by owner: `Sentry and PostHog-clean.md` is
the only file kept in `.agent/research/sentry-and-posthog/`. The source
`.md` and `.docx`, the temporary PDF, and the preserved
`-clean-attempt-1.md` were deleted at owner direction once the canonical
audit and skill updates were captured here.
