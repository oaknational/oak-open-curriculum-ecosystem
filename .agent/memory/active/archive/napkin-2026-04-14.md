...
### Session 2026-04-14f: Upstream offset/limit fix in our codegen

**Explored oak-openapi codebase to trace root cause.** The
swapped descriptions were introduced in commit `ea55594f`
(2026-01-08, Remy Sharp) — intentionally added per-unit
pagination context for AI agents, but placed on the wrong
parameter names. The `.describe()` calls were manually edited
into a generated file (not from the source schema).

**oak-openapi's generator is known-broken.** CLAUDE.md in that
repo says: "`pnpm generate:openapi` is currently broken and
must not be run." Changes must be made to both source and
generated files by hand. The crash (`TypeError: ObjectExpression
expected ObjectProperty but got CallExpression`) occurs on the
`keywords` endpoint, regardless of Node version.

**Fix locally with a removal-condition test.** Rather than
waiting for upstream, added a `PARAM_DESCRIPTION_OVERRIDES` map
in our codegen keyed by `{path}:{paramName}`. Each entry stores
both the correct description and the known-buggy upstream
description. A removal-condition test reads the schema cache and
asserts the upstream bug persists — when it's fixed and
`pnpm sdk-codegen` refreshes the cache, the test fails with an
explicit message to remove the override. Pattern: self-expiring
workaround with automated detection.

**Read the repo instructions before working in unfamiliar code.**
Initially ran the broken generator and created git state
confusion (stash conflicts with regenerated files). Reading
CLAUDE.md first would have avoided this entirely.
...
same lint output is wasteful when one `> <scratch>/file` + `Read`
would suffice.
