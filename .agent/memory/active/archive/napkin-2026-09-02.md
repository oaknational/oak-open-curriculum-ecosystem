...
## Napkin rotated (2026-08-14 dedicated consolidation, Quasar wakes Nadir, 0fabcd)

Rotated inside the owner-launched dedicated consolidation (consolidate-until-done
frame; claim 68e53f09). The processed window — the 2026-08-07 rotation record and
every entry through 2026-08-14 06:08Z (3,490 lines) — is preserved verbatim in
`archive/napkin-2026-08-14.md` (`cmp`-proven byte-identical at the move; one
post-move label truing disclosed in that file: the 2026-08-14 owner-correction
entry's "verbatim core" label now reads "expletive redacted" to stay honest over
the estate-wide expletive sweep committed at 659331a69). Every behaviour-changing
item was read and dispositioned first-hand; the commits and the homes are the
record. Headline landings: three new pattern files
(`values-enter-by-first-hand-right-frame-read`, `destructive-path-preconditions`,
`timing-artefact-read-as-state`); the five pending-graduations rows homed
(principles ×2, user-collaboration, metacognition, use-monitor rule); rule
clauses in `sonarqube-mcp-instructions` (issueStatuses facet),
`bot-identity-on-third-party-systems` (cwd-pin / length-guard / echo-back
tripwires), `verify-vendor-call-shapes` (sub-case enumeration +
execution-time re-verify), `start-right-team` (adopted-claim premise
recompute), and the prose-expert template (rhetorical-strengthening check);
four staged entries in `distilled.md`.

**Comms watermarks declared by this pass (PDR-094)**: non-heartbeat classes
swept through **2026-08-14T06:16:28Z** — all 496 post-watermark coordination
events body-read full-line in-seat, zero unhomed durable signals (the fleet's
capture-at-occurrence held). Heartbeat class swept through
**2026-08-12T06:29Z** with the supplemental batch aggregate: 3,045 events,
span 2026-08-07T16:12Z→2026-08-12T06:29Z, 10 seats, top emitters Plover
lifts Troposphere 1080 / Civet spins Cavern 590 / Swordfish wakes Trench
378 / Wren calls Downdraft 377 / Forge lifts Kindling 219; zero over-length
bodies; spread sample body-read. Archive moves this pass: 4,420 events
(1,375 pre-watermark past-window + 3,045 heartbeats), loss-free counts
verified, provenance gate green before and after; live stream 1,313 events,
all in-window or newly swept.
...
- **Codex vendor-memory corpus audit** — owner-AUTHORISED routed lane
  (Director ROUTE event f7565aab, 2026-08-07; one seat-session, disposition
  record deliverable). Still unexecuted at this rotation. Sizing facts from
  the branch lineage (2026-08-07): ~4.6k lines plus 46 rollout summaries,
  locally git-versioned; the Cursor and Gemini per-user stores on the same
  machine verified empty.
- **Named-consumer seeds** held in `archive/napkin-2026-08-07.md` at their
  dated entries (retrospective corpus data points; folly-story temperament
  associations; design-wake loss-scan pointers) — pointer-carry, the archive
  is the substance. The 2026-08-14 archive adds this window's free-play
  seeds at their dated entries.
- **Claude per-user memory index (`MEMORY.md`) needs a verified curation
  pass** (163/140 on its line-limit hook at the 2026-08-12 flag; grown
  since) — per `knowledge-preservation-over-fitness-warnings`, never
  blind-trimmed; per-user buffer graduation is the cure shape.
- **PDR-098 recurrence evidence routed**: the licensed-activity-is-not-value
  class recurred at a curator seat 2026-08-14 (this session's owner
...
## Fold-merge union note (2026-08-17, Smith hunts Obsidian, e98f17)

The 2026-08-17 rotation merge met the pre-rotation napkin lineage riding
PR #882's branch (cut before the 2026-08-14 rotation). Union resolution:
the rotated napkin stands (its window is preserved verbatim in
`archive/napkin-2026-08-14.md`; 113 of 116 branch-side entries verified
present there, one under its trued heading); the three entries below were
appended on that branch only and are carried here verbatim so nothing is
lost. The richer sizing facts from the branch's Codex-audit carried-item
bullet are folded into the live bullet above.
...
## 2026-09-02 ~11:2xZ (Luna seeks Twilight, 5c0ddc) — wrap after the #945 landing: two facts I passed on without seeing, a dead wake, and the order-not-content lesson (metacognition / free-play / concept-exploration / wrap; owner-invoked)

- LANDED against the session target: #945 merged by the owner 11:04:45Z as `bf8db3a8e`
  (branch deleted); live-service validation GO on the preview; the memory-record union
  second-read by Kiln before push. Records rode the merge; this block and the lane truing
  below are the wrap's only additions, left uncommitted on the primary at owner word.
- Mistake (transmitted assumption, twice): (1) told the owner "your approval is the only
  gate" from the inherited lane text "code-owner review required — bot-authored"; the owner
  says ruleset 19395183 exempts the bot (relayed by Kiln 10:45Z; not read by me from the
  ruleset API). The lane text was written from a 2026-09-01 ruleset read and never
  re-verified. (2) wrote "release 1.175.2" into repo-continuity from the shape of a Sonar
  timestamp; #946 shipped in 1.175.3 — caught by the second reader. Same signature both
  times: a fluent fact from a record or a memory, never touched. Cure that held: "have I
  seen it, or only read about it?" before every declarative sentence to the owner.
- Order-not-content: the paused lane's "dependency" on MCP-655 was an ORDER dependency (wait
  for a merge) the owner dissolved by cherry-picking the exact fix commit onto the paused
  branch ("option 2"); validation ran in parallel; the second merge of main reconciled the
  identical hunks trivially and only the four files the fix line moved AFTER the pick needed
  a decision (all: take main's side, since the dependent branch had no edits of its own
  there). Concept-exploration seed with a growing shape, routed as a pointer for the
  pr-lifecycle / proportionality practice, not authored today: "de-serialise a dependent PR
  by cherry-picking the dependency's exact commit; expect the memory-file union at the
  second merge; the falsifier is a dependent branch that ALSO edits the files the dependency
  line moves after the pick".
- Owner corrections absorbed: "ignore the napkin length and other fitness function issues"
  (a dedicated consolidation follows; nothing trimmed); "not important enough to spend a
  cycle on!" (the union's ornaments — carries, a dated heading, a second read of the pushed
  tip — after the merge was the landing; I took the cure and skipped the second pass).
- What worked: the semantic-merge skill's second reader (Kiln, from the object store, no
  tree access) found the one wrong number in a minute; the clean-side preservation via index
  stages made both unions provable (heading set-diff + body-line diff + era witness);
  `git diff --quiet 2f14f6f76 <branch> -- <file>` decided the four code conflicts without a
  read of their content; the owner's Claude Code sign-in proved the token path end to end
  where a metadata probe could not.
- Free-play harvest (associations, not findings): (a) RFC 9207's issuer mismatch and the
  two-lineage napkin look shaped alike — identity by lineage vs identity by content; both
  need a shared ancestor to stop re-litigating (kept, hums quietly); (b) Kiln's 1.175.2
  catch reminded me of the 2026-09-01 `--proto` array-expansion catch: a second reader
  holding the invariant beats the author's second look (kept; already doctrine); (c) "the
  napkin's conflict-proneness is a review-forcing feature" — forced, discarded; (d) the
  runbook names what moves under a slug row but not under a route row — the changelog row
  failed for a reason the runbook could not name (kept; pointer for MCP-630's runbook truing:
  each row should say what moves beneath it — corpus, upstream API, or app).

### Practice/tooling feedback

- **Surface**: `agent-tools:pr-watch <n> --repo … --watch` as a Monitor. **Signal**:
  friction. It emitted nothing across two pushes and a full green check run (~30 min); its
  silence was indistinguishable from "no change". Replaced by a 60 s `gh pr view` poll that
  emits only on a reviewDecision/mergeStateStatus/head change and terminates on
  MERGED/CLOSED — it caught the owner's merge within a minute. Wish: pr-watch emits one line
  per head change and per check-state transition, and a heartbeat line at a fixed cadence so
  a dead watcher is visible.
- **Surface**: `claims open` areas. **Signal**: friction. No amend verb: a mis-named area
  path meant close + reopen (recorded in the closure summary). Wish: `claims amend --area`.
- **Surface**: comms watcher backstop. **Signal**: impression. The 3600 s `timeout` exit
  (124) fired twice today, each time re-armed within a minute; the re-arm is cheap and the
  exit is visible — the design holds.

### Loss scan and metaloss (wrap)

- Compressed reasoning: the union decisions (which side, why) are in the merge commit
  messages `8d840655c` and `0090465a4` and the ARC channel entries; decision-sufficient for a
  successor who has to redo a napkin union. Promises: production smoke of rows 1.1–1.3 after
  the release carrying `bf8db3a8e` — FORWARDED to the owner in the thread record (the release
  run had not appeared at the wrap; the owner has already proven the production sign-in on
  1.175.4 for MCP-655); Kiln's optional second read of the pushed tip — declined at owner
  word; nothing else promised. Attribution inferences: "ruleset exempts the bot" is the
  owner's word RELAYED by Kiln, not read from the ruleset; "Cursor authenticated on the #945
  preview" is the owner's word relayed by Kiln. Blind spots: the watcher was down 09:50→09:51Z
  and 10:50→10:51Z (backstop re-arms; swept with `comms list` — no events lost); the
  pr-watch's silent half hour; Kiln's and Finch's contexts. Index of homes: thread record
  §Lanes (#943→#945 lane, COMPLETE), `uat-reports/2026-09-02-preview.md`, #945 bot comments
  5507228301 / 5508074306, the ARC channel file (untracked, conserved by this entry and the
  thread record), the experience letter `2026-09-02-luna-seeks-twilight-the-dependency-was-an-order-not-a-content.md`,
  the per-user memory (RESUME HEADS, merge-rulings, the option-2 pattern). External bound:
  the two transmitted-assumption errors were caught by the owner and by a peer, never by my
  own scan — point outside eyes at any declarative sentence I make about gates, versions or
  states I did not read myself. A third pass would only re-find the owed production smoke
  and the relayed-word attributions; the recursion closes here.
- Addendum at 11:14Z: the promise above is DISCHARGED — release 1.176.0 (`cbcd76231`, run
  11:11Z) carries `bf8db3a8e`; production rows 1.1–1.3 PASS at 11:13:56Z (healthz 200
  no-store; PRM names `https://clerk.thenational.academy`; 401 with `resource_metadata`).
  And a sharper reading of mistake (1): my own per-user memory (the 2026-07-21 merge
  rulings) already held "ruleset 19395183 — bot exempt from review only"; I transmitted the
  inherited record's contrary sentence over my own durable memory without reconciling the
  two. The failure was not a missing fact but an unreconciled pair — when a record and a
  memory disagree, that disagreement IS the finding; read the source before repeating either.
...
- CORRECTION (owner, verbatim "I don't think that is true, I think the bot is on the bypass
  ...
  Copilot leg all along; the tool told me, I had not run it before speaking. (Luna hit the
  same pair from the other side — her memory held the ruling, the inherited record held the
  contrary sentence; two seats, one unreconciled pair.)
...
- CORRECTION (rule, caught at wrap): I had opened a continuity BRANCH and drafted a PR for
  these records; the no-handover-commits ruling (owner, 2026-07-15) bans exactly that, and
  the owner's wrap word said "leave the changes uncommitted in primary main". Luna read it
  right first. Cure: records written into the primary's working tree, uncommitted, layered
  on hers after a settle-wait; the branch copy stays uncommitted and disposable.
...
- Promises sweep: every commitment discharged (Luna update, #945 comment, gate-states,
  ...
  forwarded with owners — the owner: Finch's ff-only pull on the primary (blocked by the
  uncommitted `.cursor/mcp.json` he ruled to leave), the #908 → #915 sequence; nobody:
  MCP-656 and the SDK v2 exploration stay pointers. Luna's production smoke: discharged by
  her own addendum above (1.176.0 carries `bf8db3a8e`).
- Attribution inferences flagged: release 1.175.4 (11:03Z) — by timing the #948 docs merge
  cut it, unverified; "Luna applied the three carries at 8b642d911" — her word, the owner
  declined a second pass; "Cursor exercised tools" — the owner's word, content unseen.
- Blind-spot bounds: no view of Luna's or Finch's terminals; the primary's state read only
  through its gitdir, file mtimes and a read-only status; Copilot's review never bound
  `bbc4f7b7a` before the owner merged (the merge was his call, not a settlement verdict).
...
- External bound and error signature: three of the day's four owner-visible corrections came
  from the owner reading my sentences, one from a rule I re-read at wrap, one from CI; none
  from my own checks. Point external scrutiny at owner-facing claims about GATES, about
