## Session 2026-04-03 - semantic atlas consolidation

### What Was Done

- Used the deep-convergence workflow to consolidate the semantic atlas set in
  `.agent/research/developer-experience/novel/`.
- Added `novel/README.md` as a reading guide that marks the core atlas,
  focused supplements, exact-trace notes, and recommended reading paths.
- Updated the frontier map and specialist sweep so they no longer read as if
  the exact-trace pass were still pending.
- Tightened the terminology in the atlas publication layer so the guide and
  supporting notes match the later exact-trace set.
- Verified the guide and touched atlas notes with `markdownlint`, then
  completed the follow-up file-scope validation pass.
- Added a distilled process rule about using `rg -uu` when validating ignored
  research estates, after catching a false-clean sweep caused by ignore rules.
- Ran `pnpm practice:fitness:informational`; the informational warnings were
  pre-existing in directive and governance docs outside this edit slice.

### Patterns to Remember

- Once a research cluster has both broad briefs and exact traces, add a
  reading guide and explicitly mark which notes are core versus supporting;
  otherwise the archive becomes accurate but hard to navigate.
- Ignore rules can quietly invalidate documentation hygiene sweeps; treat
  ignored research estates as a special search mode rather than trusting the
  default repo search behaviour.
...
- Planned targeted validation around markdown linting, evaluative-language
  leaks instead of treating the note as an unstructured essay.
...
- Validated the new notes and fixed the remaining heading-spacing and
  line-length markdown issues.
...
- If the user asks for fully semantic writing, apply that rule to local repo
  surfaces as well as to external surfaces.
