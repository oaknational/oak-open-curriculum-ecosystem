...
- **Actual:** the original hotspot key did drop, but Sonar re-fired the same S4036 rule at the *new* call sites (`execFileSync('git', …)` at lines 287 and 310 inside the new capabilities, and at line 39 of the e2e test). Plus three new S5443 hotspots fired on `<scratch>/evil` test fixtures, and a new S3776 critical violation appeared because my reviewer-absorption layered fetch-then-retry-show inside an already-nested catch block. Net change: `new_security_hotspots_reviewed` went 90.9% → 62.5%; `new_violations` went 27 → 28. The cluster commit made the QG worse, not better, despite the original hotspot being closed.
...
`~/.claude/plans/composed-petting-hejlsberg.md`. The plan
...
- **Observation**: `agent-tools` is this repo's TypeScript-specific
  implementation surface for Practice-owned capabilities. The capability
  contract needs to stay portable, while implementation details stay
  host-local and optional per host.
...
- ADR/PDR drift found and partially repaired: collaboration-state references
  ...
  note.
- Agent-tools README/docs now say explicitly that the workspace is a
  host-local implementation of portable Practice capabilities, not the
  Practice itself. Graduated in this consolidation pass via PDR-035,
  ADR-165, and the `agent-tools` README boundary section.

### Owner doctrine: context-specific layers stay thin

- **Surface**: Practice + application architecture + `agent-tools`
- **Signal**: owner doctrine / architectural clarification
- **Observation**: the repo has three distinct areas: application code and
  configuration; the portable Practice; and host-local operational tooling
  such as `agent-tools`. Across all three, functionality should be pushed to
  the lowest general layer that preserves architectural excellence. The most
  context-specific layer sits at the top and must be the thinnest possible
  wrapper, preferably configuration only.
- **Behaviour change / candidate follow-up**: use the specificity gradient as
  a first-principles check when adding or moving capability: many-repo
  behaviour belongs in portable doctrine or general tooling; repo-local
  generic behaviour stays domain-neutral; Oak-specific broad and narrow
  wrappers must remain thin over generally useful tools. Treat Oak-specific
  state, especially hand-rolled types beside generated SDK output, as pressure
  to push more behaviour into generation or a lower layer.
- **Graduation**: PDR-035 now records the stronger owner doctrine that
  agent collaboration, coordination, work management, direction, lifecycle,
  identity, claims, handoff, review routing, and adjacent mechanisms belong
  to the Practice by default; ADR-165 records this repo's local phenotype
  boundary.
- **Source plane**: active
...
## 2026-04-28 — Workspace boundaries are layer boundaries

**Context:** owner clarified the missing enforcement edge in the specificity
gradient: most code in the repo is not properly separated into layers, and
distinct architectural layers MUST live in distinct workspaces.

### Behaviour change

- Treat a same-workspace split by directory, module, barrel, or internal
  package convention as organisation within one layer, not as layer separation.
- Use workspace topology as the enforcement surface for ADR-154: if a generic
  mechanism, repo-generic capability, purpose-specific reusable tool, Oak-wide
  wrapper, and narrow Oak-domain leaf differ by layer, they need separate
  workspaces.
- Before moving code, build a layer/workspace matrix. The current executable
  home is
  `architecture-and-infrastructure/current/workspace-layer-separation-audit.plan.md`.
- Oak-specific authored state remains pressure to push behaviour down into a
  lower, more general workspace or into generated artefacts where the
  schema-first contract applies.

### Graduation

- `principles.md` and ADR-154 now state the one-layer-per-workspace rule.
- The Oak surface isolation programme now treats workspace splitting as the
  immediate audit/migration-preparation lane, not just an Oak-name removal
  exercise.
- **Source plane:** active
