...
## 2026-05-11 — Dusky Masking Cloak / claude-code / opus-4-7-1m / `c5ff7f`

**Surprise — opener was overtaken by substrate before the next session opened.** The 2026-05-11 graph-execution-prep opener defined step 1 as "absorb the two 2026-05-07 architecture-expert-betty topology BLOCKERs into graph-stack.plan.md before ACTIVE promotion / ADR-173 ratification." But verification against source-of-truth substrate (graph-stack.plan.md last-updated 2026-05-10; ADR-173) showed both BLOCKERs already remediated in-place — the 2026-05-10 edit had absorbed them between the review (2026-05-07) and the opener (2026-05-11). The opener was authored presuming outstanding remediation; the substrate had moved past it.

**Insight**: a planning artefact that defers work to a "next session" can be overtaken by execution before the deferral closes. Opener authors carrying forward language from a closeout document need to re-verify substrate state at opener authoring time, not just at next-session opening. The "Ground state before constructing plans" feedback memory applied at session open here saved a no-op execution; without that grounding step, I would have made a redundant amendment edit and called it landed.

**Behaviour change**: when an opener prescribes "amend X to absorb Y from review Z", the first action of the next session is to verify whether Y has already been absorbed into the canonical surface — and to reshape step 1 accordingly. The user's explicit choice of "Keep step 1 as full amendment session" (re-reading the review artefact directly) prevented declaring closure by inspection; reviewer attestation surfaced a real third concern (D-4a: ADR-041 amendment needed) the inspection-only path would have missed.

**Surprise — multi-reviewer dispatch caught a finding the opener's prescribed reviewer set wouldn't have.** Opener prescribed `architecture-expert-betty + code-expert` for step 2 (Inc.1 decomposition). Betty returned CLOSED with one minor follow-up (WS4.2 earliest-start nuance). `code-expert` independently surfaced a structurally distinct finding: WS2.1+WS3.1 scaffold pair is NOT parallel-safe because both write to repo-root monorepo registration files (`pnpm-workspace.yaml`, root `tsconfig.json`, root `package.json`). The plan body had grouped WS2.1+WS3.1 alongside WS2.2+WS3.2 and WS2.3+WS3.3 in the "parallel-safe pairs" list, masking the scaffold step's shared write surface. Different-lens reviewers found different gaps.

**Insight**: reviewer dispatch is not redundancy when the reviewers have different lenses. Boundary correctness (betty) and file-conflict surface (code) are distinct concerns; both are needed. The opener was right to prescribe both; the plan-as-amended now carves WS2.1+WS3.1 out of the parallel-safe set explicitly.
