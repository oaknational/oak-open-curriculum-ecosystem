...
(`git commit -F - ><scratch>/commit.log 2>&1`) also completes
...
(`git commit -F - ><scratch>/commit.log 2>&1; cat <scratch>/commit.log`).
...
it's a frack up"* + *"these documents ARE the Practice, and cutting
...
- Three rehoming open items recorded as honest PDR-026 deferrals
  ...
  - personal holding-bay item disposition → the research-notes bay README (bay since retired)
