...
### Session 2026-04-07 — P1b validation + P2 SDK alignment + docs/infra

#### Surprises

**S1: Playwright port contention in parallel Turbo tasks.**
Expected: `test:widget:ui` and `test:widget:a11y` could run in parallel.
Actual: Both share `playwright.widget.config.ts` which starts Vite on a
fixed port. When Turbo ran them concurrently, one got
`ERR_CONNECTION_REFUSED`. Same issue for `test:ui`/`test:a11y` (port 3333)
and `smoke:dev:stub`. Fix: isolated ports (3334 for server Playwright,
5174 for widget Playwright) plus `dependsOn` for tasks sharing a config
(real constraint, not a workaround — shared webServer definition).

**S2: accent-strong validated as non-text but used as text colour.**
Three reviewers (design-system, accessibility, code-reviewer) independently
flagged: `accent-strong` on `surface-page` passes 3:1 non-text but fails
4.5:1 text, yet generic `a:hover` was using it as text colour at 16px.
Fix: generic `a:hover` uses `text-decoration-thickness` instead of colour
change. Banner link unaffected (20px bold = large text, passes 3:1).

**S3: MCP Apps CSP metadata placement — spec says content items only.**
MCP reviewer cited `<scratch>/mcp-ext-apps/docs/csp-cors.md` line 61: CSP goes
on `contents[]` items, not on `registerAppResource` config. All SDK
examples match this. Extracted to `WIDGET_UI_META` constant.

**S4: "lesson-listing" was never a planned feature.**
User corrected: the widget file was wrongly named `lesson-listing.html`.
There was never a plan to build a lesson listing. Renamed to
`oak-banner.html` throughout. Lesson: name things by what they ARE, not
by imagined future features.

#### Decisions

- Generic `a:hover` uses underline thickness, not colour, for state change
- `dev:widget-in-host` is the semantic name (replaces `dev:basic-host`)
- Turbo `check` uses single `turbo run` invocation, not separate commands
- Portability validator excludes vendor MCP Apps skills (not our canonical skills)
- `a:focus-visible` must always have an outline ring, not just underline change
