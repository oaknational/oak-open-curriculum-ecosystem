...
## 2026-04-26 — Codex — reviewer norm correction

**Correction:** I incorrectly framed sub-agent review as requiring special
authorisation. In this Practice, specialist sub-agent review is standard and
preferred for substantive work when the platform supports it. The key
distinction is not "review blocks everything"; it is "review findings need
explicit disposition, and blocking findings / hard gate failures block
closure."

**Lesson:** reviewer dispatch should be treated as normal evidence, not an
exceptional escalation. When skipped, record why. When run, record which
findings were implemented, rejected with rationale, or deliberately deferred
with owner-visible evidence.

**Promotion trigger:** none new; this refines existing reviewer-phasing and
reviewer-invocation doctrine already present in distilled memory and
executive guidance.
...
## 2026-04-26 — MCP Apps widget list + tests vs configuration (Composer / cursor)

Date normalised from a future-dated `2026-04-27` heading during the
2026-04-26 coordination closeout.
