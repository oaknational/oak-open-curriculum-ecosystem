...
## Session: 2026-05-31 — repair of invalid source-buffer archive move

The prior Eclipsed source-buffer pass moved this content into
`archive/napkin-2026-05-31-eclipsed-source-processing.md`, replaced the active
file with a pointer, then deleted the failed archive copy during repair. That hid
live source material from fitness before an acceptable item-level disposition
proof existed. The content was restored here by Open Lofting Cliff after owner
correction. Do not archive or replace this active buffer again unless every source
item has a valid disposition ledger under `consolidate-until-done`.

### Handoff repair note — stale brief surfaces can reintroduce the bad frame

During session handoff, the older
`.agent/memory/operational/codex-docs-consolidation.brief.md` still contained
pre-repair instructions that named napkin rotation and distilled criticality in
the old frame. That would have sent the next single agent toward stale
archive/rotation assumptions. Behaviour change: when a repair session produces a
pass-forward brief in chat, update the repo brief surface too, otherwise the next
session may read the stale artefact instead of the corrected instruction.

## Session: 2026-05-31 — EEF D0 completion + intent-vs-letter audit (Opalescent Transiting Prism / `73491c`)

### Codex docs-consolidation source-buffer lane — zsh glob expansion on claim patterns

During this dedicated docs-consolidation session, I tried to open a source-buffer
claim with unquoted `--area-pattern .agent/memory/active/archive/**` and
`--area-pattern .agent/state/collaboration/comms/**`. zsh expanded those globs
into thousands of concrete filenames before `agent-tools` saw the arguments, so
the command failed noisily and produced a huge, useless output blob. Behaviour
change: any collaboration-state command that takes a pattern argument must quote
the pattern literally (`--area-pattern '.agent/state/collaboration/comms/**'`),
especially in curation lanes where the target directories are intentionally
large.
