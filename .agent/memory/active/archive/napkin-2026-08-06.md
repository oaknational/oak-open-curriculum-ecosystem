...
## Session: 2026-08-05 — MCP-143 stacked PR second opinion

### Mistakes Made

- Tried to probe `HttpEnvSchema` with `pnpm --filter ... exec tsx -e`; source-mode
  import failed because `@oaknational/env` has no directly consumable exports
  before the workspace build. For env-schema behaviour, use the package's Vitest
  configuration or built workspace artefacts rather than an ad hoc `tsx -e`.

### What Was Done

- Ran independent code, Clerk, security, MCP, test, and docs/ADR reviews across
  stacked PRs #757, #759, and #761. Submitted verified `CHANGES_REQUESTED`
  reviews on all three, attributed to their introducing PR rather than treating
  stack-head findings as standalone changes.

### Patterns to Remember

- Vercel's `VERCEL` and `VERCEL_ENV` are controlled by the same system-variable
  exposure setting, so they are correlated signals and cannot corroborate each
  other against that setting being disabled.
- Canonicalising OAuth metadata to one origin does not make alternate origins
  equivalent client endpoints: RFC 9728 clients require the advertised resource
  to match the URL they used. Alternate deployment aliases must be made
  unreachable or explicitly unsupported.
- Collaboration `comms direct --body` is capped at 1500 characters. Compress
  directed routing messages below that limit or use a durable handoff record
  with `--body-file`; do not attempt a long inline relay first.
- On 2026-08-05, Vercel scoped `CANONICAL_HOST` to Production only for the MCP
  project; Preview and Development had no `CANONICAL_HOST`, which is the intended
  production-resource boundary.

## Session: 2026-08-05 — PR #757 re-review correction

### Mistakes Made

- The first re-review correctly required a production allowlist, but then made
  the post-deployment Clerk realm flip and redirect probe a pre-merge blocker.
  That inverted the ratified cascade: the fail-closed guard must land before the
  coordinated live-key/www deployment can prove the new production state.

### Patterns to Remember

- Review rollout gates against the dependency direction. A post-deploy
  verification cannot block the prerequisite guard PR when doing so makes the
  deployment sequence circular; preserve the operational risk as a named
  release gate instead.
- PR reviews should record a WCAG pass for rendered Markdown semantics,
  including heading hierarchy, and end with explicit reviewer/model provenance.
