...
## Session 2026-03-07 — Directory Complexity Canonical Plan

### Patterns to Remember

- When a user asks for one canonical plan, move the detailed execution path into a single `current/` or `active/` file and reduce adjacent plans to references plus intent/status.
- During `/jc-consolidate-docs`, check platform-specific memory and current-platform plan artefacts explicitly; often they are confirmatory rather than generative, but record that the check happened.
...
## Session 2026-03-07 — MCP Apps Inventory Terminology

### What Was Done

- Traced the current and archived planning/research artefacts for the MCP Apps
  migration and the associated widget/runtime files.

### Patterns to Remember

- In this repo, MCP Apps migration docs are about replacing the old
  OpenAI-specific widget/app surface (`window.openai`, `openai/*`,
  `text/html+skybridge`) with MCP Apps standard surfaces
  (`_meta.ui.resourceUri`, `text/html;profile=mcp-app`,
  `@modelcontextprotocol/ext-apps/server`).
- This is distinct from the repo's OpenAPI schema-first rule, which remains
  mandatory for SDK/codegen and MCP tool execution. Do not describe the work as
  replacing OpenAPI infrastructure unless a file explicitly says that.

---

## Session 2026-03-07 — oak-preview Snagging Plan

### What Was Done

- Confirmed the `download-asset` preview issue was a Vercel preview-auth/bypass
  configuration quirk, not a product defect: the commit deployment URL was not
  on the bypass list, while the branch deployment URL was.
- Created the oak-preview snagging plan, now promoted to
  `.agent/plans/sdk-and-mcp-enhancements/active/oak-preview-mcp-snagging.execution.plan.md`,
  to separate deploy-safe MCP snags from reindex-dependent search validation.
- Wired the plan into the collection indexes, initially through
  `current/README.md` and later through `active/README.md`, plus the collection
  `README.md`, the collection `roadmap.md`, and the reverse coordination link
  from the active MCP Apps migration plan.
- Tightened the plan after reviewer feedback: removed one overclaimed schema
  finding, made validation commands executable, clarified the Phase 3 boundary
  against Domain C/C5, and added a requirement to promote deploy-vs-reindex
  knowledge into durable operational docs before archive.

### Patterns to Remember

- A live preview failure is not automatically a product defect. For preview
  deployments, distinguish app/runtime bugs from host/proxy/auth protection
  mismatches before creating remediation work.
- When a new snagging plan sits alongside an active migration plan, add
  bidirectional links and an explicit boundary note so future sessions do not
  merge the scopes back together.

---

## Session 2026-03-07 — oak-preview MCP Smoke Pass

### What Was Done

- Exercised the live `oak-preview` HTTP MCP server through the published MCP
  tool surface after reading the shipped tool descriptors and running the
  required `get-curriculum-model` orientation call first.
- Verified representative happy-path workflows:
  browse (`browse-curriculum`), semantic search (`search`), cross-scope topic
  exploration (`explore-topic`), typed fetch (`fetch` for lesson/unit/subject/
  thread), lesson planning detail tools (`get-lessons-summary`,
  `get-lessons-transcript`, `get-lessons-quiz`, `get-lessons-assets`),
  progression tools (`get-threads-units`, `get-thread-progressions`,
  `get-prerequisite-graph`), and metadata tools (`get-rate-limit`,
  `get-changelog-latest`).
- Verified negative paths: `fetch` returns a clear 404 for a missing lesson and
  `search` returns a specific validation error when `suggest` is called without
  the required `subject` or `keyStage` context.
- Found one functional environment issue: `download-asset` returns a signed URL,
  but the preview deployment currently responds `401` to that URL when accessed
  directly via `curl -I`, so the preview link is not publicly usable end-to-end.

### Patterns to Remember

- For Oak MCP servers, read the tool descriptors first and always call
  `get-curriculum-model` before touching the curriculum tools; the server
  guidance is explicit and the descriptors encode important workflow contracts.
- `download-asset` can appear healthy at the MCP level while still failing at
  the browser/download layer if the preview deployment is protected by preview
  auth/SSO. End-to-end verification needs one extra direct HTTP check against
  the returned signed URL, not just a successful tool response.

---

## Session 2026-03-07 — Consolidate Docs After MCP Apps Plan Activation

### What Was Done

- Ran the consolidate-docs workflow against the MCP Apps migration doc changes.
- Verified the new active plan is wired into the collection README, active
  index, roadmap, and specialist reviewer guidance.
- Checked Claude-side memory and recent Claude-side plans; found no additional
  MCP Apps migration content that had not already been promoted into canonical
  repo docs.
- Fixed one live distilled-memory drift: the MCP Apps entry still pointed at a
  non-existent research path and the old `ext-apps` version.

### Patterns to Remember

- When a duplicate research file is deleted, do a direct reference sweep even if
  the canonical collection docs already look correct; memory files often keep
  stale paths longer than plans do.
- Platform-side plans are worth sampling during consolidation, but if their
  content has already been promoted into canonical repo plans, note that and do
  not duplicate it.

---

## Session 2026-03-07 — Legacy STDIO Workspace Rule

### What Was Done

- Added a canonical agent rule at `.agent/rules/legacy-stdio-workspace.md` and
  thin Cursor/Claude wrappers so agents reject new feature work in
  `apps/oak-curriculum-mcp-stdio`.
- The rule points future work at
  `apps/oak-curriculum-mcp-streamable-http` and shared SDK layers instead.

### Patterns to Remember

- For repo-wide agent guidance, prefer the three-layer practice pattern:
  canonical rule in `.agent/rules/`, thin platform wrappers in
  `.cursor/rules/` and `.claude/rules/`.
- Claude path-scoped rule wrappers need explicit `paths` frontmatter; a plain
  "Read and follow ..." file is not a proper Layer 2 trigger.
- When a rule exists only to protect a transitional legacy workspace, state the
  cleanup condition explicitly: delete the rule when the workspace is deleted.

---

## Session 2026-03-07 — Consolidate Docs Queue Consistency

### What Was Done

- Ran `/jc-consolidate-docs` against the current SDK/MCP planning surfaces after
  the oak-preview snagging plan rewrite and the output-schema plan refresh.
- Aligned the live queue semantics across:
  ...
  - `.agent/plans/sdk-and-mcp-enhancements/active/oak-preview-mcp-snagging.execution.plan.md`
- Promoted both current plans into the collection hub and roadmap so the
  collection no longer implies the snagging plan is the only visible current
  executable.
- Checked Claude-side memory and sampled Cursor-side plans again; found no new
  durable content requiring promotion into canonical repo docs.
- Verified the practice box is still empty (`.gitkeep` only).

### Patterns to Remember

- If a plan becomes partially complete while still living in `current/`, update
  all three layers together: the plan body/frontmatter, `current/README.md`, and
  the collection hub/roadmap text that describes the queue.
- "Current", "active", and "future" vocabulary must be consistent across the
  collection. A standalone plan is not really standalone if the parent indexes
  still describe it with stale lifecycle language.
- During consolidation, platform-side memory and plan checks are often
  confirmatory rather than generative; record that the check happened so the
  next session does not repeat it blindly.
...
| `.agent/directives/AGENT.md` | 151 | 200 | OK |
| `.agent/directives/principles.md` | 136 | 200 | OK |
| `.agent/directives/testing-strategy.md` | 393 | 400 | OK |
| `.agent/directives/schema-first-execution.md` | 39 | 200 | OK |
| `docs/governance/typescript-practice.md` | 113 | 200 | OK |
| `docs/governance/development-practice.md` | 109 | 200 | OK |
| `docs/operations/troubleshooting.md` | 166 | 200 | OK |
| `CONTRIBUTING.md` | 409 | 400 | OVER (+9) |
| `.agent/memory/distilled.md` | 186 | 200 | OK |
| `.agent/practice-core/practice.md` | 212 | 240 | OK |
| `.agent/practice-core/practice-lineage.md` | 321 | 320 | OVER (+1) |
| `.agent/practice-core/practice-bootstrap.md` | 430 | 400 | OVER (+30) |
...
## Session 2026-03-07 — Consolidate Docs After Snagging Promotion

### What Was Done

- Ran `/jc-consolidate-docs` after promoting the oak-preview snagging work from
  `current/` to `active/`.
- Swept plans, prompts, and canonical memory for stale references to
  `current/oak-preview-mcp-snagging.execution.plan.md`; only the live napkin
  still pointed at the deleted path.
- Updated the live napkin to reference the active snagging entry point instead.
- Re-checked Claude-side memory and the relevant consolidation experience note;
  found no additional durable MCP Apps or snagging guidance that had not
  already been promoted into canonical repo docs.
- Verified the practice inbox is still empty and found no new code pattern that
  met the admission barrier.

### Patterns to Remember

- After promoting a plan from `current/` to `active/`, sweep the live napkin as
  well as plans and prompts; memory files often keep the last stale path.
- Platform-side memory checks during consolidation are often negative evidence;
  still record that they were checked so later sessions do not repeat the same
  verification work.
