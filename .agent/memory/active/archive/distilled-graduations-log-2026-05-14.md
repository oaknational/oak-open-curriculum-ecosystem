...
appended_passes:
  ...
  - appended_at: 2026-05-24
    appended_by: Shaded Silencing Dusk / codex / GPT-5 / 019e59
    sections_added:
      - 'Backfill rotation 2026-05-24 — verified distilled homes'
    reason: 'Phase 3 of the memory-surface critical-drain plan pruned distilled entries whose permanent homes were verified before removal. Substance lives at PDR-061, PDR-067, PDR-068, PDR-069, metacognition.md, per-user-memory-is-a-buffer.md, agents-default-no-gender.md, and test-coverage-review-lens.md.'
  - appended_at: 2026-05-24
    appended_by: Sylvan Sprouting Petal / codex / GPT-5 / 019e59
    sections_added:
      - 'Backfill rotation 2026-05-24 — verified distilled homes'
...
Phase 3 of the 2026-05-24 memory-surface critical-drain plan removed live
`distilled.md` entries only after verifying that their durable homes already
carry the substance. This was a no-duplication prune, not a compression pass.

Verified homes:

- *Surface classification routes fitness response — buffers are not memory*
  → [PDR-067](../../practice-core/decision-records/PDR-067-surface-classification-for-fitness-response.md).
...
- *Doctrine-first vs first-principles is cognitive-approach diversity*
  → [PDR-069](../../practice-core/decision-records/PDR-069-doctrine-first-vs-first-principles-diversity.md).
- *Metacognition has two modes — retrospective and generative*
  → [metacognition.md](../../directives/metacognition.md).
- *Platform-specific per-user memory is a buffer, not a personal store*
  → [per-user-memory-is-a-buffer.md](../../rules/per-user-memory-is-a-buffer.md).
- *Agents have no gender unless they self-declare*
  → [PDR-061](../../practice-core/decision-records/PDR-061-agent-pronoun-default-and-conduct-correction-graduation.md)
  and [agents-default-no-gender.md](../../rules/agents-default-no-gender.md).
- *Test-design lens precedes shared-state hunt on flaky suites*
  → [test-coverage-review-lens.md](../patterns/test-coverage-review-lens.md).
- *"Decide at write time" deferrals are unmade load-bearing decisions*
  → [deferred-at-write-time-is-unmade-load-bearing-decision.md](../patterns/deferred-at-write-time-is-unmade-load-bearing-decision.md).
- *Coordinator handoff has two distinct moments*
  ...
  and `start-right-team` §"Coordinator Handoff (Two Moments)".
- *Verdict reasoning must be substance, not citation*
  → [citation-as-reasoning-at-moment-of-verdict.md](../patterns/citation-as-reasoning-at-moment-of-verdict.md).
- *Vocabulary discipline applies when absorbing inputs*
  → [dogma-vocabulary-closes-inquiry.md](../patterns/dogma-vocabulary-closes-inquiry.md).
- *Per-workspace inherited-tree gates are the default for workspace-scoped dirt*
  → `start-right-team` §"Running the gates".
- *2026-05-13 Three-Napkin Synthesis pointer*
  ...
  [passive-guidance-loses-to-artefact-gravity.md](../patterns/passive-guidance-loses-to-artefact-gravity.md),
  [PDR-029](../../practice-core/decision-records/PDR-029-perturbation-mechanism-bundle.md),
  and `pending-graduations.md` F2-F10 routes.
- *Sidebar protocol now has the default-action shape*
  → [inter-agent-sidebar-with-default-action.md](../patterns/inter-agent-sidebar-with-default-action.md),
  amended 2026-05-24 with the Mistbound-Lanternlit and Mistbound-Vining
  worked instances.
- *Read `--help` first on any agent-tools CLI before first session use*
  → [agent-tools README](../../../agent-tools/README.md) § CLI Norms,
  especially the every-depth `--help` contract and the rule that CLI help
  text is the canonical discovery surface under in-flight refactors.
- *Use `--body-file <path>` for non-trivial comms bodies*
  → [agent-tools README](../../../agent-tools/README.md) § Comms body input,
  including the mutually exclusive `--body` / `--body-file` flags, shell
  substitution hazard, and literal file-read cure.
- *Supertest tests are integration by classification, not e2e*
  → retired as superseded, not carried forward as doctrine. Current homes are
  [testing-patterns.md](../../../docs/engineering/testing-patterns.md)
  § Test File Classification and [testing-strategy.md](../../directives/testing-strategy.md)
  § Development Workflow: Supertest is the HTTP-level E2E tool because its
  in-process server drives loopback socket IO.
- *`pnpm check` cleanliness belongs in session-handoff*
  → [session-handoff/SKILL-CANONICAL.md](../../skills/session-handoff/SKILL-CANONICAL.md)
  step 11, which requires a clean `pnpm check` for sole-contributor and
  handoff-owner completion and names the comms singleton discipline.
- *Hook-bypass equivalence: `--no-verify` is broader than one flag*
  → [no-verify-requires-fresh-authorisation.md](../../rules/no-verify-requires-fresh-authorisation.md)
  § Rule, covering `--no-verify`, env-var hook skips, hooksPath overrides,
  `GIT_HOOKS_PATH`, and equivalent hook-bypass mechanisms.
- *Curator pass logs are metadata, not substance stores*
  → [PDR-081](../../practice-core/decision-records/PDR-081-curator-role-and-substrate-care-lane.md)
  § Per-pass log contract and
  [curator-pass/SKILL-CANONICAL.md](../../skills/curator-pass/SKILL-CANONICAL.md)
  § Per-Pass Log Contract.
- *Archive is after curation, not instead of curation*
  → [knowledge-preservation-over-fitness-warnings.md](../../rules/knowledge-preservation-over-fitness-warnings.md)
  § The Invariant and § Scope, including archive-after-processing for buffers.
- *Verification asks need artefacts, not trust confirmations*
  → [verify-dont-trust.md](../../rules/verify-dont-trust.md)
  § Rule, § Apply This Before, and § What To Do Instead.
- *Reviewer passes need critical absorption, not blind acceptance*
  → [verify-dont-trust.md](../../rules/verify-dont-trust.md) § Rule and
  § Worked Instances, which now name reviewer output as evidence to test, not a
  substrate pointer to trust.
- *Compaction-boundary handoff differs from session-close handoff*
  → [handoff-messages-self-contained.md](../../rules/handoff-messages-self-contained.md)
  § When the receiver IS your future self, which now names runtime monitor,
  cron, watcher, and loop-instruction verification after compaction.
- *Fresh substrate still needs absorption time*
  → [recursion-of-doctrine-under-team-cadence-speed.md](../patterns/recursion-of-doctrine-under-team-cadence-speed.md)
  for writer-side absorption latency under team cadence,
  [substrate-pointer-read-as-current-state.md](../patterns/substrate-pointer-read-as-current-state.md)
  for reader-side stale substrate reads, and
  [PDR-080](../../../practice-core/decision-records/PDR-080-coordination-event-absorption-is-signal-driven.md)
  for signal-driven absorption-before-rotation.
- *Transport shape determines E2E classification*
  → [testing-patterns.md](../../../../docs/engineering/testing-patterns.md)
  § Test File Classification and § MCP Transport Layer Testing, plus
  [testing-strategy.md](../../../directives/testing-strategy.md)
  § Development Workflow for the HTTP-level E2E and MCP protocol E2E tool
  boundaries.
- *Phased gated activation beats big-bang integration*,
  ...
  *Multi-agent friction is an infrastructure problem, not a workaround
  occasion*, and *Principle violations get reverted, not deferred*
...
- *Bare timer races leak unless cleaned up*
  → [guarded-fire-and-forget-cleanup.md](../patterns/guarded-fire-and-forget-cleanup.md)
  § Pattern, § Correct Pattern, and § Why; the pass also corrected
  `apps/oak-curriculum-mcp-streamable-http/src/mcp-handler.ts` so the
  proven cleanup path clears the timeout in `finally`.
- *Sonar MCP `show_security_hotspot.comments` is NOT the rationale carrier*
  → [sonarqube-mcp-instructions.md](../../rules/sonarqube-mcp-instructions.md)
  § Hotspot review, which now names REST
  `/api/hotspots/show?hotspot=<hotspot-key>` changelog inspection as the
  verification surface after rationale-bearing hotspot status changes.
- *Acceptance value-proxies must compare against independent ground-truth
  measures*
  → [testing-patterns.md](../../../../docs/engineering/testing-patterns.md)
  § Acceptance Value-Proxies and
  [testing-strategy.md](../../../directives/testing-strategy.md)
  § Acceptance Value-Proxies.
- *Revision-tranche verification: re-read original defect location*
  → [verify-dont-trust.md](../../rules/verify-dont-trust.md) § Rule,
  § Apply This Before, § Anti-Patterns, and § Worked Instances.
- *Plan-as-source-of-truth supersedes coordinator brief on workspace/path/key
  specifics*
  → [agent-collaboration-channels.md](../../executive/agent-collaboration-channels.md)
  § Repeated Routing Pitfalls.
- *Read-only support pattern*
  → [agent-collaboration-channels.md](../../executive/agent-collaboration-channels.md)
  § Read-Only Support Assignments.
- *Docker MCP tool listings are the namespace check*
  → [quality-tooling-mcp-coupling.md](../../../../docs/engineering/quality-tooling-mcp-coupling.md)
  § Codex / Docker MCP fallback.
- *assumptions-expert on a PDR-SET surfaces cross-PDR coupling defects*
  → [invoke-assumptions-expert.md](../../rules/invoke-assumptions-expert.md)
  § Trigger Conditions and § Invocation.
- *Re-ground git state immediately before staging on any long session*
  → [stage-by-explicit-pathspec.md](../../rules/stage-by-explicit-pathspec.md)
  § Pre-Stage Re-Ground for Long Sessions.
- *Hyphenate `PR-#NNN` when referencing issues in prose*
  → [development-practice.md](../../../../docs/governance/development-practice.md)
  § Documentation Practice.
- *Catalogue-not-block resolves advisory-fitness tension*
  → [commit/SKILL-CANONICAL.md](../../skills/commit/SKILL-CANONICAL.md)
...
- *Owner-given unblock hints during selective pause need explicit scope*
  ...
  § Coordinated-session comms cadence, especially the coordinator-vs-worker
  selective-pause clause.
- *Destructive incidents cure through structure, not through more care* and
  *Consolidation surfaces are first-class observability infrastructure*
  → [user-collaboration.md](../../../directives/user-collaboration.md)
  § Owner Working Style. These were the final two owner-profile observations
  from the 2026-05-22 `.remember/recent.md` promotion set; the other six had
  already graduated to the same section.
- *DECISION-COMPLETE is the readiness gate, not paperwork after execution*
  → [PDR-018](../../../practice-core/decision-records/PDR-018-planning-discipline.md)
  § DECISION-COMPLETE is the readiness gate.
- *Verify vendor call shapes against installed-or-published docs at
  plan-author time*
  → [verify-vendor-call-shapes-at-plan-author-time.md](../../rules/verify-vendor-call-shapes-at-plan-author-time.md)
  § Rule and § Why This Rule Exists, with
  [PDR-018](../../../practice-core/decision-records/PDR-018-planning-discipline.md)
  carrying the parent planning-discipline doctrine.
- *PR closeout has two distinct evidence loops*, *PR title/body are an active
  review surface*, *For planning PRs, report two verdicts separately*, and
  *Remote metadata transitions are part of state handoff*
  → [agent-collaboration.md](../../../directives/agent-collaboration.md)
  § PR Closeout Discipline, with
  [agent-collaboration-channels.md](../../executive/agent-collaboration-channels.md)
  § Repeated Routing Pitfalls carrying the routing-card reminders.
