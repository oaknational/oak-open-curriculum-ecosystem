...
## 2026-05-04 (Ferny Spreading Petal, `d0d13f`) — PDR-046 layered-knowledge-processing drafted; Layer-2 second pass underway

Layer-2 second pass picked up the Fronded handoff. Highest-leverage
trio identified by the previous session: PDR-046 (layered
processing methodology), PDR-047 (rule-applies-always), PDR-048
(insight-capture-at-moment-of-occurrence). PDR-046 drafted first
per the methodology's self-application: the rule that produced
the pass is the first rule the pass produces.

PDR-046 shape: three moves mirroring the three failure modes
named in the napkin's previous-session "Layered processing"
entry. Move 1 — process layers bottom-up; never interrupt a
lower layer to remediate an upper layer. Move 2 — suspend
in-process form-keeping on the active layer. Move 3 — address
a layer's residual fitness pressure by graduating substance
upward, not by compression. Composition section names the
methodology in its full shape: *preserve first, restructure
second; fitness measures the resting system, never the
in-process system; the staircase is walked one layer at a time,
bottom-up, until knowledge has reached its durable home.*

Adopter-scope decision (PDR-019 genotype/phenotype test):
distinct-enough-to-warrant-new-PDR rather than PDR-026
amendment or PDR-038 amendment. Reason: the rule governs
multi-layer knowledge processing in *any* Practice-bearing
repo's staircase, not just deferral honesty (PDR-026's scope)
and not just enforcement-doctrine maturity (PDR-038's scope).
The host-side `consolidate-docs.md § Learning Preservation
Overrides Fitness Pressure` extension is a separate host-local
follow-up — PDR-046 sits *above* the per-write rule as the
orchestration discipline; the per-write rule itself is unchanged.

Practice Core diff bundle prepared for owner review:
- `decision-records/PDR-046-layered-knowledge-processing.md` (new).
- `decision-records/README.md` (index row added).
- `CHANGELOG.md` (graduation entry).
- `pending-graduations.md` (status marked
  `graduated 2026-05-04 to PDR-046`).

Per PDR-003: drafting complete; owner reviews before staging.
PDR-047 and PDR-048 sequenced behind PDR-046 review per the
brief's explicit instruction.

### Worked-instance lessons from drafting PDR-046

- **Layer-1 pre-processing made Layer-2 graduation cheap.** The
  source napkin entry already named the principle (Fronded
  captured it verbatim from owner mid-pass at moment of
  occurrence), the four-layer stack, the three failure modes,
  and two of the three named cures. PDR-046 drafting was almost
  entirely a structural lift from the napkin entry into the PDR
  shape. This is the methodology validating itself: insight
  captured at the moment of occurrence by the previous session
  was reusable verbatim by the next session at full fidelity.
- **The PDR shape forces the rationale to surface.** The
  alternatives section ("apply the per-write rule recursively
  per layer and stop there" vs "treat fitness diagnostics as the
  primary signal and re-shape work around them") only became
  visible during drafting — neither was named in the napkin
  entry. Writing the PDR forces the alternatives into view; the
  napkin entry didn't have to. This is structural complementarity
  between the two surfaces, not redundancy.
- **Cross-Core PDR↔PDR connective tissue paid back.** Six
  Related entries in PDR-046 (PDR-014, PDR-022, PDR-026, PDR-028,
  PDR-029, PDR-038) plus PDR-043 (rush-impulse cues) plus
  PDR-045 (companion deliverable). Each link is structural, not
  decorative; the reader can navigate from any one to the
  composition picture. The portability constraint allowed all of
  these (cross-Core references are explicitly preserved per the
  decision-records/README.md portability section).

### Open follow-up (sequenced)

- `.agent/commands/consolidate-docs.md § Learning Preservation
  Overrides Fitness Pressure` host-local extension — add a
  pointer from the per-write rule up to PDR-046 as the
  orchestration rule the per-write rule composes with. This
  is host-local follow-up work; the PDR itself does not
  require it. Sequenced behind PDR-046 owner review and
  landing.
