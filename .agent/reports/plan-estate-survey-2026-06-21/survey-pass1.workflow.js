...
const HOLISTIC_SCHEMA = {
  ...
  properties: {
    ...
    purpose: { type: 'string' },
    substance_class: { type: 'string', enum: ['good', 'mixed', 'bad', 'speculative'] },
    ...
    salvage_value: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['idea', 'class', 'file_line'], properties: { idea: { type: 'string' }, class: { type: 'string', enum: ['good', 'speculative', 'bad'] }, file_line: { type: 'string' } } } },
...
PLUS the owner's SUBSTANCE re-aim, curated at the IDEA level (a single plan can hold good, speculative, AND bad ideas at once, so per-plan verdicts are too coarse): substance_class — a per-plan SUMMARY for triage only: good | mixed | bad | speculative — with substance_rationale citing file:line; content_quality — strong | adequate | weak | empty — is what is IN the plan good, versus merely present and V0-conformant — with content_quality_note citing file:line; and salvage_value — the LOAD-BEARING idea-granular inventory: a list of EVERY constituent idea or intent in the plan, each as { idea: a one-line statement, class: good | speculative | bad, file_line }, so the consolidation can extract and re-home every good and speculative idea and the no-loss audit can verify the bad set holds nothing of value. Use an empty list only for a genuinely empty plan.\``
