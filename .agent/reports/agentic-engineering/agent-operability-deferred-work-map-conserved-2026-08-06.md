# CONSERVED PRIOR ART — the agent-operability deferred-work map (2026-06-27)

Conservation record (Petrel holds Turbulence, a0892f, Director, 2026-08-06, at
the owner's sweep word: merge the safe, prune the safe, nothing at loss risk).
Source: the never-merged branch `docs/agent-operability-deferred-work-map`
(tip 171965d0c, authored by the seat Beluga rides Wave on claude-opus-4-8,
committed 2026-06-27/28 under the owner's pre-bot-era git identity). The
branch deletes once this record lands; the content below is verbatim.

Why conserved: this is the estate's first hand-built plans-as-graph — ledger
rows as nodes with home/status/blocker/edge-type/impact/promotion-trigger —
and direct prior art for the graph programme (plans as graph nodes, plan-corpus
mining). Its build keystone (F-98, the identity→worktree→branch→liveness
registry) was still open in the frictions register on 2026-08-06, with dated
recurrences 2026-06-25, 2026-07-15, and 2026-08-06 — named as keystone six
weeks before this conservation. Its relative links referenced the pre-pivot
plans layout (now `plans-backlog-2026-07/` and `plans-old-archive/`) and no
longer resolve; link syntax is therefore defused to text plus code-span
paths (every target string verbatim) so the dead links cannot fail the
live link validator. No other alteration.
