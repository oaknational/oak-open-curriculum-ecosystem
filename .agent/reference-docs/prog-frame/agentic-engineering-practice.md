...
**Action**: Design and build the practice from first principles through real product work (the Oak Open Curriculum Ecosystem — SDK, MCP servers, semantic search). This started as an exploration of co-pilot style AI support and rapidly evolved into an agent-first workflow. As of February 2026, for at least the previous six months, every line of code, configuration, and documentation has been authored by agents, while one human provided direction, feedback, and continuous guardrail design. Every component was created to solve a concrete problem encountered during delivery: rules to prevent repeated mistakes, sub-agents to provide specialist review, plans to organise multi-step work, institutional memory to preserve learning across sessions, quality gates to enforce standards without exception, and platform bindings to make it all discoverable through use.

**Result**: A self-reinforcing system that is documented (ADR-119), mapped (`.agent/practice-core/practice.md`), and operational. The practice governs all work on the oak-open-curriculum-ecosystem repository. As the repository becomes public and Oak's product engineers begin working on it, the practice is the mechanism by which they will be enabled to deliver safely and at increased velocity using AI tools — it teaches itself to every new developer and AI agent that enters the codebase. The act of creating the practice, naming it, documenting it, and making it self-teaching *is* the act of enabling others. Agent self-assessment and recursive improvement of the Practice are first-class operational loops, not optional extras.
...
- **Method doctrine → enabling scaffolds**: not imposing a single mandatory formula for all teams, but providing high-signal patterns, tooling, and safeguards that teams can apply with judgement to their domain context
- **Process improvement → self-improving system**: not continuous improvement of existing processes but the creation of a system that continuously improves itself through interlocking feedback loops. The learning loop (mistakes → napkin → distilled learnings → rules → work) is one. The recursive self-improvement loop (architect improves agents → better agents produce better reviews → better reviews improve code → higher bar raises agent requirements) is another. The consolidation loop (common threads extracted → simpler system → easier consolidation) is a third. These feedback loops — both negative (error-correcting) and positive (capability-compounding) — are exactly what stabilise complex systems, and they are what make the practice a *practice* rather than a static framework
...
- **Directives and rules** (`.agent/directives/`) — 5 directive documents governing all work: AGENT.md (entry point), principles.md (operational rules), testing-strategy.md, schema-first-execution.md, metacognition.md. Operational rule reinforcements in `.agent/rules/`. Domain-specific architecture guidance in `docs/agent-guidance/` (e.g. semantic-search-architecture.md)
- **Practice Core** (`.agent/practice-core/`) — six Practice Core files: the plasmid trinity (practice.md system map, practice-lineage.md principles and exchange mechanism, practice-bootstrap.md annotated templates) plus entry points (README.md for humans, index.md for agents), changelog (CHANGELOG.md), and the Practice Box for incoming cross-repo material
...
- The oak-open-curriculum-ecosystem repository was built entirely under the practice's governance: a Curriculum SDK generated from the OpenAPI schema, two MCP servers (stdio + HTTP with OAuth), a semantic search system combining four ranking methods across seven data sources, core packages, and comprehensive documentation. Current repository scale is approximately **957,000 lines** (rounded to the nearest thousand, measured from tracked text files in February 2026), including product code, tests, configuration, and documentation.
