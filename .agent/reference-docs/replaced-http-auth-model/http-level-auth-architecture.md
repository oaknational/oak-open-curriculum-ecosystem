...
## Files Deleted (Now Restored in This Directory)

All deleted files have been restored from git history and are available in this directory for reference.

### Implementation

- **`mcp-auth.ts`** - Core HTTP middleware (originally `src/auth/mcp-auth/mcp-auth.ts`)
- **`mcp-auth-clerk.ts`** - Clerk integration (originally `src/auth/mcp-auth/mcp-auth-clerk.ts`)
- **`mcp-router.ts`** - Route registration (originally `src/mcp-router.ts`)

### Tests

- **`mcp-auth.unit.test.ts`** - Unit tests for core middleware
- **`mcp-auth-resource-validation.integration.test.ts`** - RFC 8707 validation tests
- **`auth-www-authenticate.integration.test.ts`** - WWW-Authenticate header tests
- **`clerk-auth-middleware.integration.test.ts`** - Clerk middleware tests
- **`mcp-router.integration.test.ts`** - Router integration tests

See **`README.md`** in this directory for complete file mapping and restoration details.
