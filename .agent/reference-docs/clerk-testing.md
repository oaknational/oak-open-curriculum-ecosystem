...
sdkScoped: 'false'
canonical: /docs/guides/development/testing/overview
sourceFile: /docs/guides/development/testing/overview.mdx
---
...
2. [Create a new session](/docs/reference/backend-api/tag/sessions/post/sessions){{ target: '_blank' }} for the user.
3. [Create a session token](/docs/reference/backend-api/tag/sessions/post/sessions/{session_id}/tokens){{ target: '_blank' }} using the session ID returned in the previous step.
...
```shell
POST https://REDACTED.clerk.accounts.dev/v1/client/sign_ups?__clerk_testing_token=REDACTED
```
