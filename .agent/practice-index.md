...
| [PDR-078](practice-core/decision-records/PDR-078-liveness-heartbeat-contract.md) — liveness-heartbeat contract (portable cadence + threshold + redundancy + exemption shape; Status Candidate) | Phenotype is **planned ADR-186 (comms-event-heartbeat-lifecycle-substrate)** — to be authored at Cycle 7 of [`post-m1-attestation-tidy-up.plan.md`](plans/agentic-engineering-enhancements/current/post-m1-attestation-tidy-up.plan.md). ADR-186 will record the repo-bound substrate (`event_type='heartbeat'` lifecycle-substrate event-type with `[HEARTBEAT]` watcher rendering, composing with the existing ADR-183 tag namespace). PDR-078 flips to Adopted at Cycle 8 concurrent with the thin SKILL pointer landing |
