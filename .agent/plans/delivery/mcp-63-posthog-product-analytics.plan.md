...
that private or transport-specific operation. In the
`@posthog/mcp@0.10.0` baseline, the automatic high-level wrapper
removes legitimate `context` and `conversation_id` tool-call arguments
before dispatch. Its low-level wrapper removes legal `nextCursor` and
top-level `_meta` when it reconstructs `tools/list` responses. Oak's
timeless requirement is protocol transparency, so neither wrapper is
accepted.
...
MCP-230 through MCP-238 PR stack. Draft PR #576 at
`aac01d12dd696e4e1831a683b41fde692be721ca` remains a superseded
source artefact until every replacement branch is pushed and
reviewable; it is not accepted implementation evidence. MCP-239 through
MCP-244 separately bound application configuration, deployment
declarations, runtime composition, resource registration, lifecycle,
protocol-equivalence, and built-artefact proof.
...
request identifier. When callbacks are wired, an existing delegate
callback runs first, request observation runs second, and the wrapper
consumer callback runs third. The observer delegates `start`, `close`,
session access, and protocol-version notification without changing
...
| Delegation | Run an existing delegate callback, then observation, then the wrapper consumer callback; preserve exact message, metadata, options, result/error identity, and send-promise identity |
...
most the fixed non-recursive signal defined above, and never changes or
annotates the MCP response. Projection and capture are bounded
synchronous work before delegation; they do not await analytics
delivery or replace the delegate's exact promise, including after
capture failure. Oak configures the package logger only with the fixed
content-free mapper. Isolated package-contract tests prove that
failures swallowed by the manual API reach that logger; mapper tests
and the controlled acceptance event in MCP-117 detect failure without
making vendor log content part of the contract.
...
Repository-safe proof completes MCP-63. MCP-117 separately owns the
controlled deployed-event and minimal-Person evidence; that evidence
does not block this repository delivery plan.
...
4. **Repeat-use identity is narrow, rotatable, and
   ...
   (`MCP-117`): the controlled event resolves one minimal Person with
   no Oak-supplied person properties.
...
9. **Controlled deployed acceptance is separately owned.** After
   repository completion, MCP-117 proves that one authenticated
   controlled tool call produces the expected event and minimal Person
   in the approved EU project, with no excluded property. This
   owner-held proof does not block MCP-63.

## Todos

- MCP-230 reconciles this transport-observer contract and ADR-218.
- MCP-231 corrects the provider-neutral analytics runtime contract.
- MCP-232 enforces the PostHog vendor-import boundary.
- MCP-233 adds the buildable PostHog adapter package boundary.
- MCP-234 adds destination-scoped actor pseudonyms.
- MCP-235 adds the closed PostHog event policy.
- MCP-236 observes the three supported transport operations.
- MCP-237 adds the closed product-analytics sink.
- MCP-238 composes the runtime and isolated package logger.
- MCP-239 validates selected application configuration.
- MCP-240 declares the application adapter and deployment contract.
- MCP-241 composes the application runtime and transport observer.
- MCP-242 observes authenticated resource reads.
- MCP-243 closes analytics through the shared process lifecycle.
- MCP-244 proves protocol equivalence and the built analytics path.
- MCP-117 separately records the controlled deployed acceptance
  evidence after repository completion.
