...
| `@sentry/profiling-node` adds native binary build cost to CI | Medium | Low | Pin version; add `'@sentry/profiling-node': true` to `allowBuilds` in `pnpm-workspace.yaml`; measure install time delta |
