...
| P0 | [monorepo-workspace-topology-adr-and-canonical-plan.plan.md](./monorepo-workspace-topology-adr-and-canonical-plan.plan.md) | Supersede ADR-108 with an all-workspace topology ADR: pipeline stages **S0–S6** (not monolithic “codegen”), three roles (primitive emitters / library authors / app consumers), stage×workspace matrix with multi-stage triage, ADR-154 thin leaves; consolidate plans then enforce |
...
| P1 | [search-cli-ingestion-pipeline-consolidation.plan.md](./search-cli-ingestion-pipeline-consolidation.plan.md) | Collapse the search-cli dual-pipeline ingestion (live-API + bulk) into one canonical schema-derived flow; eliminates the structural possibility of bulk-vs-API document divergence at write time. Paired with [bulk-schema-driven-code-generation](../../semantic-search/future/02-schema-authority-and-codegen/bulk-schema-driven-code-generation.md) which closes the bulk Zod-generator gap. |
