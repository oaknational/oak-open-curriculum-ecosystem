...
### Verified Findings (2026-04-15)

1. **Native wrapper scope**
   ...
   - **Source reading**: `wrapMcpServerWithSentry()` patches `connect`
     ...
     Tool and prompt results have dedicated result extraction; resource
     results do not.
   - **Oak-path spike**: running Oak's real `createMcpHandler` +
     `StreamableHTTPServerTransport` path with a wrapped `McpServer`
     produced 3 native `mcp.server` transactions for:
     `tools/call get-curriculum-model`,
     `prompts/get find-lessons`,
     `resources/read docs://oak/getting-started.md`.
   - **Patch-surface spike**: `tool` / `resource` / `prompt` changed
     identity after wrapping, but `registerTool` /
     `registerResource` / `registerPrompt` did not.
   - **Error-parity spike**: `registerTool('boom', ...)` and
     `tool('boom', ...)` both emitted a native
     `tools/call boom` transaction, but only the deprecated `tool()`
     path produced a captured exception event. The `registerTool()`
     path returned `CallToolResult { isError: true }` with no native
     exception capture.
   - **Conclusion**: native Sentry does instrument Oak's actual
     Streamable HTTP request path at the transport/request level for
     tools, prompts, and resources, but it does **not** wrap the
     handler registration path Oak actually uses.

2. **`sentry-mcp` export audit**

   - `wrapToolHandler`, `wrapResourceHandler`, and `wrapPromptHandler`
     are the runtime wrapper surface.
   - `createInMemoryMcpObservationRecorder` plus the
     `McpObservation*` / `MergedMcpObservation*` types are independent
     from native Sentry and currently serve only Oak fixture/test
     transcript capture.
   - **Conclusion**: the runtime wrappers and the recorder/types should
     be treated as separate retention decisions, not bundled together.

3. **Test infrastructure**

   - `createFakeHttpObservability()` and fixture mode in
     `http-observability.ts` use
     `createInMemoryMcpObservationRecorder()`.
   - `http-observability.unit.test.ts` asserts merged observations
     across wrapped tool/resource/prompt callbacks.
   - Characterisation tests
     `handlers-observability.integration.test.ts` and
     `register-resources-observability.integration.test.ts` exist to
     pin current wrapper usage.
   - **Conclusion**: recorder-backed test infrastructure is real
     migration work, but it is not evidence that runtime wrappers
     should survive. Any surviving test-only need should move app-local
     or into `@oaknational/sentry-node` fixture support.
...
4. **Call-site surface**
   ...
   - **Runtime**:
     ...
     `apps/oak-curriculum-mcp-streamable-http/src/register-prompts.ts`,
   ...
   - **Fixture/test-only**:
     `apps/oak-curriculum-mcp-streamable-http/src/test-helpers/observability-fakes.ts`,
     `apps/oak-curriculum-mcp-streamable-http/src/observability/http-observability.unit.test.ts`,
     `apps/oak-curriculum-mcp-streamable-http/src/handlers-observability.integration.test.ts`,
     `apps/oak-curriculum-mcp-streamable-http/src/register-resources-observability.integration.test.ts`
   - **Package-internal**:
     `packages/libs/sentry-mcp/src/*`,
     `packages/libs/sentry-mcp/src/wrappers.unit.test.ts`,
     `packages/libs/sentry-mcp/tests/capture-policy.typecheck.ts`,
     `packages/libs/sentry-mcp/README.md`,
     `packages/libs/sentry-mcp/package.json`
   - **Config-only**:
     `packages/core/oak-eslint/src/rules/boundary.ts`,
     `packages/core/oak-eslint/src/rules/lib-boundary.unit.test.ts`
   - **Conclusion**: "tests use it" describes the migration surface;
     it does not justify keeping the runtime package.

5. **Off-mode safety**

   - **Source reading**: the wrapper resolves `sendDefaultPii` from the
     active client, defaults `recordInputs` / `recordOutputs` to
     `false` when no client exists, early-returns from error capture if
     there is no client, and falls back to non-recording spans when
     tracing is disabled.
   - **No-init spike**: wrapped server + wrapped transport lifecycle
     completed without `Sentry.init()`. Tool/resource/prompt handlers
     still ran and the wrapper did not throw.
   - **Conclusion**: off mode is safe; the native wrapper degrades
     gracefully when the SDK is not initialised.

6. **`sendDefaultPii: false` interaction**

   - **Source reading**:
     `recordInputs` / `recordOutputs` default from `sendDefaultPii`.
     `sendDefaultPii: false` filters only network-PII span fields:
     `client.address`, `client.port`, and `mcp.resource.uri`.
   - **Spikes**:
     - Fake transport tool-call spike: with `sendDefaultPii: false`,
       no `mcp.request.argument.*` and no
       `mcp.tool.result.content`; with `true`, both appear.
     - Real Oak-path spike: with `sendDefaultPii: false`, native
       `mcp.server` transactions still recorded method names,
       tool/prompt names, request IDs, transport metadata, and protocol
       metadata, while omitting request arguments, prompt/tool payload
       details, and resource URI.
   - **Conclusion**: `sendDefaultPii: false` is compatible with native
     MCP tracing. It suppresses payload capture by default but does not
     disable the wrapper itself.

### Recommendation (based on verified evidence)

- **Recommendation: reduce, not remove immediately.**
- Do **not** retain the current runtime per-handler wrappers as the
  intended steady-state design. Native Sentry is broader and should be
  the target architecture.
- Do **not** remove `@oaknational/sentry-mcp` outright yet. Oak's real
  registration path uses `registerTool` / `registerResource` /
  `registerPrompt`, and native Sentry does not patch those methods.
  Deleting the runtime wrappers today would lose handler-level capture
  on the path Oak actually runs, especially for tool failures that the
  SDK converts into `CallToolResult { isError: true }` rather than
  JSON-RPC error responses. This is not hypothetical: the verified
  `registerTool('boom')` spike emitted a transaction but no captured
  exception event, while the deprecated `tool('boom')` path did.
- The only surface with evidence-based survival value is the
  in-memory recorder + observation types for fixture/test support, and
  even that currently looks like migration scaffolding rather than
  justification for a standalone shared runtime package.
- Practical reading of "what survives?": the runtime wrappers should be
  migrated away from once the native-wrapper adoption plan explicitly
  resolves the `register*` gap; the recorder/types should either move
  app-local or into fixture support elsewhere, then the package can be
  removed entirely.
