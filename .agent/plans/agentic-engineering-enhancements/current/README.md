...
| [large-corpus-analysis-runbook-build-and-prove.plan.md](large-corpus-analysis-runbook-build-and-prove.plan.md) | Adjacent: prove and home the Discovery/Surprises/Directed corpus-analysis method (design in the [report](../../../reports/agentic-engineering/large-corpus-analysis-runbook-design-2026-06-29.md)); blocked on a writeable, execution-authorised session for the first napkin Discovery run | QUEUED |
| [napkin-corpus-discovery-run.plan.md](napkin-corpus-discovery-run.plan.md) | The real, full napkin-corpus DISCOVERY RUN (the end): grain + longitudinal prompt refinement, a cheap pre-spend probe, a checkpointed ~13M owner-gated run, then the deterministic driver + discovery artefacts + conservation hand-off. Recall is the tuning means; conserved discovery (recurring + longitudinal) is the success. Aggregation math frozen. Supersedes v3-extraction-grain | REVIEWED — READY FOR EXECUTION (full run owner-gated) |
| [large-corpus-analysis-v3-extraction-grain.plan.md](large-corpus-analysis-v3-extraction-grain.plan.md) | Refining steps + probe + gated rerun; absorbed into the discovery-run plan | SUPERSEDED → napkin-corpus-discovery-run.plan.md |
| [corpus-analysis-conservation.plan.md](corpus-analysis-conservation.plan.md) | Conserve the v2 corpus-analysis discoveries: graduate the distilled buffer via consolidate-until-done (disposition ledger pre-staged — 4 already-covered, 4 amendments, 5 new patterns), author the `agentic-judgment-conserve-by-default` firing rule (PDR-122 invariant 2), and promote the tooling to the `oak-corpus-analysis` skill + agent-tools scripts | REVIEWED — READY FOR EXECUTION |
