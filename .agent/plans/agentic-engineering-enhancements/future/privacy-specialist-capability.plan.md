...
- `security-expert` includes privacy and data-minimisation concerns in its
...
- **`privacy-expert`** would provide read-only privacy-by-design and
...
- Narrow HTTP/API personal-data obligations and GDPR/UK GDPR boundary semantics
  (`web-api-gdpr-expert`)
- Narrow HTTP/API attack-surface hardening (`web-api-security-expert`)
...
- **`privacy-expert`** owns broad privacy-by-design, minimisation,
  ...
  **not** own exploitability triage, broad cyber posture, or narrow web/API
  GDPR details.
- **`security-expert`** owns the practical default security/privacy risk
  review and exploitability posture. It does **not** own broad
  privacy-governance and privacy-by-design doctrine.
- **`cyber-security-expert`** owns security posture, threat models, and
...
- **`web-api-gdpr-expert`** owns narrow personal-data obligations and
  ...
  the repo.
- **`docs-adr-expert`** owns documentation accuracy and ADR drift. It does
