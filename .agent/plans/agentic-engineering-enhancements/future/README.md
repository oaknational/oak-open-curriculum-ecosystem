...
| Later | [intent-to-commit-and-session-counter.plan.md](intent-to-commit-and-session-counter.plan.md) | Add advisory `intent_to_commit` field to active-claims (v1.3.0) with a registry-level `session_counter` as session-count TTL primitive; commit skill posts intent before staging, verifies staged ownership, and clears intent after commit | Evidence threshold met by lock-contention plus staged-bundle substitution, disappearance, and accretion; promote only when owner explicitly asks to implement the queue |
| Later | [joint-agent-decision-protocol.plan.md](joint-agent-decision-protocol.plan.md) | Add a discuss, decide, record, act protocol with explicit recorder/actor assignment for multi-agent decisions | Evidence threshold met by the observed 2-to-3 agent phase transition; promote when implementation capacity opens or owner explicitly directs |
| Later | [joint-agent-decision-protocol.plan.md](joint-agent-decision-protocol.plan.md) | Discuss → decide jointly → assign recorder → assign actor → close the loop. Extends WS3A decision-thread schema with `joint_decision` and `joint_decision_acknowledgement` entry kinds; adds a four-role vocabulary (discusser/decider/recorder/actor); session-handoff role-handoff sub-step; consolidate-docs § 7e joint-decision snapshot. Sits alongside WS3B sidebar; addresses the 3-agent-touching phase transition observed 2026-04-26 | Owner-direct promotion (close as of 2026-04-26 but not yet explicit "go"), OR a second 3-agent-touching session producing ≥ 2 clashes, OR WS3B sidebar lands and the first sidebar-resolved decision lacks recorder/actor assignment, OR a decision thread reaches `decision` kind without action within 3 sessions due to ambiguous role assignment |
