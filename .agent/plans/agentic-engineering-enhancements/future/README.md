...
| Later | [intent-to-commit-and-session-counter.plan.md](intent-to-commit-and-session-counter.plan.md) | Add advisory `intent_to_commit` field to active-claims (v1.2.0) with a registry-level `session_counter` as session-count TTL primitive; commit skill posts intent before staging and clears it after the commit lands; consolidate-docs § 7e gains an intent-snapshot report | Owner-direct promotion, OR a fourth lock-contention incident on any active branch, OR a second observed instance of an agent's staged set being wiped by parallel-commit churn (three contention events already observed on `feat/otel_sentry_enhancements` 2026-04-26) |
