...
| ↪️ MOVED | `hooks-portability.plan.md` → [`../../agent-tooling/future/`](../../agent-tooling/future/hooks-portability.plan.md) | — | (relocated 2026-05-05) |
| ↪️ MOVED | `cross-vendor-session-sidecars.plan.md` → [`../../agent-tooling/future/`](../../agent-tooling/future/cross-vendor-session-sidecars.plan.md) | — | (relocated 2026-05-05) |
...
| Later | [seam-map-plan-template-archetype.plan.md](seam-map-plan-template-archetype.plan.md) | Turn the owner-approved EEF seam-map planning lesson into a reusable plan-template component or archetype for non-linear deliverable DAGs | Owner schedules the focused authoring session, another non-linear feature plan needs the shape, or the planning specialist lane is ready to absorb it |
| ↪️ MOVED | `adapter-generation.plan.md` → [`../../agent-tooling/future/`](../../agent-tooling/future/adapter-generation.plan.md) | — | (relocated 2026-05-05) |
| ↪️ MOVED | `canonical-first-skill-pack-ingestion-tooling.plan.md` → [`../../agent-tooling/future/`](../../agent-tooling/future/canonical-first-skill-pack-ingestion-tooling.plan.md) | — | (relocated 2026-05-05) |
| ↪️ MOVED | `agent-classification-taxonomy.plan.md` → [`../../agent-tooling/future/`](../../agent-tooling/future/agent-classification-taxonomy.plan.md) | — | (relocated 2026-05-05) |
| ↪️ MOVED | `agent-infrastructure-coherence-audit.plan.md` → [`../../agent-tooling/future/`](../../agent-tooling/future/agent-infrastructure-coherence-audit.plan.md) | — | (relocated 2026-05-05) |
...
| Later | [external-skills-substrate-learning.plan.md](external-skills-substrate-learning.plan.md) | Convert the external skills substrate study into a Practice-fit candidate register and promote only the smallest behaviour-changing slices, starting with feedback-loop-first diagnosis and prototype expiry if review confirms they fill real gaps | Owner or coordinator confirms execution timing for the decided first slice, assumptions/proportionality review passes, existing homes for C1/C2 are checked, and required docs/ADR review is assigned before doctrine files are amended |
...
| Later | [team-handoff-routing-and-action-log-exploration.plan.md](team-handoff-routing-and-action-log-exploration.plan.md) | Explore team-scoped `session-handoff` routing, role/boundary closeout modes, shared team journals, and a distinct auditable action-log event stream | Owner chooses first slice, another team session repeats full-handoff redundancy, P8 attention/action-state needs audit-log semantics, or consolidation promotes the role-routing lesson |
...
| ↪️ MOVED | `intent-to-commit-and-session-counter.plan.md` → [`../../agent-tooling/future/`](../../agent-tooling/future/intent-to-commit-and-session-counter.plan.md) | — | (relocated 2026-05-05) |
| ↪️ MOVED | `joint-agent-decision-protocol.plan.md` → [`../../agent-tooling/future/`](../../agent-tooling/future/joint-agent-decision-protocol.plan.md) | — | (relocated 2026-05-05; implemented 2026-04-26 with WS3B sidebars/escalation) |
| Later | [memetic-immune-system-and-progressive-disclosure.plan.md](memetic-immune-system-and-progressive-disclosure.plan.md) | PDR-044 adaptive-immunity layer + practice trio additive activation + doctrine-scanner CLI consolidation + triggered rule loading pilot. Strategic roadmap; each scope item promotes independently | Companion `current/doctrine-enforcement-quick-wins.plan.md` lands all six workstreams green, AND a consolidation pass produces structured evidence the chosen workstream is the highest-leverage next step, AND owner direction confirms in scope |
| Later | [action-time-structural-interrupt-design-space.plan.md](action-time-structural-interrupt-design-space.plan.md) | General doctrine-traction gap: most of the 87 rules + PDRs + patterns are recall-dependent and lose to artefact gravity. Reconciles PDR-029 + PDR-044 + metacognition via a three-axis frame (firing × detection × response) and locates the empty quadrant — mechanical-fire + cognitive-detect + advisory-response for semantic pathogens at action time. Defers mechanism exploration to closure-pressure | Reconciliation (t1) confirmed + semantic-pathogen inventory (t2) exists + closure-pressure fold/sibling decision (t5) resolved + owner selects a bounded reversible slice |
| Later | [codex-exec-cli-deep-dive.plan.md](codex-exec-cli-deep-dive.plan.md) | Richer `pnpm agent-tools:codex-exec` surface (`run` with cross-platform timeout, `extract` with JSON-schema validation, `validate-brief`) deferred from ADR-180 because the simpler shipped surface (`last-message`) covered the immediate need without fighting complexity discipline | Second concrete consumer for at least one candidate subcommand + reshape strategy that respects 50-line function / 250-line file / complexity ≤8 limits + assumptions-expert review |
| Soon (owner-prioritised) | [worktree-per-agent-transition.plan.md](worktree-per-agent-transition.plan.md) | Move concurrent agents off one shared checkout onto one git worktree per agent, dissolving the shared-working-tree + shared-index coupling (F-83) every n≥2 window pays. Direction owner-resolved (adopt worktrees); sequencing owner-prioritised as infra "soon" | Cross-worktree coordination-home resolution complete and proven coherent (`resolveCoordinationHome`, WS-3 F-41) + owner scopes a bounded reversible worktree-lifecycle slice |
