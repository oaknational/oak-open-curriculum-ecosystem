...
| ~~Later~~ | ~~[codex-session-identity-plumbing.plan.md](codex-session-identity-plumbing.plan.md)~~ | ~~Make `CODEX_THREAD_ID`-derived identity the default across Codex thread registration, shared-state writes, legacy anonymous-row reporting, and any verified title/statusline/hook surface~~ | **Promoted 2026-04-28; completed at [`../archive/completed/codex-session-identity-plumbing.plan.md`](../archive/completed/codex-session-identity-plumbing.plan.md)** |
