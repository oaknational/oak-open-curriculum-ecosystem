...
| Later | [joint-agent-decision-protocol.plan.md](joint-agent-decision-protocol.plan.md) | Add a discuss, decide, record, act protocol with explicit recorder/actor assignment for multi-agent decisions; extends decision threads with `joint_decision` and acknowledgement entry kinds | Evidence threshold met by the observed 2-to-3 agent phase transition; promote when implementation capacity opens or owner explicitly directs |
