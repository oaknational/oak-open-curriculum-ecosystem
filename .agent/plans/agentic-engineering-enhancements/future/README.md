...
| ~~Next~~ | ~~[intent-to-commit-and-session-counter.plan.md](intent-to-commit-and-session-counter.plan.md)~~ | ~~Owner-directed layer beyond the completed `git:index/head` commit-window claim: add advisory `intent_to_commit`, a first-class commit queue, and exact staged-bundle ownership checks~~ | **Promoted to `current/` 2026-04-27** as [`intent-to-commit-queue.execution.plan.md`](../current/intent-to-commit-queue.execution.plan.md); `session_counter` remains future-only |
