...
| `.agent/memory/executive/memory-state-substrate-contracts.manifest.json` | 200 | `surfaces[].validator` command string includes `--events-dir .agent/state/collaboration/comms-events` | **update** — WS4 retargets the path argument to the new root, and reconciles against the post-WS3 CLI flag shape (the `--events-dir` flag itself is Class A WS3 territory; this row references it secondarily) | WS4 (after WS3 settles CLI flag) |
