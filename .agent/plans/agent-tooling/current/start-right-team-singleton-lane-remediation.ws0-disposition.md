...
| `.agent/memory/operational/collaboration-state-conventions.md` | (mixed — line-refs pending WS4 sweep) | Documents conventions including retired paths | **update — partial WS0** — WS0 records presence; WS4 produces line-by-line disposition during sweep | WS4 |
| `.agent/memory/executive/memory-state-substrate-contracts.md` | (mixed — line-refs pending WS4 sweep) | Substrate contracts referencing retired roots | **update — partial WS0** — see above | WS4 |
| `.agent/memory/executive/memory-state-substrate-contracts.manifest.json` | (mixed — line-refs pending WS4 sweep) | Manifest may still enumerate retired paths | **update — partial WS0** — produced alongside the .md | WS4 |
...
| Every stale comms-root surface has one disposition | Class A (source — update), Class B (doctrine — update), Class F (historical-evidence — preserved), Class G (plans — update-at-execution), Class H (test-fixture — keep). Bulk class F covers the 313 collaboration/ matches and 14 archived memory matches by category. |
