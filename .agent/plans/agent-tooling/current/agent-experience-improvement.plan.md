...
todos:
  ...
  - id: ws3-b1-shared-resolver
    content: "WS-3 B1: one shared `resolveCoordinationHome` replacing the per-command silent cwd-fallback finders. LANDED in three commits. b5408291d consolidated TWO duplicate `findCollaborationRepoRoot` copies (`cli-comms-send`, `tui/config`) plus the bare `process.cwd()` fallback (`cli-comms-validate`) into one resolver; reviewed by code-expert, architecture-expert-fred, test-expert. CORRECTED by c90150ffa after the owner re-posed the problem (proper-question lesson): the home is the PRIMARY checkout resolved via `git worktree list --porcelain` (first entry), not a cwd sentinel-walk — so an agent in ANY linked worktree resolves the ONE shared coordination home (the real F-41 fix, proven end-to-end from a real linked worktree), not its own local copy. The needless `resolveRootFromDir` extraction in `core/repo-root.ts` was reverted. A closeout adversarial scan (architecture-expert-wilma) then caught that the commit-queue TOPIC dispatcher (`bin/agent-tools-cli-topics.ts`) still defaulted repoRoot via `git rev-parse --show-toplevel` (the linked-worktree, not the primary) — the exact commit-queue case the F-41 register names; fixed in 4fd640089 by routing it through `resolveCoordinationHome`. F-41 now closed across comms AND commit-queue defaults. All commits (b5408291d, c90150ffa, 4fd640089) gate-green; proven end-to-end from a real linked worktree."
    status: completed
  - id: ws3-b2-default-home-resolution
    content: "WS-3 B2: DEFERRED 2026-06-21 (owner decision) to ../future/coordination-home-explicit-targeting-migration.plan.md. The proper-question reframe (the home is the PRIMARY checkout, git-resolved via `git worktree list`) dissolved the original 'refuse bare-relative + explicit --repo-root migration' framing. The remaining tail: the explicit-path write commands (claims open/close/heartbeat, comms inbox/watch, commit-queue) still `required(...)` their path options, so from a linked worktree a relative path resolves against cwd (the wrong copy). Cure: DEFAULT those options via `resolveCoordinationHome` when omitted, then migrate the estate's invocations (watcher rule, commit skill, start-right, ~a dozen tests) to OMIT the defaultable paths — non-breaking for the primary checkout, fixes the worktree case. Land as ONE coordinated change when a session can absorb it. (Note: the commit-queue TOPIC default was already corrected this session — see ws3-b1.)"
    status: deferred
