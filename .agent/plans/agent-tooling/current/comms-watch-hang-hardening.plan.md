...
todos:
  ...
  - id: c2-liveness-default-on
    content: "c2 (one TDD cycle, one commit): promote the existing watcher-heartbeat liveness surface from opt-in to default-on. The substrate already exists and is sound (cli-comms-watch.ts composeHeartbeatTick → writeWatcherHeartbeat, schema in watcher-heartbeat.ts, staleness classifier in watcher-staleness.ts with the 3× threshold and five-state result) but is dead in practice: it only arms when `--heartbeat-file` is passed, and the rule's canonical invocation does not pass it — so the 2026-06-10 stall was externally unclassifiable. Change: when `--heartbeat-file` is absent, derive the default path from the seen-file (`<seen-file>.heartbeat.json`); `--no-heartbeat` opts out. Tests describe: default invocation writes the heartbeat at the default path on each interval; the derived path lands next to the seen-file; explicit flag still wins. Doc sync in the same commit: the rule's canonical invocation section gains the staleness self-check consumers run at cycle boundaries (`collaboration-state` staleness check or stat the heartbeat file vs 3× interval) — note a hung process cannot self-report, so the consumer check is the detection path c1's loud death does not cover (e.g. a hang in a code path without a deadline)."
    status: pending
