...
**Last Updated**: 2026-07-15 (evening — seat closure)
**Status**: 🟡 UNSEATED (item 1 COMPLETE and MERGED; items 2–8 returned to the Director for re-routing)
...
> items 2–8 and the follow-on register returned to the Director via comms event `ab047eef`.
> **Item-8 evidence base (five documented gaps)**: `comms watch --exclude-tag` missing (remit);
> `--only-directed` missing (Draco/Tuna 16:33–34Z + this seat's interim awk block filter,
> proven in-session when heartbeat wakes expired a commit-queue intent mid-ceremony);
> watcher-heartbeat schema is CLOSED — extra keys (e.g. `mode`) make the file unreadable to
> `assert-watcher-live`; commit-queue guard matches bare `index/head` only (Ceres 16:46Z);
> no `claims` intent-rewrite verb (Aurora 19:18Z). Follow-ons standing: EntryArgs
> discriminated-union reshape; ~215-site conditional-assertion estate (lint-rule cure
> candidate); SDK `unwrapOk` → `unwrap` consolidation.
