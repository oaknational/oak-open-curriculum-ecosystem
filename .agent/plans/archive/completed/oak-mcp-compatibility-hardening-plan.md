...
- Generate a shared translation module during `pnpm type-gen` that:
  ...
  - Is implemented through TDD with pure functions and unit tests that exercise every conversion path; follow the testing workflow in `.agent/directives/testing-strategy.md`.
...
- [x] Adjust consuming workspaces (`oak-curriculum-mcp-stdio`, `oak-curriculum-mcp-streamable-http`, `oak-search-cli`) to import from the published SDK surface rather than packages/.../src, verifying `package.json` dependencies point to the workspace package.
