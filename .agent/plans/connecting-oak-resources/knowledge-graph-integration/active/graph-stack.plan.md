...
todos:
  ...
  - id: ws1-graph-core-scaffold
    content: "WS1.1: Scaffold packages/core/graph-core workspace. Files mirror packages/core/result/ with two divergences (multi-entry tsup + sub-path exports). Acceptance has eleven items (a)–(k). (a) Register `packages/core/graph-core` in pnpm-workspace.yaml `packages:` list (explicit, not wildcard). (b) Register `packages/core/graph-core` in knip.config.ts `workspaces` map with `project: ['src/**/*.ts']` (existing per-workspace convention; absent registration produces a repo-wide knip failure at pre-commit). (c) package.json: @oaknational/graph-core, type:module, exports map pre-declares six entries — `.` plus `./term`, `./dataset`, `./jsonld`, `./canon`, `./vocab` — each pointing to dist/<module>/index.{js,d.ts} + development:src/<module>/index.ts. Scripts mirror result/ (build: `tsup && tsc --emitDeclarationOnly --project tsconfig.build.json`; clean; lint; lint:fix; type-check; test; test:watch). (d) tsup.config.ts uses `createLibConfig({ entry: { index: 'src/index.ts', 'term/index': 'src/term/index.ts', 'dataset/index': ..., 'jsonld/index': ..., 'canon/index': ..., 'vocab/index': ... } })` — NOT the 3-line re-export form. **Do NOT pass `dts` to createLibConfig**: declarations are produced exclusively by the `tsc --emitDeclarationOnly` step in the build script; setting `dts` on tsup would double-emit and collide. (e) tsconfig.json (extends ../../../tsconfig.base.json; includes src/**/*.ts, **/*.test.ts, *.config.ts). tsconfig.build.json (declaration: true, declarationMap: true, emitDeclarationOnly: true, rootDir: ./src, outDir: ./dist; excludes test files) — tsc emits .d.ts mirroring src tree so dist/term/index.d.ts etc resolve. tsconfig.lint.json (extends ./tsconfig.json, includes src/**/*.ts + **/*.test.ts + *.config.ts; pointed at by eslint.config.ts wsTsProject variable). All three tsconfig files required; absent tsconfig.lint.json breaks the eslint project resolution. (f) vitest.config.ts is the 3-line re-export of `baseTestConfig` (single-entry test config is correct; vitest.config.base.ts sets `passWithNoTests: true` so zero tests pass cleanly). (g) eslint.config.ts mirrors result/ with `coreBoundaryRules` on src/**/*.ts (ADR-154 framework-vs-consumer enforcement: prohibits imports from packages/libs/** and packages/sdks/**). (h) src/index.ts and all five src/<module>/index.ts files contain `export {};` (empty barrel + stubs). (i) README.md describes substrate intent + RDF 1.2-native stance; cites ADR-173 and ADR-179 by number only — no plan names, no increment numbers, no versioned roadmap. (j) Zero tests in WS1.1 (test-expert: atomic-landing invariant means no test without product code; vitest wiring verified by WS1.2's first real test). ADR-179 enforcement at WS1.1 is structurally trivial via the empty barrel; reviewer-enforced at each export boundary in WS1.2 onward (no transport-shaped types in exports). (k) Stage `pnpm-lock.yaml` alongside the scaffold files — adding a workspace updates the lockfile and must land in the same commit. 'Tree green' matches `.husky/pre-commit` exactly: format-check + markdownlint + knip + depcruise pass repo-wide AND `turbo run type-check lint test` passes across all workspaces (the new workspace's own type-check/lint/test exit 0; pre-existing workspaces unchanged). tsup emits dist/{index,term/index,dataset/index,jsonld/index,canon/index,vocab/index}.js; tsc emits matching .d.ts files. One commit, full commit-queue protocol from active-claims read forward. Reviewer flag for the WS1.1 PR: config-expert (focused: tsup multi-entry × tsconfig.build.json rootDir/outDir interaction × `dts` omission) is the single highest-leverage check."
    status: pending
    sub_increment: 1a
...
|---|---|---|---|
| 1 | **Foundation** (this plan) | `graph-core`, minimal `graph-ingest` (jsonld-compatible mode + generic Turtle/SKOS parsing), minimal `graph-project` (property-graph + adjacency), `graph-corpus-sdk` scaffold + Oak Curriculum Ontology Threads graph as first attached corpus | **ACTIVE** — Inc.1a WS1.1 next |
