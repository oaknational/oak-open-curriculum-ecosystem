...
sibling_plans:
  - "../future/oak-kg-threads-surface.plan.md"
...
todos:
  ...
  - id: ws2-source-mapping
    content: "WS2.3 (scope amended 2026-05-23 per pre-execution reviewer convergence): SourceMapping primitives — JSON Pointer (RFC 6901) for JSON-LD-shaped inputs; TurtleSourceLocation (line/column) for Turtle inputs; wrapped in discriminated `SourceLocation` union; sourceMap keyed by stable `quadKey()` canonical Quad serialisation. The original 'JSON Pointer for both' framing was a convergent type-expert + test-expert BLOCK (Turtle isn't JSON; applying RFC 6901 fabricates a synthetic JSON wrapper referencing an internal model artefact rather than the actual source — a type-level lie). The `quadKey()` string-keyed SourceMap cures a CRITICAL type-expert finding that `Map<Quad, SourceLocation>` keyed by object reference is unsafe under graph-core's structural-equality deduplication — two structurally-equal Quads at different construction sites would silently miss each other's source paths through a reference-keyed map. **LANDED at commit `6cc7b339` (2026-05-23, Sparking Melting Magma)** — new `./source-path` sub-path barrel + 32 unit tests covering jsonPointer validator (RFC 6901 escape rules), parseSegments/serialiseSegments round-trip, quadKey canonical serialisation across structurally-equal Quads + language-tagged Literal + datatype-typed Literal. Per-quad source-location attachment in the WS2.2 parsers is the load-bearing follow-on (new `ws2-source-map-parser-integration` entry below); ships the §Test discipline invariant #3 contract test alongside the parser integration."
    status: completed
    depends_on: [ws2-jsonld-compatible]
    sub_increment: 1a
  - id: ws2-source-map-parser-integration
    content: "WS2.3 follow-on (split out from `ws2-source-mapping` at the gate-1a pre-execution reviewer convergence, 2026-05-23). Per-quad source-location attachment in the WS2.2 parsers: every Quad produced by `parseJsonLdCompatible` and `parseTurtle` carries a resolvable `SourceLocation` reachable via `sourceMap.get(quadKey(q))`. Parser return-type changed from `Result<DatasetCore, ...>` to `Result<{ dataset, sourceMap }, ...>` per test-expert C4 atomic-landing requirement. **LANDED**: type-expert pre-execution RESHAPE (2026-05-23) on n3.js v2.0.3 source verified that per-quad token position is hidden from both the public + private APIs (`node_modules/n3/src/N3Parser.js:1079-1082` — `_emit` receives only quad terms). Cure: Option B (pre-split input + post-correlate by subject IRI scan); `TurtleSourceLocation` widened to `{ line: number | null; column: number | null }` because compound triples can be unambiguous-line-resolvable or null. JSON-LD parser ships **all-quads-map-to-root-pointer** at this cycle — `jsonld.toRDF` strips provenance and the recursive walker against `JsonLdValue` ran into jsonld's restrictive value union (cannot traverse without forbidden assertions or restricted `Object.keys`/`Object.entries`); precise per-`@id` JSON Pointer resolution is a future cycle when a real consumer needs it. Ships the §Test discipline invariant #3 contract test at `packages/libs/graph-ingest/src/invariant-3-source-path-preserved.unit.test.ts` — every Quad has a resolvable source location (existence; not precision). Reviewer flags: type-expert (n3 position-access verified) + test-expert (existence-only assertion shape per (c) BLOCK on specific-Turtle-path-value assertion)."
    status: completed
    depends_on: [ws2-source-mapping]
    sub_increment: 1a
  - id: ws2-jsonld-precise-source-paths
    content: "WS2.3 follow-on (further split out 2026-05-23 from `ws2-source-map-parser-integration` when the JSON-LD-walker authoring hit jsonld's restrictive value-union typing). Replace the current all-quads-map-to-root-pointer placeholder in `parseJsonLdCompatible` with precise per-`@id` JSON Pointer resolution. Requires a typed JSON-LD-tree walker that traverses without `Object.keys`/`Object.entries` (restricted by workspace ESLint) and without `Record<string, unknown>` cast or `unknown` parameter; likely shape is a custom `typeSafeJsonLdEntries<T>()` helper in `@oaknational/type-helpers` plus a recursive walker against the resolved value type. Worth landing when the first consumer of precise source paths (e.g., a teacher-facing diagnostic surface naming the source-document line of a citation) actually exists; per `consolidate-at-third-consumer` discipline, deferring while no consumer is justified."
    status: pending
    depends_on: [ws2-source-map-parser-integration]
  ...
  - id: ws3-property-graph
    ...
    sub_increment: 1a
  - id: ws3-adjacency
    content: "WS3.3: Property-graph adjacency primitives — LANDED at commit `f4ca84f6` (2026-05-21 residual-landing chore commit covering substantive adjacency authoring from a prior session; status flip recorded 2026-05-22 by Foamy Fathoming Compass / `ecb459`). 2 files: `packages/libs/graph-project/src/adjacency/{index.ts,index.unit.test.ts}`. Three free functions `outgoing(graph, nodeId)` / `incoming(graph, nodeId)` / `neighbours(graph, nodeId)` returning `readonly` arrays (not Iterables — PropertyGraph is fully materialised in-memory). Identity comparison via canonical `equals` from `@oaknational/graph-core/term` (structural BlankNode comparison preserved). Conceptual seam vs `DatasetCore.match()` preserved per cycle constraint — adjacency operates on PropertyGraph node→node traversal; quad-pattern matching stays on Dataset. Test suite 22 tests green in graph-project workspace including type-level test that adjacency operates on PropertyGraph types not Quad. Reviewer flags as originally specified (type-expert iterator API shape; architecture-expert-barney boundary check) absorbed at substance-authoring time in prior session. Unblocks WS4.5 EefStrandsGraphView `subgraph` BFS implementation. Original cycle text: WS3.3: Property-graph adjacency primitives (incoming/outgoing/neighbours) over the projected nodes-and-edges from WS3.2. **MUST NOT duplicate DatasetCore.match() semantics** — adjacency is node→node traversal over the property-graph projection; quad-pattern matching stays on Dataset (WS1.3). The Threads adapter (Inc.1b WS4.2) may choose either surface for its inverse-edge lookup; both must be available because they are distinct conceptual seams (research §10 fluent API on Dataset vs research §11 projection adjacency on PropertyGraph)."
    status: completed
    depends_on: [ws3-property-graph]
    sub_increment: 1a
  - id: ws4-graph-corpus-sdk-scaffold
    content: "WS4.1: Scaffold packages/sdks/graph-corpus-sdk workspace with a typed corpus-adapter boundary. LANDED — the workspace exists on main with the typed corpus-adapter boundary; the WS4.5 EefStrandsGraphView adapter landed within it via PR #114 at 77fcf746. Status flipped 2026-06-02 during the graph-estate consolidation t7 pass, verified by workspace presence (packages/sdks/graph-corpus-sdk/src/eef-strands + index.ts) and green gates on main."
    status: completed
    depends_on: [ws3-adjacency, ws2-source-mapping]
  - id: ws4-oak-ontology-thread-adapter
    content: "WS4.2: Oak Curriculum Ontology Threads adapter inside graph-corpus-sdk (generic Turtle/RDF/SKOS parsing via graph-ingest; curric:Thread enumeration and curric:includesThread inverse-edge mapping here); deterministic extraction test against the pinned straight-copy ontology raw import. Sequenced by consumer-readiness per ADR-173."
    status: pending
...
Every subsequent graph-shaped artefact — including the substrate substance
inherited from the archived `graph-query-layer.plan.md`, the eventual
...
The combinatorial-arc cross-plan scheduling relationship (previously phrased here as "Inc.3 design stability fires the combinatorial arc's promotion trigger") is owned by `graph-combinatorial-arc.plan.md`'s own Promotion Trigger and is not an open decision in this plan.
...
- **WS5 — Coordination amendments** (CLOSED 2026-06-02): executed in substance by the graph-estate consolidation — see the `ws5-coordination-amendments` todo entry.
...
| Plan | Coordination |
|---|---|
| [`../future/nc-knowledge-taxonomy-surface.plan.md`](../future/nc-knowledge-taxonomy-surface.plan.md) | **Parked.** The NC taxonomy plan is not amended or subsumed here. If the owner later promotes NC taxonomy work, it can attach to the substrate as a separate corpus adapter and consumer surface. |
| `graph-query-layer.plan.md` (archived superseded) | **Substrate substance inherited.** The `GraphView<TNode, TEdgeType>` interface and projection discipline migrated into `graph-core`/`graph-corpus-sdk` (WS4.4/WS4.5); the old 7-operation MCP-exposure framing is retired. MCP exposure remains a consumer-side decision tracked outside this plan. |
| `graph-resource-factory.plan.md` (completed, archived) | **Status remains DONE.** The factory currently lives in the curriculum MCP app's surface layer; it stays where it is and continues to work. If MCP exposure of new graph capabilities is later chosen, the factory may be revisited then. |
| `misconception-graph-mcp-surface.plan.md` (completed, archived) | **Status remains DONE.** The current misconception MCP tool stays live and unchanged. Its rewrite onto `graph-corpus-sdk` is owned by the unified substrate-migration plan (graph-estate-consolidation Judgement call 4). |
...
- Pinned `oak-curriculum-ontology` revision for Oak Ontology Threads extraction (foundation-increment WS4 dependency) — still open; required before WS4.2 begins.

**Related Plans**:

- [`../current/graph-estate-consolidation.plan.md`](../current/graph-estate-consolidation.plan.md) — the estate master plan; owns the unified substrate-migration plan (Judgement call 4) that replatforms the existing graph tools onto this substrate.
- `graph-query-layer.plan.md` (archived superseded) — its substrate substance landed via WS4.4/WS4.5.
- [`../future/nc-knowledge-taxonomy-surface.plan.md`](../future/nc-knowledge-taxonomy-surface.plan.md) — parked; not subsumed here.
