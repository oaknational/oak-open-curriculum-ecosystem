...
**2026-05-22 concurrent-execution relaxation (owner direction)**: substantive new graph implementation work (WS2.2, WS2.3, WS3.3, WS4.1, WS4.4, WS4.5) is now **explicitly authorised to proceed on this branch in parallel with the PR-108 snagging work**, under the cross-plan file-scope partition that prevents collisions. The "appended only with explicit owner direction" clause from the prior framing is now satisfied. Merge ordering — which PRs land in what order — is a session-time operational choice, not a planning constraint, and is not specified here.

**Meta-plan cross-reference**: a fresh session picking up this branch reads
`../../../feat-mcp-graph-support-foundation-meta.md` first — it names every plan currently in force on this branch, the cross-plan dependency picture, the file-scope partition, current state of work, and the open owner-class structural questions (PDR-063..066 routing Q1/Q2/Q3, ff1/ff2 owner actions, Cycle 8/10 contingencies).
