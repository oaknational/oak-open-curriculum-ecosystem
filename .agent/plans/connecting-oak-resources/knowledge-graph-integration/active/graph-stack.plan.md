...
**🛑 PR #108 MERGE BLOCKER (2026-05-21, Feathered Circling Horizon)**: PR #108 (`feat/mcp-graph-support-foundation` → `main`) carries the entirety of the Inc.1a substrate landings above plus the Inc.1d planning amendment-set. The branch is currently **failing two quality gates**: CodeQL alert #90 (`js/missing-rate-limiting`, false-positive on `createRequestLogger`) and SonarCloud Quality Gate (40 new issues, 12 unreviewed hotspots, 6.0% new-code duplication ≥ 3.0% threshold). **The PR must not merge until those gates clear.** The disposition strategy, per-finding ledger, and execution plan live in [`../current/pr-108-snagging.plan.md`](../current/pr-108-snagging.plan.md) (status `planning`, type `quality-fix`, 12 cycles across 3 phases). Per the `never-disable-checks` rule and the Sonar Disposition Policy, the snagging plan resolves findings by per-instance disposition (FIXED / FALSE_POSITIVE / SAFE) and policy-aligned mechanical exclusion — never by weakening the gate. **The snagging plan is the immediate next operational step on this branch; substantive new graph implementation work (WS2.2, WS2.3, WS3.3) must not land on PR #108 until the snagging plan completes** (the next graph implementation cycles should either land in a new PR after #108 merges, or be appended only with explicit owner direction).
