...
todos:
  ...
  - id: ws2-source-map-parser-integration
    content: "WS2.3 follow-on (split out from `ws2-source-mapping` at the gate-1a pre-execution reviewer convergence, 2026-05-23). Per-quad source-location attachment in the WS2.2 parsers: every Quad produced by `parseJsonLdCompatible` and `parseTurtle` carries a resolvable `SourceLocation` reachable via `sourceMap.get(quadKey(q))`. Parser return-type changes from `Result<DatasetCore, ...>` to `Result<{ dataset: DatasetCore; sourceMap: SourceMap }, ...>` (test-expert C4: atomic-landing of the return-type change MUST update existing parser test call sites in the same commit). Substantive implementation work that the typed primitives surface alone does not contain: (a) JSON-LD-compatible parser needs a custom document walker; the current `jsonld.toRDF(...)` route strips per-element provenance during the n-quads round-trip; (b) Turtle parser needs an n3.js StreamParser-based refactor; the current sync `parse(input, null)` callback does not expose line/column per quad. Ships the §Test discipline invariant #3 contract test (every Quad has a resolvable source location) at `packages/libs/graph-ingest/src/invariant-3-source-path-preserved.unit.test.ts`. Reviewer flags: type-expert (custom walker + StreamParser type-shape; SourceLocation discriminated union narrowing at consumer sites), test-expert (invariant #3 contract test design; fixture sizing for the per-quad coverage assertion)."
    status: pending
