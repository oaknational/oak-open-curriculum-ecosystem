...
Currently, the bulk sdk-codegen pipeline (`code-generation/typegen/bulk/`) uses
...
API pipeline: two upstream schemas, two sdk-codegen consumers, one
principle.

### Prerequisite: Schema Endpoint

The schema.json currently ships inside the bulk download archive. To
consume it at sdk-codegen time without downloading all bulk data, we need
...
1. **Request a `/api/bulk/schema` endpoint** from the API team that
   returns just the JSON schema (no bulk data). This enables sdk-codegen
   to fetch it alongside the OpenAPI schema.
2. **Fallback**: Commit `schema.json` to
   `packages/sdks/oak-sdk-codegen/schema-cache/bulk-schema.json`
...
- [ ] `pnpm sdk-codegen` produces equivalent or better output
