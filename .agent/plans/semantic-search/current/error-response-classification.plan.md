...
| File | Change |
|------|--------|
| New: `packages/sdks/oak-curriculum-sdk/src/classification/` | Error classification pure functions |
| `packages/sdks/oak-curriculum-sdk/src/mcp/execute-tool-call.ts` | Wire classification into MCP response handling |
| `apps/oak-curriculum-mcp-stdio/src/app/validation.ts` | Potentially pass HTTP status as discriminant |
| Generated: `mcp-tools/tools/*.ts` | May need changes to `validateOutput` signature |
