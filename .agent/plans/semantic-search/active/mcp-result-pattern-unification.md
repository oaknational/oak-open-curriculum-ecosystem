...
**Best started after**: Phase 3a WS2 (GREEN) is complete.
At that point the semantic-search tool exists, providing
a consumer that already uses `Result<T, E>` and can
validate the unification does not break anything.

**Timing advantage**: If WS5 (compare and replace) removes
the old `search` aggregated tool, several files in the
scope become simpler or are deleted entirely, reducing
the migration surface. However, this work should not
wait for WS5 — it can proceed as soon as WS2 is green.

**Does not block**: Phase 3a at any point. The
semantic-search tool bypasses `ToolExecutionResult`
entirely because it calls the Search SDK directly.
