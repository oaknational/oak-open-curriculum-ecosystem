...
**Parent**: [README.md](../README.md) | [roadmap.md](../roadmap.md) (Milestone 2)  
**Created**: 2025-12-28  
...
Pattern-aware ingestion affects data completeness, not search quality directly. However, measurement is still required:

1. **Before**: Record ES doc counts per subject/keystage via ES aggregation
2. **After**: Record ES doc counts post-ingestion
3. **Validate**: Compare against bulk download reference counts (see [evaluations/README.md](../../../evaluations/README.md))
4. **Record**: Document ingestion coverage in [current-state.md](../current-state.md)
5. **Search test**: Run `pnpm eval:per-category` to verify search still works on new data

**Success indicator**: ES counts match bulk download unique lesson counts (±3 lessons per subject)
...
- [Curriculum Traversal Strategy](../../curriculum-traversal-strategy.md)
- [Curriculum Structure Analysis](../../../analysis/curriculum-structure-analysis.md)
- [complete-data-indexing.md](./complete-data-indexing.md)
