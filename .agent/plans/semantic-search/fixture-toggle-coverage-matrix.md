...
| Playwright            | Responsive suite toggles fixture ↔ live mode, captures screenshots, and runs axe across bp-xs/md/lg/xxl.                                                                   | Playwright asserts empty-state copy remains visible, no overflow, and axe remains zero violations.           | Playwright scenario forces error fixtures, verifying tone, retry CTA, and absence of console errors.   | All runs export artefacts into `tests/results/semantic-search/` for review. |
