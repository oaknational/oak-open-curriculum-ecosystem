...
1. **Stabilise canonical MCP helper surface**
   ...
   - **Incremental actions:**
     ...
     4. Retain schema-derived strictness in `emit-schema.ts`/tool templates—path parameters stay required, query blocks are only optional when every entry is optional, and `ToolArgs` must remain required whenever parameters exist.
     5. Keep `tool-descriptor.ts` completely generic—no imports from generated data or aliases—and ensure `generate-types-file.ts` derives aliases strictly from `definitions.ts`.
     6. After each template change, run `pnpm type-gen` (repo root) and log the outcome in this plan before continuing. Only move ahead once regeneration succeeds and manual inspection confirms the expected structure.
     7. Once generator output is stable, run `pnpm build --filter @oaknational/oak-curriculum-sdk` to confirm duplicate exports and type collapses are resolved; if it still fails, loop back to the relevant template before touching runtime code.
     8. Rewrite generator unit tests to import the generated modules at runtime and assert observable behaviour (e.g., `getToolFromOperationId('getSequences-getSequenceUnits')` returns the expected descriptor), eliminating text-based checks.
     9. When updating runtime consumers, ensure each invocation path preserves the literal tool name (or descriptor type) so `ToolArgs` does not collapse to an intersection; add focused tests covering `executeToolCall` and registry lookups before rerunning `pnpm type-check`.
     10. Retire the legacy OpenAI connector path: delete the generator template, stop emitting `src/types/generated/openai-connector`, and surface the `search` / `fetch` aggregators via the universal MCP metadata so the SDK remains the single source of truth.
     11. Retire the legacy OpenAI connector path: delete the `type-gen/typegen/openai` templates, stop emitting `src/types/generated/openai-connector`, and surface the `search` / `fetch` aggregators via the universal MCP metadata so the SDK remains the single source of truth.
...
_Run these commands from the repo root with zero edits between them and **without** package filters. Always execute `pnpm clean`, `pnpm type-gen`, and `pnpm build` in that order before invoking `pnpm type-check`; the type-check step consumes the freshly built dist output and will fail if the build has not completed. Each failure informs the recovery steps above; log outcomes in this plan document._
...
5. `pnpm type-check` — enumerate downstream consumers still relying on removed exports (run **after** a successful `pnpm build` so compiled SDK artefacts are present).
...
- [ ] Step 3 — `pnpm test` (unfiltered, full workspace) green with behavioural generator tests. _Current focus:_ progress the SDK refinements now that the sandbox search-index expectation is aligned.
  ...
  - In-flight 2025-10-24 (Codex): refactoring type-gen to emit dereferenced response schemas and a generated request-parameter map without runtime assertions. Dynamic module loading has been removed in favour of a fully typed generator path; runtime helpers now read only the generated data. During the 2025-10-24 session we discovered that loosening generated `ToolParams`/`ToolArgs` types caused cascading build failures. **Action:** restore schema-derived strictness (required path/query where the schema demands it) and remove the runtime generic helpers so `execute-tool-call` and `McpToolRegistry` rely on the precise `ToolArgsForName`/`ToolClientForName` aliases. Pending follow-up: stabilise the new generator helpers, update unit tests to match dereferenced component naming, and re-run the SDK quality gates. Latest full root run: clean/type-gen/build/type-check all green; lint still red (legacy SDK/stdio issues + new `execute-tool-call.ts` assertion violation), repo-wide `pnpm test` still fails on MCP tool generator assertions.
