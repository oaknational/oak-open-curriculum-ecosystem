...
This violates the Cardinal Rule: *All structural knowledge should flow from the SDK at type-gen time.*
...
```mermaid
flowchart TB
    subgraph TypeGen["SDK Type-Gen (source of truth)"]
        direction TB
...
```typescript
// type-gen/typegen/search/subject-domain/subject-hierarchy.ts
...
```typescript
// type-gen/typegen/search/subject-domain/ks4-patterns.ts
...
```typescript
// type-gen/typegen/search/subject-domain/curriculum-pattern-config.ts
...
| File | Purpose |
|------|---------|
| `type-gen/typegen/search/subject-domain/subject-hierarchy.ts` | Subject parent-child relationships |
| `type-gen/typegen/search/subject-domain/ks4-patterns.ts` | KS4 complexity per subject |
| `type-gen/typegen/search/subject-domain/curriculum-config.ts` | 68 subject×keystage patterns |
| `type-gen/typegen/search/subject-domain/generator.ts` | Code generator |

### Modified Files

| File | Change |
|------|--------|
| `type-gen/typegen/index.ts` | Export new generator |
| `type-gen/typegen.ts` | Call new generator |
| `apps/.../bulk-transform-helpers.ts` | Import from SDK instead of local definitions |
| `apps/.../curriculum-pattern-config.ts` | Re-export from SDK (then delete) |
...
6. **Move curriculum-pattern-config.ts** — from app to SDK type-gen
7. **Implement generator.ts** — produce subject-domain.ts output
8. **Wire into typegen.ts** — run `pnpm type-gen -- --ci`
...
5. **Cardinal Rule compliance** — Running `pnpm type-gen` propagates changes
...
| [subject-hierarchy-enhancement.md](../active/subject-hierarchy-enhancement.md) | Current implementation (temporary, to be superseded) |
