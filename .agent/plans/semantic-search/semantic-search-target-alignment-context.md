...
_Last updated: 2025-09-26 (sequence facet instrumentation + theme coverage planning)_
...
- Phase 1 now emphasises extending theme/design token test coverage and using the Playwright MCP server to interrogate the dev UI before closing the milestone.
...
- Theme/design token coverage is incomplete: need targeted tests that assert colour/typography variables, component theming, and Oak token usage; redundant tests should be pruned.
- Playwright MCP investigation still pending to surface React/styling/UI issues in the running dev server (no Elasticsearch credentials required yet by design).
...
- **TDD enforcement**: follow `docs/agent-guidance/testing-strategy.md`; write failing tests first, run `pnpm test` to prove engineering changes, and keep `pnpm test:e2e` as the system-design oracle.
- **Credentials**: Elasticsearch keys are intentionally absent for now—defer live-cluster validation until Phase 1 UI/testing work is complete.
...
### Theme/design test coverage audit (2025-09-26)

- Existing coverage: `apps/oak-open-curriculum-semantic-search/app/lib/theme/bridge.integration.test.tsx:12` asserts the CSS variables style tag and token swaps generated by the ADR-045 bridge, while `apps/oak-open-curriculum-semantic-search/app/lib/theme/ThemeContext.integration.test.tsx:5`, `apps/oak-open-curriculum-semantic-search/app/ui/ThemeSelect.integration.test.tsx:10`, and `apps/oak-open-curriculum-semantic-search/app/lib/theme/theme-utils.unit.test.ts:5` cover mode switching, persistence, and cookie serialisation.
- Gaps: No unit test targets `app/lib/theme/ThemeCssVars.tsx` output or the per-mode `createLightTheme`/`createDarkTheme` tokens; search UI suites (`apps/oak-open-curriculum-semantic-search/app/ui/SearchResults.unit.test.tsx`, `apps/oak-open-curriculum-semantic-search/app/ui/SearchFacets.unit.test.tsx`, `apps/oak-open-curriculum-semantic-search/app/ui/SearchSuggestions.unit.test.tsx`, `apps/oak-open-curriculum-semantic-search/app/ui/StructuredSearchClient.unit.test.tsx`) assert behaviour only and never prove Oak token usage ($font, $color, spacing, aria attributes); admin/docs integration tests (`apps/oak-open-curriculum-semantic-search/app/admin/page.integration.test.tsx`, `apps/oak-open-curriculum-semantic-search/app/api/docs/page.integration.test.tsx`) likewise omit checks for Oak layout/typography tokens or theme data attributes.
- Next steps: Add focussed unit tests for `ThemeCssVars` and the theme factories, extend UI component tests with expectations around Oak data attributes or computed style variables, and introduce assertions that admin/docs shells expose the expected theme tokens alongside existing behavioural checks.
