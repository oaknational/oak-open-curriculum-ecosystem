...
13. ACTION [DONE 2025-10-05]: Replace the generic `parseWithSchema` helper with domain-specific curriculum/search parsing functions that infer types directly from generated schemas (introduced dedicated endpoint/search helpers and response schema map accessors).
14. REVIEW [DONE 2025-10-05]: Verify all curriculum/search validators now consume the specialised helpers and retain strict `z.infer` outputs without `unknown` (search + curriculum validators updated; request validators now call the endpoint-specific helper).
15. QUALITY-GATE [DONE 2025-10-05]: Run `pnpm -C packages/sdks/oak-curriculum-sdk type-check` to confirm the validation layer compiles cleanly (tsc passes after helper refactor).
