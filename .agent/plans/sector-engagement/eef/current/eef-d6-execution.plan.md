...
**D6 COMPLETE (2026-06-08, Briny Charting Lagoon).** c1–c3 + c6 tool-gating landed earlier
(`496ea7ca`, ADR-193 in `83d791e8`). This session added **c4** (`eef://interpretation`
`text/markdown` resource) and **c5** (`adapt-lesson` prompt), authored SDK-side in
`oak-curriculum-sdk/src/mcp/` and co-gated app-side behind `OAK_CURRICULUM_MCP_EEF_ENABLED`.
**c4/c5 take NO egress functions** — a `string` (resource `text`) and a `PromptMessage[]`
(prompt messages) do not originate from a strict domain type, so ADR-193 §Scope's egress
requirement does not apply (the earlier "each crossing via its own egress membrane" framing
was wrong). **Attribution correction (owner-directed this session):** the c3 provenance now
passes the full corpus `source` through (organisation, url, AND named authors) — authors are
attribution data, not PII; source URLs are never stripped (a trust requirement); the prior
`Omit<…,'original_authors'>` filter + absence-test are reversed. The c4 strand index carries
each strand's `eef_url`. Next: **D7**.
