...
readiness: >-
  ...
  then-installed SDK/ext-apps) runs first. Independently re-grounded first-hand
  2026-06-06; execution-precision refinements folded into c1/c2/c3/c4/c6
  (graph-corpus-sdk devDependency for the type-only import; the /eef-strands
  import subpath; the c4→c3 ordering + pnpm install; the handlers-tool-registration
  test reconciliation).
todos:
...
## Grounded facts inherited (re-verified first-hand 2026-06-05; independently re-grounded 2026-06-06)

| Fact | Verdict | Evidence |
|---|---|---|
| D5 substrate exports `inspectStrand` / `evidenceForMove` / `EefEvidenceEnvelope` + finite input domains | HOLDS | `graph-corpus-sdk/src/eef-strands/eef-evidence.ts:51,123,159`; sub-path barrel `eef-strands/index.ts:55-58` (root `src/index.ts` exports only GraphView/Result — import from `@oaknational/graph-corpus-sdk/eef-strands`); `EefStrandId` in `strand-lookup.ts:40`, observed domains in `raw-domains.ts` |
...
| `@oaknational/curriculum-sdk` does **not** depend on `graph-corpus-sdk` (its deps: `@modelcontextprotocol/sdk`, `@oaknational/{logger,result,sdk-codegen,type-helpers}`, `@zod/core`, `openapi-fetch`, `zod`) | HOLDS | `oak-curriculum-sdk/package.json` (forces type-only def+schemas; `c1` adds graph-corpus-sdk as a devDependency for the type-only import) |
...
```text
  ...
  commit 2 (cross-package, atomic):
    serial spine:   c1 -> c2 -> c3 -> c4 -> c6
    independent:    c5 -> c6   (prompt)
    c4 -> c3 (NOT parallel): the resource reads corpus values at runtime via
      @oaknational/graph-corpus-sdk/eef-strands, which the app gains only in c3
      (workspace dependency + pnpm install)
```

Serial spine: `c1 → c2 → c3 → c4 → c6`. Independent: `c5` (prompt), `c0`
(commit 1). `c4` follows `c3` (not parallel to it) because the resource reads
corpus values at runtime via `@oaknational/graph-corpus-sdk/eef-strands`, which
the app gains only in `c3` (workspace dependency + `pnpm install`).
...
- **Cross-package commit 2.** Mitigated by authoring order (SDK `c1`/`c2` green
  before app `c3`–`c6`) and the single atomic commit ending all-tests-green. No
  SDK rebuild step is needed: both packages expose a `development` export
  condition pointing at source and the root `tsconfig.base.json` sets
  `customConditions: ['development']`, so tsc + vitest see new SDK exports as live
  TypeScript source (verified first-hand 2026-06-06).
- **R6 (enumerated ≠ registered).** `listUniversalTools` always enumerates the
  co-gated EEF entry while the loop registers it only when the flag is on, so the
  existing blanket "every universal tool is registered" test
...
- **Coordination on `universal-tools/`** with the output-schemas plan — register
  an active claim before `c0`; coordinate the carrier field-shape (D6 c0's
  optional `AnySchema` supersedes the output-schemas S0.1 `z.ZodRawShape` line;
  that plan's text is updated when S0.1 executes, not now).
