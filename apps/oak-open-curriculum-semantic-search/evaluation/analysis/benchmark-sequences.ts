 ...
 *
 * Runs ground truth benchmarks against the oak_sequences index.
 *
...
/**
 * Production search adapter for sequences.
 */
const productionSequenceSearchAdapter: SequenceSearchFunction = async (request) => {
...
async function runBenchmark(): Promise<void> {
  ...
  if (entries.length === 0) {
    console.log('No sequence ground truths found. Ground truths need to be created first.');
    console.log('See: src/lib/search-quality/ground-truth/sequences/');
    process.exit(0);
  }
  ...
  for (const entry of entries) {
    ...
    const entryResults = await runEntryQueries(entry, productionSequenceSearchAdapter);
