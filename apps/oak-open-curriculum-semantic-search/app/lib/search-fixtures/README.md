...
- **`buildMultiScopeFixture`**: returns multi-bucket payload matching `SearchMultiScopeResponseSchema` with suggestions list.
...
- Use types imported directly from `@oaknational/oak-curriculum-sdk` (e.g. `SearchLessonsResponse`, `SearchMultiScopeResponse`, `SearchSuggestionItem`).
- Facets must conform to `SearchFacetsSchema` sourced from `@oaknational/oak-curriculum-sdk`.
...
- Legacy fixture file `app/ui/search/__fixtures__/search-structured.ts`: plan removal after adoption to prevent drift from SDK-derived shapes.
