...
function logDecisionGuidance(summary: MetricsSummary, targets: TargetResults): void {
  ...
  console.log('\nDECISION:');
  if (allPass) {
    console.log('✓ All targets met. Two-way hybrid is sufficient.');
    console.log('→ Skip Phase 2, proceed with current approach.');
  } else {
    logFailedTargets(summary, targets);
    console.log('→ Proceed to Phase 2 to evaluate three-way hybrid.');
  }
  console.log('='.repeat(60));
}
