...
import type { SearchLessonsIndexDoc, SearchUnitsIndexDoc } from '../types/oak';
...
export interface BatchProcessingResult {
  ...
  readonly operations: readonly (BulkIndexAction | SearchLessonsIndexDoc | SearchUnitsIndexDoc)[];
...
export async function processBulkFileBatch(
  ...
  const allOps: (BulkIndexAction | SearchLessonsIndexDoc | SearchUnitsIndexDoc)[] = [];
  ...
  for (const bulkFile of bulkFiles) {
    ...
    const ops = source.toBulkOperations(lessonsIndex, unitsIndex);
    allOps.push(...ops);
