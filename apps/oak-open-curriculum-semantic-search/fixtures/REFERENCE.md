...
All datasets originate from the Oak Curriculum SDK (OpenAPI aligned at `packages/sdks/oak-curriculum-sdk/src/types/generated/api-schema/api-schema-sdk.json`). Suggestions are manually curated to mirror Search API payloads defined by the indices, facets, and roll-ups under `apps/oak-open-curriculum-semantic-search/src/lib`.
...
These notes inform the forthcoming `app/ui/search-fixtures` module, ensuring builder outputs remain faithful to upstream schema constraints while delivering UX-relevant variety.
