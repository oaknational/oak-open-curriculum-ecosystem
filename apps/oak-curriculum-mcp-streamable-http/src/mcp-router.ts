...
export interface McpRouterOptions {
   ...
   * Typically `mcpAuthClerk` or similar OAuth middleware.
   */
 ...
 * const mcpRouter = createMcpRouter({ auth: mcpAuthClerk });
  ...
  return (req, res, next) => {
    ...
    if (method === 'tools/call' && toolName) {
      if (toolRequiresAuth(toolName)) {
        // OAuth tool: delegate to auth middleware
