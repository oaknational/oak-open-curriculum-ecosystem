...
| `ELASTICSEARCH_URL`     | ✅         | ✅      | Elasticsearch Serverless endpoint used by the search tools. Startup fails if absent                                                                                          |
| `ELASTICSEARCH_API_KEY` | ✅         | ✅      | Elasticsearch API key used by the search tools. Startup fails if absent                                                                                                      |
| `OAK_API_KEY`           | ✅         | ✅      | Oak Curriculum API key for live data                                                                                                                                         |
| `SENTRY_AUTH_TOKEN`     | ✅         | ✅      | Sentry organisation auth token. Required at **build time** by `@sentry/esbuild-plugin` (ADR-163 §6). Runtime Sentry SDK does NOT need it — events are delivered via the DSN. |
| `SENTRY_ORG`            | ✅         | ✅      | Sentry organisation slug for build-time release/source-map upload. Required; no source literal fallback.                                                                     |
| `SENTRY_PROJECT`        | ✅         | ✅      | Sentry project slug for build-time release/source-map upload. Required; no source literal fallback.                                                                          |
| `SENTRY_REPO_SLUG`      | ✅         | ✅      | Repository slug for commit attribution, e.g. `owner/repo`. Falls back only to Vercel Git metadata when available.                                                            |

## Optional Environment Variables

| Variable                               | Default Behaviour                                              | Usage                                                                                                                                                                                                                                                                                          |
| -------------------------------------- | -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ALLOWED_HOSTS`                        | The localhost entries plus every Vercel deployment URL present | Additive: names extra hosts for the DNS-rebinding guard. Unioned with the Vercel-derived hosts and the localhost entries, never substituted for them — the variable can add a host, it cannot take one away                                                                                    |
| `CANONICAL_HOST`                       | Unset — the app self-describes from each request's Host        | Set to the address the app is served at when an edge overrides the Host (MCP-172). Every OAuth metadata/resource/challenge URL then names `https://<value>`, and the app states that host in the `x-forwarded-host`/`x-forwarded-proto` pair Clerk derives its handshake origin from (MCP-517) |
| `REMOTE_MCP_MODE`                      | `stateless` (recommended)                                      | See "MCP Transport Modes" section below for detailed explanation                                                                                                                                                                                                                               |
| `LOG_LEVEL`                            | `info`                                                         | Controls server-side log verbosity; server-side logging tidy-up tracked in the backlog                                                                                                                                                                                                         |
| `DANGEROUSLY_DISABLE_AUTH`             | **Must remain unset/`false`**                                  | Local-development valve only. Setting it to `true` in any Vercel environment fails startup — the app refuses to boot rather than serve unauthenticated                                                                                                                                         |
| `OBSERVABILITY_SINKS`                  | `[]`                                                           | JSON-array sink list. Use `["sentry"]` for live Sentry; adding `posthog` enables live capture (runtime composition landed with MCP-241, 2026-07-29) and validates the four PostHog values below at boot; production also requires a diagnostic sink (`sentry` or `file`) alongside it          |
| `POSTHOG_PROJECT_API_KEY`              | —                                                              | **Secret** (required when `posthog` selected): project ingestion key (`phc_`-class). Validated at bootstrap and consumed by the adapter (runtime composition landed with MCP-241); stripped from handler-facing runtime config, never logged or serialised                                     |
| `POSTHOG_PSEUDONYM_KEYRING`            | —                                                              | **Secret** (required when `posthog` selected): JSON array of `{"id","key"}` records — ids match `^[a-z0-9][a-z0-9_-]{0,31}$`, keys are 43-char unpadded base64url of exactly 32 bytes, ids and key material unique. Bootstrap-only                                                             |
| `POSTHOG_PSEUDONYM_ACTIVE_KEY_ID`      | —                                                              | Required when `posthog` selected: keyring id used for new actor projections; must resolve exactly one entry. Bootstrap-only, stripped from handler-facing config                                                                                                                               |
| `POSTHOG_HOST`                         | —                                                              | Required when `posthog` selected: must be exactly `https://eu.i.posthog.com` (handler-facing value class, not a secret; still stripped)                                                                                                                                                        |
| `POSTHOG_CAPTURE_MODE`                 | Must remain unset                                              | Not a supported input: any non-empty value fails startup when `posthog` is selected — the capture transport is fixed by the reviewed adapter                                                                                                                                                   |
| `OBSERVABILITY_FIXTURES`               | `false`                                                        | Fixture tee flag for post-redaction event capture                                                                                                                                                                                                                                              |
| `SENTRY_DSN`                           | —                                                              | Required when `OBSERVABILITY_SINKS` includes `sentry`; Sentry project DSN                                                                                                                                                                                                                      |
...
- **Production Deployment**: Mirrors the preview configuration. Provide explicit `ALLOWED_HOSTS` only when a host outside Vercel's system environment variables must be served — a custom domain pointed straight at the deployment, say. The value extends the derived defaults and cannot replace them, so the platform keeps supplying the deployment URLs as they change. Production also has a repo-owned ignore-build gate so non-release commits do not create ghost releases under an unchanged semantic-release version.
...
- You don't need to manually configure `ALLOWED_HOSTS` for standard Vercel deployments, and configuring it for a custom domain does not cost you these — the configured value is added to them
