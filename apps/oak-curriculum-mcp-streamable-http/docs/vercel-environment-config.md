...
| `SENTRY_MODE`                          | `off`                                                                                             | Gates real Sentry delivery: `Sentry.init` is reached only when the value is exactly `sentry` (`off` yields a noop runtime, `fixture` an in-memory one). Selecting `sentry` in `OBSERVABILITY_SINKS` without setting this to `sentry` boots the sink dark                                                                                                          |
| `OBSERVABILITY_SINKS`                  | `[]`                                                                                              | JSON-array sink list — a selection marker, not a delivery switch. Select `sentry` **and** set `SENTRY_MODE=sentry` for live Sentry; adding `posthog` enables live capture (runtime composition landed with MCP-241, 2026-07-29) and validates the four PostHog values below at boot; production also requires a diagnostic sink (`sentry` or `file`) alongside it |
