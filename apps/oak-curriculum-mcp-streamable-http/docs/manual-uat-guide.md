...
| 1.6 | Agent registration (MCP-759)           | `curl -sS ORIGIN/.well-known/oauth-authorization-server \| jq .agent_auth`, then `curl -sS -o /dev/null -w '%{http_code}\n' "$(curl -sS ORIGIN/.well-known/oauth-authorization-server \| jq -r .agent_auth.skill)"`                                                                            | First call: `{ "skill": "ORIGIN/auth.md" }`. Second call: `200` — the pointer resolves, never a `404`.                                                                                            |
