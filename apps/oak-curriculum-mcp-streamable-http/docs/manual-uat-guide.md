...
| 7.4 | `get-keyword-graph`         | `{ subject: 'maths', keyStage: 'ks2', limit: 5 }` → ranked keywords, honest totals (`totalMatchingKeywords`, `hasMore`), each keyword's `definitions` as its in-scope lessons authored them (each with `scopedLessonCount`, up to three `lessonSlugs`, `hasMoreLessons`); `{ subject: 'art', keyStage: 'ks3', lessonSlugs: ['exploring-portraits-through-paint'] }` → `subject` defined in its art sense only | `limit: 0` → JSON-RPC `-32602` (the schema declares integer [1, 100], so the bound is enforced at the input layer). |
