...
- `GET /auth.md` is served at the service root (`src/auth-md.ts`), public in every auth mode, and documents the registration flow this server actually offers an autonomous agent: RFC 8414/RFC 9728 discovery, open RFC 7591 dynamic client registration at `/oauth/register`, and a standard Authorization Code + PKCE grant (ADR-052, ADR-115, ADR-142). **This origin's own `/oauth/authorize` / `/oauth/token` do not satisfy RFC 9207 for a client that validates the authorization response's `iss`** — the proxy's own AS metadata names this origin as `issuer`, but the relayed authorization response carries the upstream's own `iss` (ADR-115 "Negative 8", a pre-existing, ratified trade-off this ticket does not touch). `/auth.md` states this and points such a client at the upstream's own AS metadata and endpoints (discovered via the PRM's `authorization_servers[0]`) instead.
...
**Traffic controls**: Rate limiting and volumetric abuse controls are
configured and owned at the Cloudflare and Vercel edge; the application
runs no in-process rate limiter. The read-only blast radius (all MCP
tools are read-only) means a bypass cannot mutate state — the worst case
is upstream load and Vercel compute spend, both bounded at the edge. See
[ADR-219](../../docs/architecture/architectural-decisions/219-rate-limiting-is-an-edge-concern.md).

**Documentation Status**: Last verified 2026-07-30 against `src/application.ts`,
