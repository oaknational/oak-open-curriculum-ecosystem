...
| `LOG_LEVEL`                         | ❌        | `info`                                                                                                                   | Set to `debug` to enable detailed request logging and transport diagnostics.         |
| `MCP_STREAMABLE_HTTP_FILE_LOGS`     | ❌        | `false`                                                                                                                  | When `true`, the server writes JSON log lines to the file sink below.                |
| `MCP_STREAMABLE_HTTP_LOG_FILE`      | ❌        | `apps/oak-curriculum-mcp-streamable-http/.logs/dev-server.log`                                                           | Overrides the file sink path; only used when `MCP_STREAMABLE_HTTP_FILE_LOGS=true`.   |
