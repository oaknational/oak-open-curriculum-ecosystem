...
/**
 * WS1 RED: CLI composition-root constructs a registry from the new env shape.
 *
 * @remarks Pins the WS5 contract — `createCliObservability` exposes a
 ...
 * operator so the test stays import- and type-safe until WS5 widens the
 * CLI observability shape.
 *
 * Failure mode: `'registry' in observability` is `false` on the legacy
 * shape — clear "consumer not yet wired" canary.
 *
 * **WS5 GREEN tightening (test-reviewer 2026-05-02 finding 3)**: once
 ...
 * deliberately weak, with the WS5 follow-up extension explicit.
 *
 * @see ../../../../../.agent/plans/observability/current/observability-multi-sink-and-fixtures-shape.plan.md
 */
// SKIP-UNTIL-WS5: Temporarily skipped to keep the trunk green during the
// multi-commit RED arc. The WS5 commit that adds `OBSERVABILITY_SINKS`
// to the search-cli runtime config and constructs a SinkRegistry on
// the cli observability composition root MUST unskip this describe
// block as part of its landing diff. Tracking note in
// .agent/memory/active/napkin.md §RED-arc skip register.
describe.skip('createCliObservability — WS5 SinkRegistry construction (RED)', () => {
  it('exposes a registry property derived from OBSERVABILITY_SINKS (not SENTRY_MODE)', () => {
    ...
    expect('registry' in result.value).toBe(true);
  });
  ...
  // to drop on the WS5 floor (test-reviewer 2026-05-02 finding P2-2).
  it.todo(
    'WS5 GREEN contents-check: registry.sentry is defined when OBSERVABILITY_SINKS=["sentry"]; registry.file when ["file"]; both when ["sentry","file"]; neither when [] — and each entry is a real ObservabilitySink, not a stub',
  );
});
